...
export function admitWorkflowExecution(value: unknown): WorkflowExecutionResponse {
  const response = expectJsonObject(value, "workflow execution response");
  if (!expectJsonBoolean(response.success, "workflow execution response.success")) {
    throw new TypeError("workflow execution response.success must be true");
  }
  if (!Object.hasOwn(response, "result") || response.result === undefined) {
    throw new TypeError("workflow execution response.result is required");
  }

  const duration = response.duration === undefined
    ? undefined
    : expectFiniteJsonNumber(response.duration, "workflow execution response.duration");
  const runId = response.runId === undefined
    ? undefined
    : expectJsonString(response.runId, "workflow execution response.runId", 512, false);
  const status = response.status === undefined
    ? undefined
    : expectJsonString(response.status, "workflow execution response.status", 128, false);
  return {
...
function prettyPrintWorkflowResult(value: unknown): string {
  const rendered = JSON.stringify(value, null, 2);
  if (rendered === undefined) throw new TypeError("workflow result is not JSON");
  return rendered;
}
...
function WorkflowExecutor({
  ...
  useEffect(() => {
    ...
    setResult(null);
    setLoading(false);
  }, [workflowId, inputSchema, requests]);
  ...
  async function startWorkflow(): Promise<void> {
    ...
    await runOwnedRequest(
      requests,
      (signal) =>
        requestJson("/_dev/api/start-workflow", {
          responseLabel: "Start workflow",
          admit: admitWorkflowExecution,
          init: {
            method: "POST",
        ...
        }),
      {
        ...
        success: (response) =>
          setResult({
            success: true,
            data: prettyPrintWorkflowResult(response.result),

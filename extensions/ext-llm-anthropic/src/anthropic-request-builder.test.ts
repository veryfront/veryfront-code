...
describe("ext-llm-anthropic/anthropic-request-builder", () => {
  ...
  it("serializes hosted native tool search only on compatible models", () => {
    const tools = [
      {
        ...
        nativeToolSearch: { mode: "hosted" as const, variant: "bm25" as const },
      },
      {
        type: "function" as const,
        name: "get_release",
    ...
    const supported = buildAnthropicMessagesRequest(
      "claude-opus-4-6",
      "anthropic",
      {
    ...
    const unsupported = buildAnthropicMessagesRequest(
      "claude-opus-4-1",
      "anthropic",
      {
        prompt: [{ role: "user", content: [{ type: "text", text: "Find a tool" }] }],
    ...
    const clientControlled = buildAnthropicMessagesRequest(
      "claude-opus-4-6",
      "anthropic",
      {
        prompt: [{ role: "user", content: [{ type: "text", text: "Find a tool" }] }],
        tools: tools.map((tool) =>
          tool.name === "tool_search"
            ? { ...tool, nativeToolSearch: { mode: "client" as const } }
            : tool
        ),
      } as never,
      false,
    ...
    const providerOptionBypass = buildAnthropicMessagesRequest(
      "claude-opus-4-6",
      "anthropic",
      {
        prompt: [{ role: "user", content: [{ type: "text", text: "Find a tool" }] }],
        ...
        providerOptions: {
          anthropic: { toolSearch: { mode: "hosted", variant: "bm25" } },
        },
      } as never,
      false,
      createWarningCollector(),
  ...
  it("replays Anthropic tool-search provider blocks without synthesizing tool results", () => {
    const prompt = [
      { role: "user", content: [{ type: "text", text: "Find release tools" }] },
      {
        role: "assistant",
        content: [
          {

...
| `MLFLOW_EXPERIMENT_NAME`                        | No       | Experiment name override for environment-based activation. A configured `trackingUri` ignores this variable and defaults from project/eval context, then `veryfront-evals`, unless config sets `experimentName`.            |
| `MLFLOW_RUN_NAME`                               | No       | Run name override for environment-based activation. A configured `trackingUri` ignores this variable and defaults to `<definitionId>-<runId>` unless config sets `runName`.                                                 |
| `MLFLOW_ARTIFACTS_URI`                          | No       | HTTP(S) MLflow artifact proxy endpoint for environment-based activation. A configured `trackingUri` ignores this variable and uses the run artifact root or tracking server proxy unless config sets `artifactsUri`.        |
...
For OAuth, configure `oauthTokenUrl`, `oauthClientId`, `oauthClientSecret`, and
the optional `oauthScope` together. A configured token endpoint is never paired
with `MLFLOW_OAUTH_CLIENT_ID` or `MLFLOW_OAUTH_CLIENT_SECRET` from the host.
...
A configured `trackingUri` activates the exporter directly; otherwise

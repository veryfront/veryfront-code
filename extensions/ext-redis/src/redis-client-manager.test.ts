...
describe("redis-client", () => {
  ...
  it("shares one in-flight connection for equivalent options", async () => {
    ...
    const first = manager.getClient({ url: "rediss://cache.example.test" });
    const second = manager.getClient({ url: "rediss://cache.example.test" });
  ...
  it("does not reuse a client created with different connection options", async () => {
    ...
    const first = await manager.getClient({ url: "rediss://first.example.test" });
    const second = await manager.getClient({ url: "rediss://second.example.test" });

    assertEquals(first === second, false);
    assertEquals((first as FakeRedisClient).disconnectCalls, 0);
    assertEquals(await manager.getClient({ url: "rediss://first.example.test" }), first);
  ...
  it("connects different configurations independently without head-of-line blocking", async () => {
    ...
    const manager = createRedisClientManager({
      ...
      loadFactory: () =>
        Promise.resolve((options) =>
          options.url === "rediss://first.example.test" ? firstClient : secondClient
        ),
    });

    const first = manager.getClient({ url: "rediss://first.example.test" });
    const second = manager.getClient({ url: "rediss://second.example.test" });
  ...
  it("reuses an active client and reconnects only after explicit disconnect", async () => {
    ...
    const first = await manager.getClient({ url: "rediss://cache.example.test" });
    assertEquals(await manager.getClient({ url: "rediss://cache.example.test" }), first);
    assertEquals(createCount, 1);

    await manager.disconnect();
    assertEquals((first as FakeRedisClient).disconnectCalls, 1);
    assertEquals(await manager.getClient({ url: "rediss://cache.example.test" }) === first, false);
    assertEquals(createCount, 2);
  });

  it("resolves connection configuration from the environment", async () => {
    let received: RedisClientFactoryOptions | undefined;
    const env = new Map([["REDIS_URL", "rediss://cache.example.test"]]);
    const manager = createRedisClientManager({
  ...
  it("cancels and disposes a connection that completes after disconnect", async () => {
    ...
    const pending = manager.getClient({ url: "rediss://cache.example.test" });
    ...
    assertEquals(await manager.getClient({ url: "rediss://cache.example.test" }), fresh);
  });
  ...
  it("cancels all differently configured requests active before disconnect", async () => {
    ...
    const first = manager.getClient({ url: "rediss://first.example.test" });
    const queued = manager.getClient({ url: "rediss://queued.example.test" });
  ...
  it("disposes a client whose connection attempt fails", async () => {
    ...
    await assertRejects(
      () => manager.getClient({ url: "rediss://cache.example.test" }),
      Error,
      "connect failed",
    );
  ...
  it("applies a failure cooldown before retrying the same connection", async () => {
    ...
    currentTime += 5_000;
    await manager.getClient({ url: "rediss://cache.example.test" });
    assertEquals(createCount, 2);
  });

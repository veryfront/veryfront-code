...
function resolveClientOptions(
  ...
  if (configuredUrl === undefined || configuredUrl.length === 0) {
    throw new TypeError("Redis client requires an explicit URL or REDIS_URL");
  }
  const url = requireRedisUrl(configuredUrl);
...
export function createRedisClientManager(
  ...
  function isConfigured(options: RedisClientOptions = {}): boolean {
    const configured = options.url ?? readEnv("REDIS_URL");

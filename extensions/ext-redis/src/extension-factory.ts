/** Internal dependency seam for the Redis aggregate extension factory. */
...
import { registerOwnedDistributedCacheKeyPrefix } from "veryfront/extensions/distributed/cache-support";
...
export interface RedisExtensionRuntimeDependencies {
  createClientManager(): RedisClientManager;
}

type RedisExtensionRuntime = Required<Pick<Extension, "setup" | "teardown">>;

const defaultDependencies: RedisExtensionRuntimeDependencies = {
  createClientManager: createRedisClientManager,
};
...
/** Build only the stateful runtime hooks behind the public extension factory. */
export function createRedisExtensionRuntime(
  options: RedisExtensionOptions = {},
  dependencies: RedisExtensionRuntimeDependencies = defaultDependencies,
): RedisExtensionRuntime {
  ...
  return {
    setup(ctx) {
      ...
      active = true;
      sharedManager = dependencies.createClientManager();
      resources = [];
      ...
      const provider: DistributedRuntimeProvider = {
        ...
        async createCacheBackend(options) {
          requireActive();
          registerOwnedDistributedCacheKeyPrefix(options.keyPrefix);

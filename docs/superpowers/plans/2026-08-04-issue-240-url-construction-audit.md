...
## Not a dependency emitter

`rewriteVendorImports` (`src/transforms/esm/import-rewriter.ts:126-146`) emits `${moduleServerUrl}/_vendor.js?v=${vendorBundleHash}` and rewrites only members of `REACT_PACKAGES`. It is first-party, hash-busted, and never carries a third-party dependency, so it is outside this criterion entirely. An earlier revision of this document grouped it with `rewriteBareImports`; that was wrong.

## Follow-up recommended, not taken here

Delete `rewriteBareImports` and `ModuleResolver`, or route them through the strategy pipeline. Both are public API surface with no internal caller, and each carries its own divergent copy of specifier-to-URL logic. Leaving them is safe today because nothing in the serving path reaches them, but they are exactly where a future caller would silently reintroduce floating dependencies.

Out of scope for this milestone for two reasons. Deleting a public export is a breaking change that deserves its own decision. And pinning a code path that no caller reaches cannot be exercised by an honest test, so the change would ship unverified: the emission wall in `src/transforms/import-rewriter/__tests__/no-unversioned-emission.test.ts` covers the strategies that actually serve requests. Recording them here is the approved exception, not an oversight.

...
- [ ] Resolve relative, absolute `file:`, root import-map, and scoped import-map aliases using the installed Deno runtime's import-map semantics. Normalize URL-like keys and requests in URL space against the correct map base/importer, select the most-specific applicable mapping, and apply that selected address exactly once; never recursively feed an import-map target back through the same map. Preserve the requested identity, canonical matched key/provenance, and selected terminal address as ordered evidence, then continue ordinary local or manifest-self traversal from that terminal. Reject malformed/non-URL-like addresses, prefix key/address slash mismatches, ambiguity, noncanonical encoding, symlink/path escape, and unsupported configuration as operational failures. Validate both the requested identity and selected address under target policy. Derive permitted bare self-package names and exported subpaths from the manifests owned by each production context; private `#` aliases and self identities are valid only when their one selected address terminates inside core. Allow a resolved target only when it remains under `src/` or `cli/`, or is a builtin supported by that production target. Reject mappings into `extensions/**`, every `@veryfront/ext-*`, every `jsr:*`, `npm:*`, HTTP(S), `data:`, `blob:`, third-party bare package identity, and any file URL outside the declared core roots.

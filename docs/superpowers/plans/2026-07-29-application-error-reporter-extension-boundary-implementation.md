...
**Architecture:** Application code explicitly constructs configured extensions. The generic extension kernel owns one reusable, runtime-authenticated exclusive-configured-contract lease; an observability adapter supplies the `ApplicationErrorReporter` definition and application-error activation policy without leaking reporter concepts into the registry, loader, or generic orchestrator. The application-error facade resolves only the lease-owned, generation-current contract, while each Sentry extension owns SDK initialization, policy, bounded flush, close, and retryable teardown. Source, package, graph, binary, and SBOM gates inspect actual reachable artifacts rather than package names or lockfile strings.
...
- Any capability that requires third-party runtime code must cross a first-party contract and be implemented by a separately packaged extension selected explicitly by application composition. Core may not own, import, probe, select, recommend, or bundle that implementation.
...
- `ApplicationErrorReporter`, `applicationErrors`, capture/flush behavior, and provider terminology belong to the observability contract/adapter. Generic extension registry, loader, and orchestration modules may know only the reusable exclusive-configured-contract definition/lease semantic.
...
- Task 11 completes the reporter extraction and a complete owner backlog only. The parent module-hardening goal remains active and the branch remains merge/release-ineligible until every owner extraction is implemented and canonical source, production-manifest, semantic-provider, npm, browser, and binary inventories all prove zero third-party core edges.
...
- Keep the work-in-progress branch merge- and release-ineligible until the strict generic source, production-manifest, semantic-provider, and artifact gates reach zero. Before that point, the strict commands are explicit expected-red evidence commands and are not substituted for the existing aggregate gate; this preserves green recovery checkpoints without hiding the debt. No baseline, suppression, or exception file may make an expected-red command pass.
- Tasks 1–8 deliver independently recoverable reporter prerequisites. Task 9 performs one atomic source-plus-artifact cutover: its Phase A is never committed or pushed separately, and its Phase B verifies and commits the exact union. Task 10 builds fresh generic source, production-manifest, semantic-provider, npm, browser, and binary evidence and freezes an owner-specific extraction backlog; Task 11 documents this breaking reporter change and regenerates that evidence after its final package/config inputs change. If the final inventory reports any non-Sentry edge, this plan ends at that explicit phase boundary and separate exact extraction plans must remove every edge before gate promotion, release-artifact work, or final module verification.
...
- [ ] Add a script-owned AST collector using exactly pinned `npm:@babel/parser@7.29.2` in `scripts/test.deno.json`/`scripts/deno.lock`. Walk `.ts`, `.tsx`, `.mts`, `.cts`, `.js`, `.jsx`, `.mjs`, and `.cjs` production files. Collect static imports/exports, type imports, dynamic imports, `import("...")` type expressions, TS import-equals declarations, triple-slash type references, CommonJS `require()`/`require.resolve()`, aliases returned by `createRequire(import.meta.url)`, `import.meta.resolve()`, Web and Node workers, service-worker registration, `AudioWorklet.addModule()`, CSS worklet module loading, `importScripts()`, `module.register()`, semantic generated code executed by eval/Function-family/`node:vm`/string-timer surfaces, connected executable DOM/JSX script elements, and source locations. Parse generated strings with their real classic/module/function-body grammar and lexical provenance; raw identifier/property spelling is never sufficient. Resolve API bindings through imports/aliases rather than matching callee spelling, including aliased `node:worker_threads.Worker`. Model raw dynamic-import promises as first-class finite provenance values, project them into namespace facts only when awaited, and retain provenance through aliases, assignments, local containers, and `Promise.all`. Invocation normalization must retain explicit `thisArg` capability inputs for higher-order `call`/`apply` boundaries. Every filesystem read, parse, binding-resolution, traversal, generated-source, or non-convergence failure is a structured fatal audit error. Deterministic work counters must bound the monotone binding/container analysis; timeouts are hang guards, not complexity proof. The core audit must not import an extension implementation to parse its own boundary.
...
- [ ] Resolve relative, absolute `file:`, root import-map, and scoped import-map aliases using the installed Deno runtime's import-map semantics. Graph nodes retain both the full logical importer identity (including empty/nonempty query/hash decorations used by scope matching) and the decoration-stripped physical source path; parsing may cache by physical path, but resolution/traversal is keyed by the pair. Resolve and classify each production entrypoint through its applicable one-hop import map before queueing it. Normalize URL-like keys and requests in URL space against the correct map base/importer, select the most-specific applicable mapping, and apply that selected address exactly once; never recursively feed an import-map target back through the same map. Preserve requested identity, canonical matched key/provenance, original selected address, logical terminal, and physical terminal as ordered evidence. Strip decorations only for physical reads of import-map/config/export files. Validate local `file:` authority before every conversion (`localhost` is local; any other authority is incomplete evidence), use shared containment that rejects absolute `relative()` results/cross-drive/UNC escape, and reject duplicate JSON keys—including escaped-equivalent keys—before `JSON.parse` normalization. Deno-accepted percent-encoded URL-like/path spellings remain complete policy evidence rather than being discarded; opaque bare keys such as `#exact%31` remain opaque and are not path-validated. Reject malformed addresses, empty import-map keys, prefix slash mismatches, ambiguity, and unsupported configuration as operational failures. Derive permitted bare self-package names and exported subpaths from the manifests owned by each production context; private `#` aliases and self identities are valid only when their selected address terminates inside core. Allow a resolved target only when it remains under `src/` or `cli/`, or is a target-supported builtin. Reject mappings into `extensions/**`, every `@veryfront/ext-*`, every `jsr:*`, `npm:*`, HTTP(S), `data:`, `blob:`, third-party bare identity, and any file URL outside declared roots.
...
- [ ] Enumerate configuration-owned code edges for every registered production context: import-map `imports` and `scopes`, exports, runtime dependency/peer/optional/bundled fields, package roots, `jsxImportSource`, `jsxImportSourceTypes`, and `compilerOptions.types`. Synthesize only the runtime selected by the active JSX mode, append Deno's runtime suffix textually without trimming the configured source, and never invent a base or inactive development-runtime load. A vendor mapping is development-only only when resolved reachability proves no production context consumes it. Export one shared builtin classifier: only names present in the runtime's canonical `node:` builtin set are importable builtins for Node/Deno contexts; browser permits none; `universal` requires acceptance by all three targets. Deno normalizes URL scheme case for `node:` while Node policy remains target-accurate. Deno globals create no import edge. Reject fake `node:` names, `jsr:`, `deno:`, `ext:`, bare, and unknown schemes; table tests cover each plus `node:fs` under Node, Deno, browser, and universal.
...
- Create: `src/observability/application-error-contract-internal.ts`
- Create: `src/observability/application-error-contract-internal.test.ts`
...
- [ ] Add one generic internal exclusive-configured-contract definition and lease. `defineExclusiveContract(name, assertImplementation)` atomically reserves the name in an internal process-wide definition registry and returns its runtime-authenticated opaque definition; redefining an already reserved name with a distinct definition fails. `acquireExclusiveContractLease(definition, runtimeOwner, loaderGeneration, provider)` returns a runtime-authenticated lease bound to the exact direct provider. Neither definitions, the reserved-name registry, leases, minting, publication, revocation, release, nor versioned resolution are exported from public barrels or package exports. Do not implement a policy-flag matrix: this semantic always means one direct concrete configured provider, no required contracts, setup before ordinary extensions, teardown after them, and no public/manual/primed/built-in/raw-loader/legacy-discovery publication path.

- [ ] Public `register()`, `unregister()`, and `reset()` from `veryfront/extensions/contracts` may mutate only ordinary/manual contracts; every exclusive definition name is protected. Exclusive entries record `{ implementation, registrationGeneration, leaseId, runtimeOwner, loaderGeneration }`. `publishExclusiveContract()` validates the implementation and exact provider-bound lease; `revokeExclusiveContract()` and `releaseExclusiveContract()` compare the unforgeable lease identity so stale teardown can never clear or release a newer generation. `tryResolveExclusiveVersioned(definition)` returns only the current lease-owned implementation. Test forged definitions/leases, public/manual mutation, re-registering the same implementation, stale revoke/release, and raw-loader/legacy/prime/built-in/provider-identity bypasses.

- [ ] Define `ApplicationErrorReporterContract` only in `src/observability/application-error-contract-internal.ts` by combining `ApplicationErrorReporterName` with `assertApplicationErrorReporter`. No generic file under `src/extensions/` may mention `ApplicationErrorReporter`, `applicationErrors`, capture/flush behavior, or Sentry. The dispatcher injects or imports this definition and resolves it with `tryResolveExclusiveVersioned()` rather than a reporter-named registry function.
...
- [ ] Run:
  ...
  ```bash
  set -euo pipefail
  deno test --frozen --allow-all src/extensions/observability/application-error-reporter.test.ts src/extensions/contract-registry-internal.test.ts src/extensions/contracts.test.ts src/observability/application-error-contract-internal.test.ts src/observability/application-error-dispatcher.test.ts
...
- [ ] Commit and push:
  ...
  ```bash
  set -euo pipefail
  git add src/extensions/observability/application-error-reporter.ts src/extensions/observability/application-error-reporter.test.ts src/extensions/observability/index.ts src/extensions/contract-registry-internal.ts src/extensions/contract-registry-internal.test.ts src/extensions/contracts.ts src/extensions/contracts.test.ts src/observability/application-error-contract-internal.ts src/observability/application-error-contract-internal.test.ts src/observability/application-error-dispatcher.ts src/observability/application-error-dispatcher.test.ts scripts/lint/extension-source-metadata.ts scripts/lint/audit-extension-contracts.test.ts
...
- Create: `src/observability/application-error-extension-runtime.ts`
- Create: `src/observability/application-error-extension-runtime.test.ts`
- Modify: `deno.json`
...
- [ ] Add a serialized `orchestrateConfiguredExtensions()` entrypoint that reuses the same active-loader generation coordinator as `orchestrateExtensions()`, snapshots all enumerable data properties before side effects, and performs no package/project/local discovery. Define and export its complete vendor-neutral API before implementation:
  ...
  `ExtensionRuntimeOwner` is an opaque token returned by `createExtensionRuntimeOwner(label)`. The coordinator permits replacement only by the same owner (hot reload/retry); a different live server/proxy/agent owner fails before teardown, and successful final teardown releases that owner. The legacy discovery orchestrator shares the coordinator and cannot displace a configured runtime. Update shared orchestration option types to readonly where they consume the same values. Importing and constructing the application's explicit config occurs before this API and is not misrepresented as lazy loading. Add public-export/compile fixtures for every option plus concurrent-owner rejection, same-owner replacement, failed-teardown retention, and sequential-owner tests.

- [ ] Add a package-private `orchestrateConfiguredExtensionsInternal(options, exclusiveContracts)` entrypoint whose second argument is `readonly ExclusiveContractActivation<unknown>[]`, where each activation is a runtime-authenticated definition plus the exact state `"active"` or `"inactive"`. The public entrypoint delegates with `[]` and therefore can never activate a reserved contract. Add `src/observability/application-error-extension-runtime.ts` as the application-error composition adapter. It owns `ApplicationErrorActivation`, snapshots/validates `config.observability.applicationErrors`, and always delegates with one `ApplicationErrorReporterContract` activation: `"inactive"` when absent/disabled and `"active"` when enabled. It owns application-error-specific error wording and is not exported as a generic extension-kernel API.
...
- [ ] Add two-phase generic exclusive-contract preflight without materializing a deferred factory. Phase one recursively inspects concrete config entries/presets plus declared deferred metadata before any import. An activated exclusive definition accepts exactly one direct, concrete, top-level config provider with no required contracts. A disabled definition rejects any provider declaration. Duplicates, preset-hidden providers, deferred providers, primes, built-ins, raw loaders, legacy discovery, and package/project/local providers are rejected before setup. Extend the deferred API with mandatory immutable metadata:
  ...
  `createDeferredResolvedExtension()` requires `declaredContracts`; `ResolvedExtension.source` and `origin` remain its provenance. Generate optional built-in declarations into one named runtime artifact from the same workspace manifest metadata used by discovery—never maintain a second handwritten contract list or import an implementation to inspect it. Config-only orchestration inspects only concrete configured entries and supplied descriptors and therefore loads no hidden exclusive provider. The legacy full-discovery path may load project/local factories to learn metadata, but must reject every reserved exclusive contract before setup and is not used by the official reporting composition roots. Phase two verifies set equality between each loaded implementation's normalized actual `contracts` and its declared metadata before any setup; mismatch aborts startup and cannot be treated as disabled or unrelated. Package/build audits verify declaration alignment so metadata cannot become a hiding place. The accepted provider identity is bound into its lease and only that provider's extension context may publish.

- [ ] Implement and test this complete activation table in `application-error-extension-runtime.ts` before any setup side effect; the generic orchestrator receives exactly one resulting active-or-inactive definition record from this adapter:
...
- [ ] Treat exclusive-contract reservation as preflight, not convention. Both active and inactive activation records participate in the same recursive provider-declaration scan; an inactive definition rejects every matching configured provider before any import or side effect and never mints a lease. Reject reserved names in `primeContracts` and built-in descriptors before `beforeActivate` or any setup runs. An active definition mints only after direct-provider cardinality validation and binds the exact provider, runtime owner, and loader generation. Public/manual registration from `beforeActivate` fails through the reserved-name guard. Legacy discovery and a raw `ExtensionLoader` have no lease and reject every reserved declaration before setup. Add table-driven generic bypass tests plus application-error adapter tests for enabled and disabled modes.
...
- [ ] Define persistent observer ownership at loader construction and expose it through both orchestration option types:
  ...
  `new ExtensionLoader(logger, { observers })` snapshots the synchronous callbacks and retains them through teardown retries. Observers never perform asynchronous work. `onSetupFailure` runs while already published contracts are live and before rollback; `onTeardownFailure` runs immediately after each ordinary teardown-hook failure while exclusive contracts remain live. Exclusive configured providers setup before ordinary records and form the final teardown barrier. If any ordinary teardown rejects or times out, retain every exclusive provider record and lease and reject before revocation. Once ordinary teardown succeeds, revoke each exclusive contract immediately before its provider teardown. Failed exclusive teardown retains ownership and retry state; successful teardown releases the lease and runtime owner. The generic loader never calls reporter capture/flush or diagnoses through an application-error reporter; the application-error adapter/host lifecycle owns bounded flush and local reporter-failure diagnosis. Add a bounded `teardownTimeoutMs` with late-settlement quarantine to both orchestration options. Observer failure is logged locally and cannot replace or suppress the lifecycle error. Cover timeout, rollback, reverse order, retry, observer-throws, ordinary teardown failure, exclusive-provider failure, revoked-contract non-resolution, and stale-lease isolation in `loader.test.ts`; cover exactly-one-provider-flush in the application-error adapter/host tests.
...
- [ ] Run:
  ...
  ```bash
  set -euo pipefail
  deno test --frozen --allow-all src/extensions/discovery.test.ts src/extensions/orchestrate.test.ts src/extensions/deferred-extension.test.ts src/extensions/builtin-extensions.test.ts src/extensions/loader.test.ts src/observability/application-error-extension-runtime.test.ts src/config/schemas/config.schema.test.ts src/config/runtime-config.test.ts
  ...
  deno check --frozen src/extensions/orchestrate.ts src/observability/application-error-extension-runtime.ts src/config/runtime-config.ts
  generic_reporter_term_status=0
  rg -n 'ApplicationErrorReporter|applicationErrors|captureApplicationError|flushApplicationErrors|Sentry' src/extensions/contracts.ts src/extensions/contract-registry-internal.ts src/extensions/loader.ts src/extensions/orchestrate.ts || generic_reporter_term_status=$?
  test "$generic_reporter_term_status" -eq 1
  ```
...
- [ ] Build an unexported server composition primitive around `orchestrateApplicationErrorExtensions()` and the dispatcher. The adapter, not the generic orchestrator, owns the application-error activation table and error wording. Add tests for disabled reporting, enabled/missing provider, structurally invalid provider, provider setup failure, sequential generations, and validation before a supplied request-acceptance callback. No existing production server or CLI entrypoint calls this primitive until Task 9.
...
- [ ] Activate `ApplicationErrorReporterContract` only when reporting is enabled. Keep the exclusive-contract registry lease as the single reporter owner; do not add a Sentry import, expose a lease to the host, or add a second global setter.
...
- [ ] Implement an injected two-phase cleanup coordinator; Task 9 adapts the existing server disposer and keeps public `ServerHandle.stop()` composite:
  ...
  The coordinator accepts named consumer stop/quiescence barriers. A rejection is unsettled unless that consumer's explicit closed/aborted barrier confirms quiescence. For an unsettled consumer, capture each failure, bounded-flush through the still-live provider, retain bootstrap/extension ownership, and reject so a later `stop()` can retry without overlap. Only a fully settled consumer phase may call `disposeBootstrapAfterRuntime()`. The generic loader reports ordinary extension teardown failures through its lifecycle observer while the exclusive reporter lease remains live; the application-error adapter/host performs any required bounded flush. After ordinary cleanup succeeds, the loader revokes the contract and the reporter extension owns close/retry. Release process ownership only after both phases succeed.
...
- [ ] Define these provider-neutral fields on an internal lifecycle options type. Task 9 adds them to `NodeVeryfrontCloudAgentServiceOptions` and feeds them to the same `orchestrateApplicationErrorExtensions()` adapter used by server and proxy; agent code never passes exclusive definitions or leases itself:
...
- [ ] Atomically switch `src/observability/application-errors.ts` to the tested dispatcher from Task 2 and remove `setApplicationErrorReporter()`. The singleton resolves only `tryResolveExclusiveVersioned(ApplicationErrorReporterContract)` for the current lease-owned coordinator generation; it never accepts a public/manual, primed, raw-loader, built-in, legacy-discovery, stale-owner, stale-generation, or forged-definition entry. It uses a direct local diagnostic sink guarded against recursion and exports vendor-neutral `captureApplicationError`/`flushApplicationErrors` plus contract types through `veryfront/observability` (`src/observability/index.ts`).
...
- Create: `scripts/lint/audit-core-provider-mappings.ts`
- Create: `scripts/lint/audit-core-provider-mappings.test.ts`
...
- [ ] Add a semantic production-surface gate for concrete extension/provider selection that may not form an import edge. Consume the authoritative Task 1/Task 10 registries and scan every production `src/`/`cli/` AST; every registered root production manifest field; every exact emitted npm manifest, JavaScript, declaration, map, config projection, and packlist payload; every browser bundle, map, generated manifest, and embedded source module; and every binary compile-input/generated-source record before compilation. Inspect string/template values, computed keys, registries, recommendations, loader arguments, generated metadata, fallback catalogs, and provider-to-package/source mappings for materialized implementation identities such as `@veryfront/ext-*` or repository `extensions/ext-*` paths. Binary-byte string scanning remains supplemental to the authoritative compile-input graph. Every match is a policy issue with an evidence-surface ID and source/artifact location; tests/comments are outside the production inventory, but production error text and recommendations are not exceptions because they still duplicate provider/package knowledge in core. Add regressions for `src/extensions/recommendations.ts`, `builtin-extensions.ts`, `first-party-import.ts`, root dependency/optional/peer/bundled fields, emitted npm/browser/generated payloads, compile inputs, computed/template spellings, and benign generic contract names. Exact registry equality and nonzero counts are mandatory for every surface, and the gate admits no name/path allowlist.
...
- [ ] Treat every broader non-Sentry edge—including AJV, esbuild, Redis, Zod,
      JSR/std, React/ReactDOM, Markdown/Shiki/Mermaid, html2canvas, Tailwind and
      CSS optimizer bundles, WebSocket, S3, Hugging Face, Claude, executable
      templates, indirect runtime loaders, and anything else reported—as a hard
      failure. The inventory is diagnostic evidence, not a baseline; it is
      overwritten on each run and never consumed as an exception list.
...
Do not implement or schedule this section from the reporter plan. After every owner-specific extraction is complete and the canonical source, production-manifest, semantic-provider, npm, binary, and browser gates are green, rewrite these requirements as a separate exact implementation plan against the then-current artifact set. Release evidence must never bless a knowingly noncompliant core artifact.
...
- [ ] `src/extensions/contracts.ts`, `contract-registry-internal.ts`, `loader.ts`, and `orchestrate.ts` contain no `ApplicationErrorReporter`, `applicationErrors`, capture/flush, or Sentry terminology; focused tests prove the public generic orchestrator cannot activate a reserved exclusive contract.
- [ ] Every Task 10 owner has a concrete extraction plan, and the parent program cannot complete, merge, or release until those plans are executed and canonical source, production-manifest, semantic-provider, npm, browser, and binary gates all report zero issues.

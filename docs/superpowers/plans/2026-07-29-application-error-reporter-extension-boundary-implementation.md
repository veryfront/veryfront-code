...
- [ ] Resolve relative, absolute `file:`, root import-map, and scoped import-map aliases while preserving the complete ordered identity trace. Detect alias cycles, longest-prefix ambiguity, symlink/path escape, and target-specific builtins. Validate every reachable hop, not only the requested and terminal identities. Derive permitted bare self-package names and subpaths from the manifests owned by each production context; private `#` aliases and self identities are valid only when their full trace terminates inside core. Allow a resolved target only when it remains under `src/` or `cli/`, or is a builtin supported by that production target. Reject aliases into `extensions/**`, every `@veryfront/ext-*`, every `jsr:*`, `npm:*`, HTTP(S), `data:`, `blob:`, third-party bare package identity, and any file URL outside the declared core roots.

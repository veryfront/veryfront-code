...
Middleware and route handlers created at module scope, such as a `rateLimit()` store or module-level counter, behave differently by environment:
...
For anything that must be correct across requests, instances, and deploys, including rate limiting, counters, and sessions, use an external store (see the `RateLimitStore` interface and the Redis example in the rate-limit reference) rather than module-scoped memory.

...
Build a Veryfront project for production with `veryfront build`, run the
production server locally with `veryfront start`, then ship the build with
`veryfront deploy` (Veryfront Cloud) or to any host that can run your chosen
runtime.

The npm package, CLI, and import name are all `veryfront` across dev and
production.

## Prerequisites

- A Veryfront project that runs with `veryfront dev`.
- Production credentials for any LLM providers and other integrations the app
  uses.
- For Veryfront Cloud: a `VERYFRONT_API_TOKEN` and a project reference.
- For self-hosted: a deploy target (container host, Node host, or static host)
  that can serve the build output.
...
This compiles pages, bundles assets, pre-renders static routes, and outputs
everything to the `dist/` directory (configurable via `build.outDir`).
...
The production server serves pre-built assets, handles API routes, and renders
...
Deploys your project to Veryfront Cloud. Your site is available at
...
Set production environment variables on your deployment platform. At minimum,
...
You can also deploy the same build output on other infrastructure that supports
your chosen runtime or container model.
...
- `veryfront start` serves the build locally on port `3000`. Hit `/` and any API
  route with `curl` and confirm responses match the dev server.
...
- Use `veryfront open --json` when you need the deployed URL in terminal output
  or automation.

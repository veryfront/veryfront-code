...
Behavioural primitives (Popover, Dialog, DropdownMenu, Tooltip, Select) get their
mechanics from an **adapter**. This is opt-in and layered:
...
Reference adapters are **vendored** — you copy one file into your repo and own it
from then on — so the engine (e.g. `@base-ui/react`) is _your_ dependency, on
_your_ version. Core publishes no adapter package.

Copy the reference adapter (currently **Base UI**, covering `popover` + `dialog`;
more primitives and engines are in progress) from
`cli/templates/ui-adapters/base-ui.tsx` into your project's `./ui-adapters/`, then
wire it up with `UIAdapterProvider` as above. Its only imports are the engine
package and `veryfront/ui` — nothing from your app.

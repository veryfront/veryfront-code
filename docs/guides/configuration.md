...
- Only a module whose default export validates as that primitive is registered.
  A tool needs an `execute` function, an agent needs an agent definition, and so
  on. A plain helper module sitting in a discovery directory is not registered
  as a primitive.

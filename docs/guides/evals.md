...
eval definition, target, and dataset identity; a report from another eval or
dataset is rejected instead of being compared.
...
Live adapters and CLI helpers require a non-empty `VERYFRONT_TOKEN` and positive
integer timeout values. CLI case filters cannot opt into mutation: a requested
write case still requires `AG_UI_EVAL_WRITE=1`, and an experimental write case
requires both `AG_UI_EVAL_WRITE=1` and `AG_UI_EVAL_EXPERIMENTAL=1`. Unknown,
duplicate, disabled, or empty case selections fail before any case runs.
Preparation, streaming, verification, and cleanup errors are recorded as
failed cases so one case does not prevent the remaining selected cases from
producing a report. A run with no passing case exits unsuccessfully instead of
treating an all-skipped selection as evidence.

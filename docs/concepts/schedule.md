...
not execute `veryfront.config.ts`, `veryfront.config.js`, or
`veryfront.config.mjs` from the checkout.
...
the directory has no `veryfront.config.ts`, `veryfront.config.js`, or
`veryfront.config.mjs`. Remote runs do not accept `--input`; omit `--remote`
when you intentionally want the existing local execution path.

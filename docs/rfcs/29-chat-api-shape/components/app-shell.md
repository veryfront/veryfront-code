...
<AppShell storageKey="vf-shell">
  <AppShell.Sidebar side="left">
    {/* dockable column; overlay on mobile; null when closed */}
...
| `icon`                                             | `ReactNode`         | panel glyph by `side`         | Override the glyph. This is documented current `ui` behavior, not a chat-RFC change. |
| + `ButtonProps` (`variant`, `size`, native, `ref`) |                     | `icon-ghost` / `icon-default` | Full Button surface passes through                                                   |

**State attributes:** none today (`aria-expanded` carries open state). The chat RFC does not change `AppShell`.
...
This is how you build external triggers (a button _outside_ `.Header`) or react to shell state anywhere inside the provider.

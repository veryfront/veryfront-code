...
```tsx
<Message.Actions>
  {/* ONE <div> - data-floating, holds its space */}
...
What `<Message.Actions />` actually renders (today's source classes, abbreviated to layout). The bar lives inside a `Message` row whose root carries the `group/msg` hover scope - the reveal keys off _that ancestor_, not the bar itself. Nothing is absolutely positioned.
...
- **`MessageActionBar.Copied` - deleted.** Today it is a _second button_ that swaps in while `copied` is true (check icon, `"Copied!"` label) while `.Copy` null-renders. Proposed: one `.CopyAction` button whose icon/label swap is the transient `data-copied` attribute, styled with CSS.
...
The standalone `MessageActionBar` re-export does not retain a controlled mode outside `Message.Root`; it requires message context and does not spec a prop-driven fallback.
...
article:hover [data-floating] {
  opacity: 1;
}
[data-copied] .copy-icon {
  display: none;
}
[data-copied] .check-icon {
  display: inline;
}
```

...
```tsx
import { agentsToPickerOptions, ChatAgentPicker } from "veryfront/chat";
...
function ChatAgentPicker(
  { value, onValueChange, minAgents = 2, enabled = true, onCreate, onManage },
) {
...
<span class="relative inline-block">
  <!-- AgentPicker.Root anchor wrapper -->
  <button class="inline-flex h-9 items-center gap-1.5 rounded-full px-3">…</button>
  <!-- pill trigger -->
  <div role="dialog" class="z-50 min-w-[280px] …">…</div>
  <!-- portalled panel, while open -->
</span>
```

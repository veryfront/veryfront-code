...
>
> - **Exported from `veryfront/chat` today:** `Chat`, `Chat.Empty`, `Chat.ErrorBanner`, `Chat.If`, `Chat.Input`, `Chat.Message`, `Chat.MessageList`, `Chat.Root`, `Chat.Skeleton`
...
> `children` replaces the default composition. Today's append-after-composer behavior is removed so `<Chat>` follows the same render-or-compose rule as the L2 compounds.
  ...
  return (
    <ChatRoot chat={chat}>
      ...
      <ChatThemeScope className="relative flex h-full flex-col overflow-hidden">
        ...
        <Chat.ErrorBanner /> {
          /* null-renders without a session error; its default

...
<div class="mb-3">
  <!-- .Root - in-flow block -->
  <button class="flex w-full items-center gap-2 text-sm">
    <!-- .Trigger - full-width flex row, gap-2 -->
    <span>Thinking…</span>
    <!--   label: shimmer animation while streaming, plain "Thought process" when done -->
    <span class="flex size-3.5 shrink-0 items-center justify-center transition-transform">
      <!-- chevron wrapper: rotates; -rotate-90 when CLOSED -->
      <svg class="size-3.5 shrink-0" />
      <!--   chevron glyph itself never rotates - the wrapper does -->
    </span>
  </button>
  <div class="mt-2 text-sm">
    <!-- .Content - in-flow block; PRESENT ONLY WHEN OPEN -->
...
| ---------------------- | ---------------------------------------------- | ------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `text` _(required)_    | `string`                                       | -                                     | The reasoning text; `.Content` renders it as Markdown.                                                                                                       |
| `isStreaming`          | `boolean`                                      | `false`                               | Drives the shimmering "Thinking…" label, the auto open/close behavior, and `data-streaming` _(proposed)_.                                                    |
...
_Grounding:_ today's context is `{ text, isStreaming, isOpen, toggle }`. The RFC adds the prop getters, `duration`, and the explicit-input form. Provider-reported reasoning duration wins when present; otherwise the hook measures wall-clock time from first reasoning token to final reasoning token.

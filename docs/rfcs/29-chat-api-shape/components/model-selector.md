...
>
> - **Exported from `veryfront/chat` today:** `ModelSelector`, `ModelSelector.Content`, `ModelSelector.Item`, `ModelSelector.List`, `ModelSelector.Root`, `ModelSelector.Search`, `ModelSelector.Trigger`
  ...
  <div role="dialog" class="z-50 min-w-[260px] rounded-lg overflow-hidden shadow-sm">
    <div class="overflow-hidden rounded-lg">
      ...
      <div class="max-h-[320px] overflow-y-auto p-2.5">
        ...
        <div class="p-0.5">
          <!-- CommandGroup per provider (heading div = provider name);
          ...
          <div role="option" class="flex items-center gap-3 min-w-0 rounded-md px-3 py-2">
            <!-- .Item -->
            <img class="size-4.5 shrink-0 object-contain" />
            <!-- provider logo -->
            <span class="min-w-0 flex-1 truncate">GPT-4o</span>
            <!-- label grows + truncates -->
            <span class="rounded-full border px-1.5 text-[10px]">New</span>
            <!-- badge; only when option has `badge` -->
...
- Grouping: when _any_ model has `provider`, the list renders one labelled `CommandGroup` per provider (insertion order); otherwise a single unlabelled group.
- `ModelOption.description` is declared on the type and the proposed default `.Item` renders it as a secondary truncated line when present.
...
(`selected` is a deprecated alias of `selectedModel` today; dropped in the proposal.) The reader also exposes `query`, `setQuery`, and `resolvedModels`, mirroring `useAgentPicker`, so a headless menu needs no Command internals.

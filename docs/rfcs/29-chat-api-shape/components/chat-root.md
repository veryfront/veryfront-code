...
>
> - **Exported from `veryfront/chat` today:** `ChatRoot`
...
<ChatRoot chat={chat}>
  ...
  <div className="my-layout">
    ...
    <ChatInput>
      {/* reads the session from this context */}
      <ChatInput.Field />
      <ChatInput.Submit />
    </ChatInput>
  </div>
...
Streams are **provider-scoped, not mount-scoped**: keyed by conversation id, so switching threads neither aborts nor orphans an in-flight stream (see _State ownership_ in the RFC). The raw context object stays unexported.

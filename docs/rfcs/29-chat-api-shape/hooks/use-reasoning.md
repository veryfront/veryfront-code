...
>
> - **Exported from `veryfront/chat` today:** `useReasoning`
...
interface UseReasoningResult {
  ...
  getContentProps: (
    overrides?: React.HTMLAttributes<HTMLDivElement>,
...
| `duration`    | `number`  | How long the model reasoned. Provider-reported reasoning duration wins when present; otherwise the hook measures wall-clock time from first reasoning token to final reasoning token. |

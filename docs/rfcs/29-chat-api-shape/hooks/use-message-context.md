...
>
> - **Exported from `veryfront/chat` today:** `useMessageContext`, `useMessageContextOptional`
...
```ts
function useMessageContext<TMessage extends ChatMessage = ChatMessage>(): UseMessageContextResult<
  TMessage
>;
...
function useMessageContextOptional<TMessage extends ChatMessage = ChatMessage>():
  | UseMessageContextResult<TMessage>
  | null;

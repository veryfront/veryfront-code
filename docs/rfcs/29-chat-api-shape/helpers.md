...
| `getAgentPromptSuggestionItems(agent)`  | `{ label, prompt }[]` - public (#2978); selection hands the _item_ back, no `.find` massaging.                                                       |
...
- **`mergeProps` is the normative merge.** It is the exact merge the library uses internally, exported for L3 consumers composing several hooks onto one element: event handlers compose (consumer first, `preventDefault` cancels internal), `className` merges Tailwind-aware (consumer wins), `style` shallow-merges consumer-wins, refs compose. See the _Merge semantics_ section of the RFC.

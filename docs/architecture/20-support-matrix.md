...
| Bun                | Yes                       | Yes        | Yes                           | Full runtime profile; local FS/watch uses Bun's Node-compatible APIs and WebSockets use the native `Bun.serve` lifecycle.                          |
...
| `NodeWebSocketServerProvider` | `@veryfront/ext-node-websocket-ws`                        | Explicit                              | Authorized WebSocket upgrades on Node.js                                                 | Node.js sockets; scoped env reads |

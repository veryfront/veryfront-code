...
| `requestStream`                     | Request a streaming response. When the request body is replayable, typed retryable failures are retried up to two times before provider output is exposed. Each attempt gets a fresh stream header deadline. ReadableStream request bodies are not retried because fetch can consume them on the first attempt.                      | [source](https://github.com/veryfront/veryfront-code/blob/main/src/provider/runtime-loader/provider-http.ts#L848)              |

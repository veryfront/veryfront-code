...
| `readMismatchedReleaseAssetManifestSchemaVersion` | Read the schema version a rejected ready body declares, when that version is the reason it was rejected.             | [source](https://github.com/veryfront/veryfront-code/blob/main/src/release-assets/manifest-schema.ts#L434) |
| `readUntrustedOwnDataProperty`                    | Read an own data property from an untrusted value without invoking accessors.                                        | [source](https://github.com/veryfront/veryfront-code/blob/main/src/release-assets/manifest-schema.ts#L456) |

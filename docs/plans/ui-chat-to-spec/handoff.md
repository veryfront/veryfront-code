...
**The coverage suite IS the matrix and the goal** — run it to see what's left
(every red row is a todo), and the goal is simply "make the whole suite green":
...
2. **Pick ONE red row** from the coverage suite — the highest-leverage failing
   ...
   `src/react/components/{ui,chat}/coverage.test.tsx` — that row IS the spec and the
   ticket (a failing test to make green). **Never create GitHub issues** and don't
...
10. **Re-run the coverage suite** — the row you picked (and its variant rows) should
    now be green. That green IS the "ticked matrix"; the next iteration just reads
    the next red row. Repeat until the suite is fully green (= the goal).
...
| ------------------------------- | ----------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Scope in _(only for NEW scope)_ | _(no skill — edit code)_                                                                                                | Add a red row to the manifest in `src/react/components/{ui,chat}/coverage.test.tsx`. That row is the spec + the ticket. **Never open GitHub issues**; nothing leaves the repo.                                                                                                                                                                                                                                                                                                                    |

...
export function BlurGradientTop() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      // Show when header is out of view (assuming header height is around 64-80px)
      const headerHeight = 80
      const isScrolledPastHeader = scrollY > headerHeight

      // Hide when near the bottom (within 100px of bottom)
      const isNearBottom = scrollY + windowHeight >= documentHeight - 100

      setIsVisible(isScrolledPastHeader && !isNearBottom)
    }
  ...
  return (
    <div className="fixed inset-x-0 top-0 h-32 z-30 pointer-events-none">
      <div className="absolute inset-0 overflow-hidden">
        ...
        <div
          ...
          style={{
            maskImage:
              "linear-gradient(to top, rgba(0, 0, 0, 0) 87.5%, rgb(0, 0, 0) 100%)",

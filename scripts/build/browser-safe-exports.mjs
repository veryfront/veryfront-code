// Dnt must compile these browser-runtime modules even though they are
// implementation details, not supported public package subpaths. The npm
// build removes their temporary dnt export entries before publication.
export const BROWSER_SAFE_INTERNAL_ENTRY_POINTS = Object.freeze({
  "./react/public": "./src/react/public.ts",
});

/**
 * Compose dnt entry points without allowing a build-only implementation
 * detail to shadow a supported public package subpath.
 *
...
export function createDntEntryPoints(publicEntryPoints, internalEntryPoints) {
  ...
  return [
    ...Object.entries(publicEntryPoints),
    ...Object.entries(internalEntryPoints),
  ]

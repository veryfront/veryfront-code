...
Deno.test("strict audit treats contained JSON modules as terminal dependencies", async () => {
  const root = await auditFixture();
  const outside = await Deno.makeTempDir();
  const outputPath = `${root}/audit.json`;
  try {
    await Deno.writeTextFile(`${root}/src/data.json`, '{"ok":true}\n');
    await Deno.writeTextFile(`${outside}/outside.json`, '{"ok":false}\n');
    const imports = [
      'import data from "./data.json" with { type: "json" };',
      'import missing from "./missing.json" with { type: "json" };',
    ];
    if (Deno.build.os !== "windows") {
      await Deno.symlink(
        `${outside}/outside.json`,
        `${root}/src/outside-link.json`,
      );
      imports.push(
        'import outside from "./outside-link.json" with { type: "json" };',
      );
    }
    await writeSource(root, "src/index.ts", `${imports.join("\n")}\n`);

    const result = await runAudit(["--root", root, "--output", outputPath]);
    assertEquals(result.code, 2, result.stderr);
    const report = JSON.parse(await Deno.readTextFile(outputPath));
    assertEquals(
      report.issues.some((entry: { specifier?: string }) =>
        entry.specifier === "./data.json"
      ),
      false,
    );
    assertEquals(
      report.issues.some((entry: { code: string; specifier?: string }) =>
        entry.code === "unresolvable-core-source" &&
        entry.specifier === "./missing.json"
      ),
      true,
    );
    if (Deno.build.os !== "windows") {
      assertEquals(
        report.issues.some((entry: { code: string; specifier?: string }) =>
          ...
          entry.specifier === "./outside-link.json"
        ),
        true,
      );
    }

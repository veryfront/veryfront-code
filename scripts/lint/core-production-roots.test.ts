  ...
  try {
    ...
    assertEquals(
      ...
      [
        ...
        { id: "studio-bridge-browser", target: "browser" },
      ],
    );
      ...
      [
        {
          ...
          exportName: ".",
          path: "browser/studio-bridge/entry.ts",
        },
...
Deno.test("live registry claims independently match every production manifest export", async () => {
  ...
  const studio = JSON.parse(
    await Deno.readTextFile("browser/studio-bridge/deno.json"),
  );
  const expected = [
    ...
    `browser/studio-bridge/deno.json\0.\0browser/studio-bridge/${
      String(studio.exports).replace(/^\.\//, "")
    }`,
  ].sort();

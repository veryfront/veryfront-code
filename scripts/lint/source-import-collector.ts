...
function hasOnlyTypeSpecifiers(
  node: AstNode,
  kindProperty: "importKind" | "exportKind",
): boolean {
  const specifiers = Array.isArray(node.specifiers)
    ? node.specifiers.filter(isNode)
    : [];
  return specifiers.length > 0 &&
    specifiers.every((specifier) => specifier[kindProperty] === "type");
}
...
function collectJsDocTypeExpressions(value: string): JsDocTypeExpression[] {
  ...
  for (const tag of value.matchAll(typeTag)) {
    ...
    let quote: "'" | '"' | "`" | undefined;
    let escaped = false;
    for (cursor = start; cursor < value.length; cursor++) {
      ...
      if (character === "'" || character === '"' || character === "`") {
        ...
        expressions.push({ start, value: value.slice(start, cursor) });
        break;
      }
    }
  }
...
interface JsDocImportDeclaration {
  start: number;
  specifier: string;
}

function collectJsDocImportDeclarations(
  value: string,
  ...
  for (const tag of value.matchAll(importTag)) {
    ...
    const clause = value.slice(clauseStart, boundary?.index ?? value.length);
    const from = /\bfrom\s*["']([^"']+)["']/.exec(clause);
    if (from) declarations.push({ start, specifier: from[1] });
  }
  return declarations;
}
...
function collectCommentDependencies(
  ...
  for (const comment of comments) {
    ...
    if (comment.type === "CommentBlock" && value.trimStart().startsWith("*")) {
      ...
      for (const expression of collectJsDocTypeExpressions(value)) {
        for (const match of expression.value.matchAll(jsDocImport)) {
          dependencies.push({
            path: filePath,
            line: commentLine(
              comment,
              expression.start + (match.index ?? 0),
            ),
            column: 1,
            kind: "type-import",
            loader: "JSDoc import",
      ...
      for (const declaration of collectJsDocImportDeclarations(value)) {
        dependencies.push({
          path: filePath,
          line: commentLine(comment, declaration.start),
          column: 1,
          kind: "type-import",
          loader: "JSDoc @import",

...
type JsonTerminalResolution = "contained" | "missing" | "escape";

function resolveJsonTerminal(
  repositoryRoot: string,
  candidate: string,
): JsonTerminalResolution | undefined {
  const physicalCandidate = physicalRepositoryIdentity(candidate);
  if (!physicalCandidate.endsWith(".json")) return undefined;

  const root = resolve(repositoryRoot);
  const candidatePath = resolve(root, physicalCandidate);
  if (!isPathContained(root, candidatePath)) return "escape";

  let fileInfo: Deno.FileInfo;
  let realRoot: string;
  let realCandidate: string;
  try {
    fileInfo = Deno.statSync(candidatePath);
    realRoot = Deno.realPathSync(root);
    realCandidate = Deno.realPathSync(candidatePath);
  } catch (error) {
    if (error instanceof Deno.errors.NotFound) return "missing";
    throw error;
  }
  if (!fileInfo.isFile) return "missing";
  return isPathContained(realRoot, realCandidate) ? "contained" : "escape";
}
...
function classifyResolvedTrace(
  ...
  if (candidate !== undefined) {
    ...
    const jsonTerminal = resolveJsonTerminal(repositoryRoot, candidate);
    if (jsonTerminal === "contained") return { issues, sourceNodes: [] };
    if (jsonTerminal === "escape") {

...
describe("merge queue review propagation", () => {
  ...
  it("does not reuse proof older than the recorded base retarget", async () => {
    const fixture = githubFixture({
      pages: {
        reviews: [[review({
          state: "APPROVED",
    ...
    assertEquals(
      result.state,
      "failure",
      "a queue entry must not merge on an approval that predates the retarget",
    );
    assertEquals(
      fixture.published[0]?.sha,
      OTHER_HEAD,
      "the merge group commit is the commit that has to carry the closed gate",
    );
    assertEquals(
      fixture.published[0]?.state,
      "failure",
      "stale-base proof must close the merge group gate rather than reuse it",
    );
  });

...
import { createVeryfrontHandler } from "./universal-handler/index.js";
import { requestTracker } from "./universal-handler/request-tracker.js";
...
import { setServerInitialized } from "./handlers/monitoring/health.js";
import { enableSSRClientOnlyFetching, enableSSRFetchInterception, setSSRServerPort, } from "../rendering/ssr-globals.js";
export function startUniversalServer(options) {
    return withSpan("server.startUniversalServer", async () => {
        const { projectDir, port, bindAddress = "0.0.0.0", signal, debug, mode, defaultProjectSlug, defaultProjectId, defaultEnvironment, } = options;
...
if (globalThis[Symbol.for("import-meta-ponyfill-esmodule")](import.meta).main) {
    ...
    try {
        ...
        const monitoringIntervalMs = parseInt(adapter.env.get("MEMORY_MONITORING_INTERVAL_MS") ?? "30000", 10);

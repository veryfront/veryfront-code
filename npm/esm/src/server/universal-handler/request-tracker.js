...
class RequestTracker {
    ...
    complete(requestId, statusCode, timedOut = false) {
        ...
        if (tracked.projectSlug) {
            ...
            ctx = ` ${cyan}${tracked.projectSlug}${reset}${dim}:${envPart}${releasePart ? `:${releasePart}` : ""}${reset}`;
        }
        // Clean format: METHOD path status duration project:env
        console.log(`  ${dim}${tracked.method.padEnd(4)}${reset} ${displayPath} ${statusColor}${statusCode}${reset} ${dim}${String(durationMs).padStart(4)}ms${reset}${ctx}`);
    }
    getInFlightCount() {

 ...
 */
function parseSlugAndBranch(subdomain) {
...
function createParsedDomain(slug, branch, environment, isVeryfrontDomain, isDraft, allowIframeEmbed) {
    return {
        slug,
        branch,
        environment,
 ...
 */
export function parseProjectDomain(host) {
    ...
    const localPreviewMatch = domain.match(new RegExp(`^([A-Za-z0-9-]+)\\.preview\\.(${LOCAL_DEV_DOMAINS})$`));
    ...
    const localProdMatch = domain.match(new RegExp(`^([A-Za-z0-9.-]+)\\.prod\\.(${LOCAL_DEV_DOMAINS})$`));
    if (localProdMatch?.[1]) {
    ...
    const prodPreviewMatch = domain.match(new RegExp(`^([A-Za-z0-9-]+)\\.preview\\.(${PROD_DOMAINS})$`));
    ...
    const prodStagingMatch = domain.match(new RegExp(`^([A-Za-z0-9-]+)\\.staging\\.(${PROD_DOMAINS})$`));
    ...
    const prodExplicitMatch = domain.match(new RegExp(`^([A-Za-z0-9-]+)\\.production\\.(${PROD_DOMAINS})$`));
    ...
    const envRootMatch = domain.match(new RegExp(`^(preview|staging|production)\\.(${PROD_DOMAINS})$`));
    if (envRootMatch?.[1]) {
        const env = envRootMatch[1];
 ...
 */
export function isVeryfrontDomain(host) {
    const domain = stripPort(host);
    if (domain === "veryfront.me" || domain === "veryfront.dev" || domain === "lvh.me")
        return true;
 ...
 */
export function getEffectiveProjectSlug(host, configuredSlug) {
    const { slug } = parseProjectDomain(host);
    if (slug)
        return { slug, fromHost: true };

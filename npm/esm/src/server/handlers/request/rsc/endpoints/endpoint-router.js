 ...
 */
export async function handleRSCEndpoint({ req, pathname, projectDir, adapter, config }) {
    ...
    try {
        ...
        switch (sub) {
            ...
            case "action":
                ...
                try {
                    ...
                    return jsonErrorResponse(HttpStatus.INTERNAL_SERVER_ERROR, e instanceof Error ? e.message : String(e));
                }
            case "manifest":
        ...
        if (e instanceof Error && e.message === "Component not found") {
            serverLogger.debug("[RSCEndpointRouter] component not found, deferring to legacy handler", { error: e.message });
            return null;
        }
        try {
            metrics.recordRSC("error");
...
async function handleModuleEndpoint({ searchParams, projectDir, adapter, }) {
...
async function handlePayloadEndpoint({ handler, searchParams, }) {
    let modules = [];
    try {
        ...
        if (manifestResponse.ok) {
            ...
            if (componentPaths && typeof componentPaths === "object") {
                modules = Object.values(componentPaths).filter((value) => typeof value === "string" && value.length > 0);
            }
        }
    }
    catch (error) {
...
function handleStreamEndpoint(searchParams) {

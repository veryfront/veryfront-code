...
export class LibModulesHandler extends BaseHandler {
    metadata = {
        ...
        priority: PRIORITY_MEDIUM_LIB_MODULES,
        patterns: [
    ...
    async handle(req, ctx) {
        ...
        if (moduleResolution !== "self-hosted") {
            this.logDebug("LibModulesHandler: self-hosted mode not enabled, skipping", { moduleResolution }, ctx);
            return this.continue();
        }
        const modulePath = pathname.slice("/_veryfront/lib/".length);
        if (!ALLOWED_MODULES.has(modulePath)) {
            this.logDebug(`LibModulesHandler: module not allowed: ${modulePath}`, { allowed: Array.from(ALLOWED_MODULES) }, ctx);
        ...
        try {
            ...
            this.logDebug(`LibModulesHandler: served ${modulePath}`, { size: content.length, filePath }, ctx);
            return this.respond(builder
                .withCORS(req, ctx.securityConfig?.cors)
                .withSecurity(ctx.securityConfig ?? undefined)
            ...
            this.logDebug(`LibModulesHandler: failed to serve ${modulePath}: ${this.getErrorMessage(error)}`, { filePath }, ctx);
            return this.respondNotFound(req, ctx, method);
        }
    }
    respondNotFound(req, ctx, method) {
        const builder = this.createResponseBuilder(ctx);
        return this.respond(builder
            .withCORS(req, ctx.securityConfig?.cors)
            .withCache("no-cache")

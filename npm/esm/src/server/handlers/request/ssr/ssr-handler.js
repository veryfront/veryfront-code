...
import { getRendererForProject, shouldRejectDueToMemory, } from "../../../shared/renderer-factory.js";
import { getHeapStats } from "../../../../utils/memory/index.js";
import { serverLogger as _logger } from "../../../../utils/index.js";
import { endRequest, startRequest, timeAsync } from "../../../../utils/index.js";
import { computeSSRETag } from "./etag-handler.js";
...
import { HTTP_INTERNAL_SERVER_ERROR, HTTP_NOT_FOUND, HTTP_OK, PRIORITY_LOW, } from "../../../../utils/constants/index.js";
 ...
 */
export function isProductionMode(ctx, _url) {
    ...
    handle(req, ctx) {
        ...
        try {
            ...
            if (ctx.projectSlug && hasMultiProjectSupport) {
                ...
                return fsAdapter.runWithContext(ctx.projectSlug, effectiveToken, () => {
            ...
            this.logDebug("Unexpected error in context setup", { error: this.getErrorMessage(error) }, ctx);
            return Promise.resolve(this.continue());
        }
    }
    handleWithContext(req, ctx, slug, requestId, url) {
        return withSpan("ssr.handleWithContext", async () => {
            ...
            try {
                ...
                this.logDebug("renderer obtained", { isDev: ctx.requestContext?.isLocalDev ?? false, projectSlug: ctx.projectSlug }, ctx);
                const nonce = generateNonce();
                ...
                const { scheme: colorScheme, fromParam: colorSchemeFromParam } = getColorSchemeFromRequest(req, url);
                ...
                if (isTrueStreaming) {
                    ...
                    if (isHeadRequest) {
                        ...
                        return this.respond(new dntShim.Response(null, { status: response.status, headers: response.headers }));
                    }
                ...
                if (error instanceof VeryfrontError && error.code === ErrorCode.FILE_NOT_FOUND) {
                    this.logDebug("SSR renderPage not found", { slug, error: this.getErrorMessage(error) }, ctx);
                ...
                this.logDebug("SSR renderPage failed with error", { slug, error: this.getErrorMessage(error) }, ctx);

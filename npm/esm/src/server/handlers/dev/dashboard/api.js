...
import { checkMemoryPressure, getCacheStats, getHeapStats, } from "../../../../utils/memory/profiler.js";
...
function handleListAgents() {
    ...
    const list = Array.from(agentRegistry.getAll().entries()).map(([id, agent]) => {
        ...
        return {
            id,
            description: cfg.description || `Model: ${agent.config.model}`,
...
async function handleExecuteTool(req) {
    try {
...
async function handleReadResource(req) {
    try {
        ...
        if (!resource)
            return errorResponse(`Resource not found for URI: ${uri}`, 404);
...
async function handleRenderPrompt(req) {
    try {
...
let devWorkflowClient = null;
function getDevWorkflowClient() {
...
async function handleStartWorkflow(req) {
    try {
        ...
        const result = await Promise.race([
            handle.result(),
            new Promise((_, reject) => dntShim.setTimeout(() => reject(new Error("Workflow execution timed out (30s)")), timeoutMs)),
        ]);
...
function handleListHandlers(ctx) {
...
async function handleListFiles(req, ctx) {
    const { adapter, projectDir } = ctx;
    if (!adapter?.fs)
        return errorResponse("No file adapter available", 500);
    if (!projectDir)
...
async function handleReadFileContent(req, ctx) {
    const { adapter, projectDir } = ctx;
    if (!adapter?.fs)
        return errorResponse("No file adapter available", 500);
    if (!projectDir)
...
function handleGetInfrastructure() {
...
function handleGetErrors() {
    ...
    const categories = errors.reduce((acc, err) => {

...
function transformModule(filePath, source, relativePath) {
    return withSpan("server.dev.dashboardUI.transformModule", async () => {
        ...
        return result.code.replace(/from\s+["'](\.\.?\/[^"']+)\.tsx?["']/g, (_match, importPath) => `from "/_dev/ui/${resolveRelativeImport(currentDir, importPath)}.js"`);
    }, { "module.filePath": filePath, "module.relativePath": relativePath });
}
async function readUiModule(uiDir, relativePath) {
...
export function handleDashboardUI(req) {

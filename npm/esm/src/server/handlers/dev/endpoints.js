 ...
 */
function getUpdateJSFunction(logPrefix) {
    return `
...
export class DevEndpointsHandler extends BaseHandler {
    ...
    handle(req, ctx) {
        ...
        const response = this.createResponseBuilder(ctx).withCache("no-cache").javascript(script, HTTP_OK);
        return Promise.resolve(this.respond(response));
    }
    getScriptForPath(pathname, url) {
        switch (pathname) {

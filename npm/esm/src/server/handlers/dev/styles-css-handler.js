...
`;
                return this.respond(responseBuilder.withContentType("text/css; charset=utf-8", errorCSS, HTTP_OK));
            }
...
@custom-variant dark (&:is(.dark, [data-theme="dark"]) *, &:is(.dark, [data-theme="dark"]));`;
    ...
    async extractProjectCandidates(ctx) {
        ...
        if (typeof wrappedFs.getUnderlyingAdapter !== "function") {
            ...
            logger.debug("[StylesCSSHandler] No FS adapter wrapper, falling back to local file scanning");
        ...
        if (typeof fsAdapter.getAllSourceFiles !== "function") {
            // Fallback: scan local files directly for local development
            logger.debug("[StylesCSSHandler] FS adapter missing getAllSourceFiles, falling back to local file scanning");
            return await this.scanLocalFiles(ctx.projectDir, ctx);
        }
     ...
     */
    async scanLocalFiles(projectDir, ctx) {
        const candidates = new Set();

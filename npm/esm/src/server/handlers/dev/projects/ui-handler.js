...
function resolveRelativeImport(currentDir, importPath) {
...
function transformModule(filePath, source, relativePath) {
    return withSpan("server.dev.projectsUI.transformModule", async () => {
        ...
        return result.code.replace(/from\s+["'](\.\.?\/[^"']+)\.tsx?["']/g, (_match, importPath) => `from "/_projects/ui/${resolveRelativeImport(currentDir, importPath)}.js"`);
    }, { "module.filePath": filePath, "module.relativePath": relativePath });
}
async function readUiSource(uiDir, relativePath) {
...
export function handleProjectsUI(req) {
    ...
    return withSpan("server.dev.projectsUI.handle", async () => {
        ...
        if (!module) {
            return new dntShim.Response(`Module not found: ${relativePath}`, {
                status: 404,
        ...
        if (cached && now - cached.timestamp < CACHE_TTL) {
            return new dntShim.Response(cached.code, { headers: JS_HEADERS });
        }
        try {
            const code = await transformModule(filePath, source, relativePath);
            moduleCache.set(filePath, { code, timestamp: now });
            return new dntShim.Response(code, { headers: JS_HEADERS });
        }
        catch (error) {
            logger.error("[Projects] Transform error", { filePath, error });
            return new dntShim.Response(`// Transform error: ${error instanceof Error ? error.message : String(error)}`, { status: 500, headers: JS_HEADERS });
        }

...
import { HMR_MAX_MESSAGE_SIZE_BYTES, HMR_MAX_MESSAGES_PER_MINUTE, serverLogger as logger, } from "../../../utils/index.js";
...
const PRIORITY_HMR = 25;
const PING_INTERVAL_MS = 45000;
export class HMRHandler extends BaseHandler {
    static clientsMap = new Map();
    static clients = new Set(); // Keep for backward compatibility with setupWebSocketHandlers
    ...
    static sendPingToAllClients() {
        if (HMRHandler.clients.size === 0)
            return;
    ...
    static requiresFullReload(path) {
        const ext = path.split(".").pop()?.toLowerCase();
        return ext === "mdx" || ext === "md" || path.includes("veryfront.config");
    }
    static broadcastUpdate(changedPaths) {
    ...
    handle(req, ctx) {
        ...
        if (req.headers.get("upgrade")?.toLowerCase() !== "websocket") {
            ...
            return Promise.resolve(this.respond(new dntShim.Response(JSON.stringify({
                status: "ok",
        ...
        if (!ctx.adapter?.server) {
            return Promise.resolve(this.respond(new dntShim.Response("WebSocket not supported", { status: 501 })));
        }
        try {
            ...
            return Promise.resolve(this.respond(new dntShim.Response("WebSocket upgrade failed", { status: 500 })));
        }
    }

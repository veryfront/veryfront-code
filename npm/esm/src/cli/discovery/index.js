...
import { registerPrompt, registerResource, registerTool } from "../../mcp/index.js";
import { registerAgent } from "../../agent/index.js";
import { registerWorkflow } from "../../workflow/index.js";
...
async function importModule(file, context) {
    if (transpileCache.has(file))
        return transpileCache.get(file);
    ...
    if (result.errors?.length) {
        throw new Error(`Failed to transpile ${filePath}: ${result.errors[0]?.text || "unknown error"}`);
    }
...
function rewriteForDeno(code, fileDir) {
    ...
    return transformed.replace(/from\s+["'](\.\.\/[^"']+)["']/g, (_match, relativePath) => `from "file://${pathHelper.resolve(fileDir, relativePath)}"`);
}
async function rewriteDiscoveryImports(code, projectDir, fs, fileDir) {
    let transformed = code;
    try {
...
const resourceHandler = {
    typeName: "resource",
    validate: (item) => item !== null && typeof item === "object" && typeof item.load === "function",
    getId: (_item, file) => filenameToId(file),
...
const promptHandler = {
    typeName: "prompt",
    validate: (item) => item !== null && typeof item === "object" && typeof item.getContent === "function",
    getId: (_item, file) => filenameToId(file),
    register: (id, prompt) => {
...
async function getNodeDeps(context) {
    if (context.nodeDeps)
...
function filePathToPattern(filePath, baseDir) {
    const cleanPath = filePath.replace("file://", "");
    let pattern = cleanPath.replace(baseDir, "").replace(/\.(ts|tsx|js|jsx)$/, "");
    pattern = pattern.replace(/\[(\w+)\]/g, ":$1").replace(/^\/+/, "");
    return "/" + pattern;
}
const discoveredAgentPaths = new Map();
export async function generateAgentIndex(baseDir) {
    ...
    try {
        ...
        for (const [id, filePath] of discoveredAgentPaths) {
            const cleanPath = filePath.replace("file://", "");
            const relativePath = pathModule.relative(generatedDir, cleanPath).replace(/\.(ts|tsx|js|jsx)$/, "");
...
export function clearTrackedAgents() {

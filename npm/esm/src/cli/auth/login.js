...
import { deleteToken, getTokenLocation, hasToken, readToken, saveToken } from "./token-store.js";
...
import { DEFAULT_LOGIN_TIMEOUT_MS, getApiUrl } from "./constants.js";
const AUTH_OPTIONS = [
...
export async function validateToken(token) {
    try {
        const response = await dntShim.fetch(`${getApiUrl()}/me`, {
...
export async function ensureAuthenticated(env = getRuntimeEnv()) {
    if (env.apiToken) {
...
export async function whoami(env = getRuntimeEnv()) {
    if (env.apiToken) {
        const userInfo = await validateToken(env.apiToken);
        if (userInfo) {
            console.log();

...
import { issuesMcpTools } from "../../issues/mcp.js";
const CACHE_DIRS = {
...
export const vfGetLogs = {
    ...
    execute: (input) => {
        return Promise.resolve(getLogBuffer().query({
            level: input.level,
            source: input.source,
...
export function createVfGetStatus(env = getRuntimeEnv()) {
    return {
        name: "vf_get_status",
...
export const vfClearErrors = {
    ...
    execute: (input) => {
        ...
        if (input.type) {
            return Promise.resolve({ cleared: collector.clearType(input.type) });
        }

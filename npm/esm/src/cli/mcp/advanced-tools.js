...
async function scanDirectory(dir, baseRoute, routes, fs) {
    try {
...
const listRoutesInput = z.object({
    type: z.enum(["all", "pages", "api", "layouts"]).optional().default("all").describe("Filter routes by type"),
...
async function detectDirectories(projectDir) {
    const directories = {};
...
async function detectIntegrations(projectDir, fs) {
...
async function detectFeatures(projectDir, hasAI) {
    const features = [];
    if (hasAI)
        features.push("ai");
...
const scaffoldInput = z.object({
    type: z.enum(["page", "api", "layout", "component", "tool", "agent", "prompt"]).describe("Type of entity to scaffold"),
    name: z.string().describe("Name/path of the entity (e.g., 'users', 'api/users', 'dashboard/settings')"),
    methods: z.array(z.enum(["GET", "POST", "PUT", "DELETE", "PATCH"])).optional().describe("HTTP methods for API routes (defaults to GET)"),
    projectPath: z.string().optional().describe("Project directory (defaults to current working directory)"),
});
// ============================================================================
// Scaffold Templates
...
const hotReloadInput = z.object({
    file: z.string().optional().describe("Specific file to trigger reload for (optional - reloads all if not specified)"),
});
...
const previewRouteInput = z.object({
    ...
    port: z.number().optional().default(8080).describe("Dev server port (defaults to 8080)"),
    format: z.enum(["html", "json", "status"]).optional().default("status").describe("Output format: html (full page), json (API response), status (just HTTP status)"),
});
...
function toRelativePath(absolutePath, projectDir) {
...
const listLocalProjectsInput = z.object({
    directory: z.string().optional().describe("Directory to scan for projects (defaults to current directory and common locations)"),
    depth: z.number().optional().default(2).describe("How deep to scan (1 = immediate children, 2 = grandchildren)"),
});
async function detectVeryfrontProject(projectPath) {
    ...
    if (!configExists && !await fileExists(join(projectPath, ".veryfrontrc")))
        return null;
...
async function scanForProjects(baseDir, depth, projects) {
    const fs = getFs();
...
const waitForReadyInput = z.object({
    ...
    timeout: z.number().optional().default(30000).describe("Maximum time to wait in milliseconds (defaults to 30000)"),
    interval: z.number().optional().default(500).describe("Polling interval in milliseconds (defaults to 500)"),
});

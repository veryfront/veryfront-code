...
async function executeCommand(cmd, args, cwd, silent) {
 ...
 * 4. Default to npm
 */
export async function detectPackageManager(projectDir, preference) {
    if (preference)
        return preference;
    ...
    if (detected) {
        ...
        if (file)
            logger.debug(`Detected ${detected} from ${file}`);
        return detected;
    }
    ...
    if (detectedFromParent) {
        ...
        if (file)
            logger.debug(`Detected ${detectedFromParent} from parent directory ${file}`);
 ...
 */
export async function installDependencies(projectDir, options = {}) {

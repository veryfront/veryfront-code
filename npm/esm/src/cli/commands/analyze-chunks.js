import { analyzeProjectChunks, generateChunkManifest, } from "../../rendering/chunk-optimizer.js";
import { cliLogger } from "../../utils/index.js";
import { createFileSystem } from "../../platform/compat/fs.js";
import { exit } from "../../platform/compat/process.js";
export async function analyzeChunksCommand(options) {
    ...
    try {
        ...
        if (analysis.suggestedChunks.length > 0) {
            ...
            for (const chunk of analysis.suggestedChunks) {
                if (chunk.deps.length <= 5)
                    continue;
                const pageCount = chunk.pages.length > 0 ? `${chunk.pages.length} pages` : "unknown pages";
                cliLogger.info(`  ${chunk.name} (${chunk.deps.length} deps, ${pageCount}, ~${chunk.benefit} bytes saved)`);
            }
            cliLogger.info("");
        }
        if (output) {
        ...
        const totalSharedUsage = Array.from(analysis.sharedDeps.values()).reduce((sum, count) => sum + count, 0);
        const avgUsage = analysis.sharedDeps.size > 0 ? totalSharedUsage / analysis.sharedDeps.size : 0;
        if (avgUsage > 3) {
            cliLogger.info(`Average shared dependency usage (${avgUsage.toFixed(2)}) suggests splitting common UI modules.`);
        }
        const hasHeavyDeps = Array.from(analysis.sharedDeps.keys()).some((dep) => dep.includes("@mui/") ||

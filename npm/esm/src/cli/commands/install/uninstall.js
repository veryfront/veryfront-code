...
import { exists, readDir, remove } from "../../../platform/compat/fs.js";
...
import { AI_TOOLS, getToolById, isValidToolId } from "./registry.js";
import { AIToolIdSchema, UninstallOptionsSchema, } from "./types.js";
const ESC = "\x1b";
...
async function multiSelect(options) {
    ...
    function draw() {
        if (lines > 0)
            clearLines(lines);
        console.log();
        console.log("  " + bold(brand("Remove AI Tool Files")) + " " +
...
export async function findInstalledTools(options, env = getRuntimeEnv()) {
...
async function isDirEmpty(path) {
    for await (const _entry of readDir(path))
        return false;
    return true;
}
export async function uninstallTargets(targets, options, env = getRuntimeEnv()) {

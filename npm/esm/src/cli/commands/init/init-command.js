...
import { cyan } from "../../../platform/compat/console/index.js";
 ...
 */
function generateIntegrationsStatusRoute(integrations) {
...
function validateOrThrow(kind, values, validate) {
    if (values.length === 0)
        return;
...
function dedupeEnvVars(envVars) {
    const seen = new Set();
...
export async function initCommand(options) {
    ...
    const integrationsStr = integrations.length > 0
        ...
        : "";
    log(`Creating new Veryfront project${name ? ` in ${name}` : ""} with template: ${template}${featuresStr}${integrationsStr}`);
    ...
    if (integrations.length > 0) {
        ...
        logger.debug(`Loaded ${loadedIntegrations.length} integrations with ${integrationFiles.length} files`);

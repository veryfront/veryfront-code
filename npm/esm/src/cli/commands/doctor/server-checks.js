...
async function safeCancelBody(response) {
    try {
...
export async function checkRSCFlag() {
    try {
    ...
    try {
        ...
        if (manifest?.ok) {
            ...
            try {
                ...
                if (json && typeof json === "object" && "hash" in json &&
                    typeof json.hash === "string") {
                    msg = `200 (hash:${json.hash}, ${dm}ms)`;
                }
            }
            catch (error) {
...
export async function checkRSCCounters() {
    ...
    try {
        ...
        const msg = `manifest:${counters.rscManifest ?? 0} page:${counters.rscPage ?? 0} stream:${counters.rscStream ?? 0} action:${counters.rscAction ?? 0} errors:${counters.rscErrors ?? 0}`;

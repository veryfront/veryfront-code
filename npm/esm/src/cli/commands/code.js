...
import { buildAgentCommand, createAgentRegistry, detectInstalledAgents, getCLIAgents, getIDEAgents, } from "../app/core/agents.js";
...
// List Command
// ============================================================================
function printAgentList(agents, installed, label) {
...
// Set Default Command
// ============================================================================
async function setDefaultAgent(agentId, model) {
    const registry = createAgentRegistry();
...
// Launch Agent
// ============================================================================
async function launchAgent(agentId, projectDir, model) {
    const registry = createAgentRegistry();
    // Find agent by id or name
    const agent = registry.agents.find((a) => a.id === agentId || a.name.toLowerCase() === agentId.toLowerCase());
    if (!agent) {
        cliLogger.error(`Unknown agent: ${agentId}`);
        cliLogger.info(`Run 'veryfront code --list' to see available agents`);
        return;
    ...
    const agentWithCommand = { ...agent, command: [command, ...args].join(" ") };
...
// Main Command
// ============================================================================
export async function codeCommand(args) {

...
import { brand, createTui, dim, error, handleInput, interceptConsole, success, } from "../ui/index.js";
// ============================================================================
// Types
// ============================================================================
...
function parseIntegrations(integrationsStr) {
    if (!integrationsStr)
        return [];
...
// Main Command
// ============================================================================
export async function newCommand(name, options = {}, env = getRuntimeEnv()) {
    let { template, integrations: integrationsStr, port = 3000, skipDeploy = false, open = true, force = false, } = options;
    ...
    if (skipDeploy) {
        ...
        const result = await scaffoldProjectFast(projectDir, template, slug, integrations);
    ...
    try {
        await fs.mkdir(projectDir, { recursive: true });
        await scaffoldProjectFast(projectDir, template, slug, integrations);

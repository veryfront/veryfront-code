 ...
 */
export const AVAILABLE_USECASES = [
    "productivity",
    "developer",
    "support",
    "social",
 ...
 */
export async function loadIntegrationConfig(integrationName) {
    const fs = createFileSystem();
    const configPath = pathHelper.join(getIntegrationDirectory(integrationName), "connector.json");
    try {
 ...
 */
export async function loadIntegration(integrationName) {
 ...
 */
export async function integrationExists(integrationName) {
    const fs = createFileSystem();
 ...
 */
export function getUseCaseConfig(useCaseName) {
    return USE_CASE_CONFIGS[useCaseName];
}
/**
 * Get all available prompts for a set of integrations
 */
export async function getAvailablePrompts(integrationNames) {
    const result = [];
    for (const name of integrationNames) {
 ...
 */
export function loadIntegrationBaseConfig() {
    return loadIntegrationConfig("_base");
}
/**
 * Generate base files needed for any integration setup
 * These are shared across all integrations
 */
export function getIntegrationBaseFiles() {
    return [
        {

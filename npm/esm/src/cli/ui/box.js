 ...
 */
import { lines, maxLineWidth, pad, repeat, visibleLength } from "./layout.js";
...
export function box(content, options = {}) {
    const { style = "rounded", padding = 1, paddingX = padding, paddingY = Math.max(0, padding - 1), // Vertical padding is usually less
    title, titleAlign = "left", borderColor = "", titleColor = "", width, } = options;
    const border = BORDER_STYLES[style];
    const contentLines = lines(content);
    const contentWidth = maxLineWidth(contentLines);
    const innerWidth = Math.max(contentWidth + paddingX * 2, title ? visibleLength(title) + 4 : 0);
    const boxWidth = width ? Math.max(width, innerWidth + 2) : innerWidth + 2;
    const actualInnerWidth = boxWidth - 2;
    const bc = (text) => (borderColor ? `${borderColor}${text}${RESET}` : text);
    const tc = (text) => (titleColor ? `${titleColor}${text}${RESET}` : text);
    const result = [];
    if (title) {
    ...
    for (let i = 0; i < paddingY; i++)
        result.push(emptyLine);
    result.push(bc(border.bottomLeft) +
        bc(repeat(border.horizontal, actualInnerWidth)) +
 ...
 */
export function joinHorizontal(align, gap, ...items) {
    if (items.length === 0)
        return "";
    if (items.length === 1)
        return items[0] ?? "";
 ...
 */
export function joinVertical(align, gap, ...items) {
    if (items.length === 0)
        return "";
    if (items.length === 1)
        return items[0] ?? "";
 ...
 */
export function divider(width, style = "rounded") {
    return repeat(BORDER_STYLES[style].horizontal, width);
}
/**
 * Create a divider with centered text
 */
export function dividerWithText(text, width, style = "rounded") {
    const border = BORDER_STYLES[style];

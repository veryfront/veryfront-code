...
class AdapterRegistry {
    ...
    doInitialize() {
        ...
        return withSpan("platform.registry.doInitialize", async () => {
            if (runtimeId === "unknown") {
                throw new Error("Unsupported runtime detected. Supported runtimes: deno, node, bun. " +
                    "For Cloudflare Workers, call runtime.set(createCloudflareAdapter(env)).");
            }
            if (runtimeId === "cloudflare") {
                throw new Error("Cloudflare Workers detected but requires manual initialization. " +
                    "Use: await runtime.set(createCloudflareAdapter(env))");
            }
            const loader = this.loaders.get(runtimeId);
            if (!loader) {
                throw new Error(`No loader registered for runtime: ${runtimeId}. ` +
    ...
    set(adapter) {
        return withSpan("platform.registry.set", async () => {
            if (!adapter.id || !adapter.name || !adapter.fs || !adapter.env || !adapter.server) {
                throw new Error("Invalid adapter: must implement RuntimeAdapter interface with id, name, fs, env, and server properties");
            }
    ...
    getSync() {
        if (!this.instance || !this.initialized) {
            throw new Error("RuntimeAdapter not initialized. Call `await runtime.get()` first, " +
    ...
    registerLoader(id, loader, options) {
        if (this.loaders.has(id) && !options?.overwrite) {
            throw new Error(`Loader for runtime '${id}' already registered. Use { overwrite: true } to replace.`);
        }
...
export async function resetLocalAdapter() {
    if (!localRegistry)
        return;

...
import { getCachedWithBatching, setInRequestCache, } from "../../../../cache/request-cache-batcher.js";
 ...
 */
export async function initializeFileCacheBackend() {
 ...
 */
export function isFileCacheDistributedEnabled() {
...
export class FileCache {
    ...
    has(key) {
        if (!this.options.enabled)
            return false;
        if (this.isDistributed())
            return false; // Use hasAsync for distributed mode
        const entry = this.fallbackCache.get(key);
        if (!entry)
            return false;
        if (Date.now() - entry.timestamp > this.options.ttl) {
    ...
    deleteByPrefixAndSuffix(prefix, suffix) {
        let count = 0;
        const suffixWithColon = `:${suffix}`;
        for (const key of this.fallbackCache.keys()) {
            if (!key.startsWith(prefix) || !key.endsWith(suffixWithColon))
                continue;
            const entry = this.fallbackCache.get(key);
            if (entry)
                this.fallbackMemoryUsed -= entry.size;
    ...
    evictFallbackIfNeeded(newSize) {
        ...
        // Evict by memory
        while (this.fallbackMemoryUsed + newSize > this.options.maxMemory && this.fallbackCache.size > 0) {
            evictOldest();
        }
    }
}

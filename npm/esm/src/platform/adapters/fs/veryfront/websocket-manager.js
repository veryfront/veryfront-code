...
export class WebSocketManager {
    ...
    handlePokeMessage(event) {
        try {
            ...
            if (isProductionMode && !isProductionPoke) {
                logger.debug("[WebSocketManager] POKE ACCEPTED - branch-scoped poke in production mode", {
                    pokeBranchId: normalizedBranchId,
                    pokeBranchName: normalizedBranchName,
            ...
            if (!isProductionMode) {
                if (normalizedBranchName) {
                    if (normalizedBranchName !== currentBranch) {
                        logger.debug("[WebSocketManager] POKE SKIPPED - different branch name in preview mode", {
                            pokeBranchName: normalizedBranchName,
                            currentBranch,
                        });
                        return;
                    ...
                    if (currentBranch === null) {
                        logger.debug("[WebSocketManager] POKE SKIPPED - branchId-only poke for main preview", { pokeBranchId: normalizedBranchId });
                        return;
                    }
                    logger.debug("[WebSocketManager] POKE ACCEPTED - branchId-only fallback in preview mode", { pokeBranchId: normalizedBranchId, currentBranch });
                }
                    ...
                    logger.debug("[WebSocketManager] POKE SKIPPED - unscoped poke for named branch preview", { currentBranch });
                    return;
                }
            }
    ...
    // ===========================================================================
    clearPersistentCacheForPublish(releaseId, environmentName) {
        ...
        logger.info("[WebSocketManager] PUBLISH POKE - clearing persistent cache", {
            projectSlug: this.deps.projectSlug,
            releaseId,
            environmentName,
        ...
        void (async () => {
            try {
                const results = await Promise.all(Array.from(deletionPrefixes).map((prefix) => this.deps.cache.deleteByPrefixAsync(prefix)));
                const totalDeleted = results.reduce((sum, count) => sum + count, 0);
                logger.info("[WebSocketManager] PUBLISH POKE - persistent cache cleared", {
                    projectSlug: this.deps.projectSlug,
                    releaseId,
                    environmentName,
                    totalDeleted,
                ...
                logger.warn("[WebSocketManager] PUBLISH POKE - failed to clear persistent cache", {
                    projectSlug: this.deps.projectSlug,
                    releaseId,
                    environmentName,
                ...
                logger.debug("[WebSocketManager] PUBLISH POKE - cache invalidation complete", {
                    projectSlug: this.deps.projectSlug,
                    pendingInvalidations: getPendingInvalidationsCount(),
                });
            }
    ...
    async performSelectiveInvalidation() {
        ...
        for (const path of changedPaths) {
            ...
            for (const fileType of fileTypes) {
                for (const sourceType of sourceTypes) {
                    deletionPromises.push(this.deps.cache.deleteByPrefixAndSuffixAsync(fileType + sourceType, path));
                }
            }
        }
        for (const parentDir of parentDirs) {
            for (const sourceType of sourceTypes) {
                deletionPromises.push(this.deps.cache.deleteByPrefixAndSuffixAsync("dir:" + sourceType, parentDir));
            }
        }
        ...
        logger.info("[WebSocketManager] TRIGGERING HMR RELOAD via invalidationCallbacks.triggerReload", {
            changedPaths,
            projectSlug: this.deps.projectSlug,

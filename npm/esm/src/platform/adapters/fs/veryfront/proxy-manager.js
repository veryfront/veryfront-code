...
import { clearSSRModuleCache, clearSSRModuleCacheForProject, } from "../../../../modules/react-loader/ssr-module-loader/cache/index.js";
...
export class ProxyFSAdapterManager {
    ...
    async getAdapter(projectSlug, token, projectId, productionMode, releaseId, environmentName, branch) {
        ...
        if (!validationResult.success) {
            ...
            throw new Error(`[ProxyFSAdapterManager] Invalid getAdapter parameters: ${validationResult.error.message}`);
        }
        ...
        return this.createAdapter(cacheKey, projectSlug, token, projectId, effectiveProductionMode, effectiveReleaseId, effectiveEnvironmentName, effectiveBranch);
    }
    assertContextMatches(cacheKey, currentContext, expected) {
        if (!currentContext) {
            logger.error("[ProxyFSAdapterManager] Null context detected", { cacheKey });
            throw new Error(`[ProxyFSAdapterManager] FATAL: Cached adapter has null context. ` +
        ...
        throw new Error(`[ProxyFSAdapterManager] FATAL: Context mismatch for cached adapter. ` +
    ...
    getContextMismatchReason(currentContext, expected) {
        if (expected.productionMode) {
            ...
            if (currentContext.sourceType === "release" && currentContext.releaseId !== expected.releaseId) {
                return `Expected releaseId "${expected.releaseId}", got "${currentContext.releaseId}"`;
            }
            if (currentContext.sourceType === "environment" &&
    ...
    createAdapter(cacheKey, projectSlug, token, projectId, productionMode, releaseId, environmentName, branch) {
    ...
    hasAdapter(projectSlug, productionMode, releaseId, branch) {
        const cacheKey = buildProxyManagerCacheKey(projectSlug, productionMode ?? false, releaseId ?? null, branch ?? null);

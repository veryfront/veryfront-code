...
import { VeryfrontAPIOperations, } from "./operations.js";
...
export class VeryfrontAPIClient {
    ...
    constructor(config) {
        ...
        this.operations = new VeryfrontAPIOperations(this.config.apiBaseUrl, tokenProvider, retryConfig);
    }
    // =============================================================================
    // Configuration
    // =============================================================================
    ...
    async searchFilesWithContent(pattern) {
        ...
        const fetched = await Promise.all(filesNeedingContent.map(async (path) => {
            try {
    ...
    listFilesByContext(projectRef, context, options) {
        switch (context.type) {
            case "branch":
                return this.operations.listBranchFiles(projectRef, context.name, options);
            case "environment":
     ...
     */
    async resolveFileWithExtension(basePath, extensionPriority = [".tsx", ".ts", ".jsx", ".js", ".mdx", ".md"]) {
    ...
    listPublishedFiles(_projectId, releaseId, environmentName) {
        const projectRef = this.getProjectSlug();
        ...
        throw new VeryfrontAPIError("Cannot list published files without releaseId or environmentName", 400);
    }
    async getPublishedFileContent(path, releaseId, environmentName) {
        const projectRef = this.getProjectSlug();
        // Use release endpoint if releaseId provided, otherwise use environment when specified
        if (releaseId) {
        ...
        throw new VeryfrontAPIError("Cannot fetch published file without releaseId or environmentName", 400);
    }
}

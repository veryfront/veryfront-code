    ...
    try {
        ...
        for await (const entry of dntShim.Deno.readDir(nodeModulesPath)) {
            if (entry.name === binaryName || entry.name.startsWith("@esbuild")) {
                ...
                try {
                    const stat = await dntShim.Deno.stat(binPath);
                    if (stat.isFile)
                        return binPath;
                }
                catch {
...
async function extractEsbuildBinary() {
    ...
    if (!vfsPath) {
        const vfsBase = getVFSBasePath();
        throw new Error(`Could not find esbuild binary in deno compile VFS.\n` +
...
async function ensureEsbuildBinary() {
    if (setupComplete)
        return;
    if (setupPromise) {
        await setupPromise;
    ...
    setupPromise = (async () => {
        if (dntShim.Deno.env.get("ESBUILD_BINARY_PATH")) {
            setupComplete = true;
            return;
        }
        if (!isDenoCompiled()) {
...
export function isEsbuildReady() {

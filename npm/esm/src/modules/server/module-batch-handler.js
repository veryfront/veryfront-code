...
import { HTTP_BAD_REQUEST, HTTP_NOT_FOUND, HTTP_OK, serverLogger as logger, } from "../../utils/index.js";
 ...
 */
export function handleModuleBatch(req, options) {
    const url = new URL(req.url);
    const pathsParam = url.searchParams.get("paths");
    return withSpan("module.batch.handleModuleBatch", async () => {
        const startTime = performance.now();
        if (!pathsParam) {
            return new dntShim.Response("Missing 'paths' parameter", {
        ...
        if (paths.length === 0) {
            return new dntShim.Response("No valid paths provided", {
                status: HTTP_BAD_REQUEST,
                headers: { "Content-Type": "text/plain" },
            });
        }
        if (paths.length > MAX_BATCH_SIZE) {
            return new dntShim.Response(`Too many modules (max: ${MAX_BATCH_SIZE})`, {
                status: HTTP_BAD_REQUEST,
                headers: { "Content-Type": "text/plain" },
            });
        }
        ...
        const results = await Promise.all(paths.map(async (modulePath) => {
        ...
        const successes = results.filter((r) => r.code !== null);
        const failures = results.filter((r) => r.code === null);
        if (successes.length === 0) {
            return new dntShim.Response("No modules could be loaded", {
        ...
        const slowModules = results.filter((r) => r.transformDurationMs > SLOW_TRANSFORM_THRESHOLD_MS);
        ...
        return new dntShim.Response(bundleCode, {
            status: HTTP_OK,
            headers: {
 ...
 */
function generateBatchBundle(successes, failures) {
    const parts = [

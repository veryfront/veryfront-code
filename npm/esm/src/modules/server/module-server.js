...
export function serveModule(req, options) {
    ...
    return withSpan("modules.serve", async () => {
        ...
        if (snippetMatch) {
            ...
            const { getCompiledSnippetAsync } = await import("../../rendering/snippet-renderer.js");
            ...
            try {
                ...
                return createModuleResponse(method, `// Transform Error\nthrow new Error(${JSON.stringify(errorMsg)});`, HTTP_SERVER_ERROR, {
                    "Content-Type": "application/javascript; charset=utf-8",
                    "Cache-Control": "no-cache",
                });
            }
        ...
        try {
            ...
            if (!findResult) {
                ...
                return new dntShim.Response("Module not found", {
...
function getDevModuleHeaders(modulePath) {
    return {
...
function createDevModuleErrorBody(modulePath, errorMessage) {
    const normalizedPath = modulePath.toLowerCase();
    if (normalizedPath.endsWith(".css")) {
...
function createModuleResponse(method, body, status, headers) {
    return new dntShim.Response(method === "HEAD" ? null : body, { status, headers });
}
async function fetchCrossProjectSource(projectRef, filePath) {
    ...
    const response = await dntShim.fetch(registryUrl, { headers });
    if (!response.ok) {

...
export class DAGExecutor {
    ...
    async execute(nodes, run, startFromNode) {
        ...
        while (ready.length > 0) {
            ...
            const results = await Promise.allSettled(batch.map((nodeId) => this.executeNode(nodeMap.get(nodeId), context, nodeStates)));
    ...
    async executeNode(node, context, nodeStates) {
        ...
        switch (config.type) {
            ...
            default:
                throw new Error(`Unknown node type "${config.type}" for node "${node.id}". ` +
    ...
    async executeParallelNode(node, config, context, nodeStates) {
    ...
    async executeWaitNode(node, config, context) {

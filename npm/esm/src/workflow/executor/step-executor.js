...
export class StepExecutor {
    ...
    async execute(node, context) {
        ...
        if (config.type !== "step") {
            throw new Error(`StepExecutor can only execute 'step' nodes, but node "${node.id}" has type '${config.type}'. ` +
        ...
        for (let attempt = 1; attempt <= maxAttempts; attempt++) {
            try {
                ...
                const output = await this.executeWithTimeout(() => this.executeStep(config, resolvedInput, context), timeout, node.id);
     ...
     */
    async executeWithTimeout(fn, timeout, nodeId) {
        let timeoutId;
     ...
     */
    async executeStep(config, input, context) {
        if (config.agent)
    ...
    async executeAgent(agent, input, context) {
        ...
        const response = await resolvedAgent.generate({
            input: agentInput,
            context,
        });
        return {
    ...
    resolveFromRegistry(id, registry, type) {

...
export class GCSBlobStorage {
    ...
    async put(data, options = {}) {
        ...
        if (!response.ok) {
            ...
            throw new Error(`Failed to upload to GCS: ${response.status} - ${response.statusText}. Body: ${errorBody}`);
        }
    ...
    async getStream(id) {
        ...
        try {
            ...
            if (!response.ok) {
                const errorBody = await response.text();
                throw new Error(`Failed to download from GCS: ${response.status} - ${response.statusText}. Body: ${errorBody}`);
            }
    ...
    async delete(id) {
        ...
        if (!response.ok) {
            const errorBody = await response.text();
            throw new Error(`Failed to delete from GCS: ${response.status} - ${response.statusText}. Body: ${errorBody}`);
        }
    }
    async exists(id) {
        const key = this.getKey(id);
        ...
        if (response.status === 404)
            return false;
        const errorBody = await response.text();
        throw new Error(`Failed to check existence in GCS: ${response.status} - ${response.statusText}. Body: ${errorBody}`);
    }
    async stat(id) {
        const key = this.getKey(id);
        const token = await this.getAccessToken();
        ...
        if (!response.ok) {
            const errorBody = await response.text();
            throw new Error(`Failed to get metadata from GCS: ${response.status} - ${response.statusText}. Body: ${errorBody}`);
        }
        const gcsObject = await response.json();
        const metadata = {};
        const rawMetadata = gcsObject.metadata;
        if (rawMetadata) {

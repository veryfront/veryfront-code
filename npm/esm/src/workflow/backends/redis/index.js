...
export class RedisBackend {
    ...
    constructor(config = {}) {
        this.config = {
            ...
            consumerName: `worker-${dntShim.crypto.randomUUID().slice(0, 8)}`,
            debug: false,
            ...config,
        };
        if (config.client)
    ...
    deserializeRun(data) {
        ...
        if (data.status && !validStatuses.includes(status)) {
            throw new Error(`Invalid workflow run data for run "${data.id}": unknown status "${data.status}". ` +
                `Expected one of: ${validStatuses.join(", ")}`);
        }
        function safeJsonParse(runId, field, value, defaultValue) {
            if (!value)
                return defaultValue;
            try {
                ...
                throw new Error(`Invalid workflow run data for run "${runId}": failed to parse '${field}' as JSON. ` +
    ...
    async updateRun(runId, patch) {
        ...
        if (patch.output !== undefined)
            fields.output = JSON.stringify(patch.output);
        if (patch.nodeStates !== undefined)
            fields.nodeStates = JSON.stringify(patch.nodeStates);
        if (patch.currentNodes !== undefined)
            fields.currentNodes = JSON.stringify(patch.currentNodes);
        if (patch.context !== undefined)
            fields.context = JSON.stringify(patch.context);
        if (patch.error !== undefined)
            fields.error = JSON.stringify(patch.error);
    ...
    async listRuns(filter) {
        ...
        for (const runId of runIds) {
            ...
            if (statuses && !statuses.includes(run.status))
                continue;
            if (filter.createdAfter && run.createdAt < filter.createdAfter)
                continue;
            if (filter.createdBefore && run.createdAt > filter.createdBefore)
                continue;
            runs.push(run);
        }
    ...
    async saveCheckpoint(runId, checkpoint) {
        ...
        await client.rpush(this.checkpointsKey(runId), JSON.stringify({ ...checkpoint, timestamp: checkpoint.timestamp.toISOString() }));
    }
    ...
    async savePendingApproval(runId, approval) {
        const client = await this.ensureClient();
        if (this.config.debug)
            logger.debug(`[RedisBackend] Saving approval: ${approval.id}`);
        await client.rpush(this.approvalsKey(runId), JSON.stringify({
            ...approval,
    ...
    async updateApproval(runId, approvalId, decision) {
        ...
        if (!rawTarget)
            throw new Error(`Approval data not found: ${approvalId}`);
    ...
    async listPendingApprovals(filter) {
        ...
        for (const key of keys) {
            ...
            if (filter?.workflowId) {
                const run = await this.getRun(runId);
                if (!run || run.workflowId !== filter.workflowId)
                    continue;
            }
            const rawList = await client.lrange(key, 0, -1);
    ...
    async acquireLock(runId, duration) {
        const client = await this.ensureClient();

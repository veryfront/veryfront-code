...
async function renderToReadableStreamImpl(element, options, server) {
    ...
    try {
        ...
        try {
            if (debug)
                logger.info("SSR trying string rendering fallback");
...
function renderToPipeableStreamImpl(element, options, server) {
    const start = performance.now();
...
export async function renderToStreamAdapter(element, options = {}) {
    const debug = isDebugMode();
    const server = await getReactDOMServer();
    if (hasFeature("renderToReadableStream") && server.renderToReadableStream) {
        if (debug)
            logger.info("SSR using renderToReadableStream");
    ...
    if (hasFeature("renderToPipeableStream") && server.renderToPipeableStream) {
        if (debug)
            logger.info("SSR using renderToPipeableStream");
        return renderToPipeableStreamImpl(element, options, server);
    }
    const { version } = getReactVersionInfo();
    if (debug)
        logger.info("SSR using string rendering", { reactVersion: version });
    try {
        const html = await renderToStringAdapter(element, options);
        return { html };
    }

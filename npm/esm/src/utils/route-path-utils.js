...
export const PAGE_EXTENSIONS = [".tsx", ".jsx", ".ts", ".js", ".mdx", ".md"];
/** Supported component file extensions (subset of page extensions) */
export const COMPONENT_EXTENSIONS = [".tsx", ".jsx", ".ts", ".js"];
...
export function isDynamicSegment(name) {
    if (!name.startsWith("["))
        return false;
    if (name.endsWith("]")) {
        return (DYNAMIC_SEGMENT_PATTERNS.standard.test(name) ||
 ...
 */
export function isCatchAllSegment(name) {
 ...
 */
export function extractRouterBasePath(pageEntityId) {
 ...
 */
export function extractRouteParams(pageEntityId, slug) {
    const params = {};
 ...
 */
export function extractRelativePath(absolutePath, projectDir) {
 ...
 */
export function extractParamsFromPattern(pattern, slug) {
    ...
    for (const patternPart of patternParts) {
        ...
        if (isDynamicSegment(patternPart)) {
            ...
            if (slugPart !== undefined)
                params[extractParamName(patternPart)] = slugPart;
            slugIndex++;
            continue;
        }

...
import { getProviderFromModel } from "../../provider/index.js";
...
import { convertMessageToProvider } from "./message-converter.js";
...
import { createStreamState, processStreamData } from "./stream-handler.js";
export class AgentRuntime {
    ...
    async generate(input, context) {
        return await withSpan("agent.generate", async (span) => {
            ...
            return await chain.execute(agentContext, () => this.executeAgentLoop(provider, model, systemPrompt, messages));
        });
    }
    /**
     * Stream a response
        ...
        return new ReadableStream({
            start: async (controller) => {
                try {
                    ...
                    await this.executeAgentLoopStreaming(provider, model, systemPrompt, memoryMessages, controller, encoder, callbacks, textPartId, toolContext);
     ...
     */
    async executeAgentLoop(provider, model, systemPrompt, messages) {
     ...
     */
    async executeAgentLoopStreaming(provider, model, systemPrompt, messages, controller, encoder, callbacks, textPartId, toolContext) {
        const { maxAgentSteps } = getPlatformCapabilities();
        const maxSteps = this.computeMaxSteps(maxAgentSteps);
        const toolCalls = [];
        const currentMessages = [...messages];

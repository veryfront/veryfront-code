 ...
 * ```
 */
export async function inspectAgent(agent, input) {
...
export function printInspectionReport(report) {
    ...
    agentLogger.info(`  Available: ${report.tools.available.length} (${report.tools.available.join(", ")})`);
...
export function getRegistryOverview() {
    ...
    return {
        ...
        resources: Array.from(registry.resources.values()).map(({ id, pattern, description }) => ({
            id,
            pattern,
            description,
        })),
 ...
 */
export function printRegistryOverview() {

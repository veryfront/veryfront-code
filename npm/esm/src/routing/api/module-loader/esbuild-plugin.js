...
import { computeIntegrity, createLockfileManager, getDenoStdNodeBase, HTTP_MODULE_FETCH_TIMEOUT_MS, HTTP_NETWORK_CONNECT_TIMEOUT, serverLogger as logger, } from "../../../utils/index.js";
        ...
        setup(build) {
            ...
            build.onLoad({ filter: /.*/, namespace: "http-url" }, async (args) => {
                ...
                async function fetchWithTimeout(url) {
                    ...
                    const timeout = dntShim.setTimeout(() => controller.abort(), HTTP_MODULE_FETCH_TIMEOUT_MS);
                    try {
                        return await dntShim.fetch(url, {
                ...
                const res = await fetchWithTimeout(requestUrl).catch((e) => {
                    return new dntShim.Response(String(e?.message || e), { status: HTTP_NETWORK_CONNECT_TIMEOUT });
                });
                if (!res.ok) {

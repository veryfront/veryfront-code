...
import { validateHTTPImports } from "./http-validator.js";
    ...
    if (isDeno) {
        return fs.exists(modulePath).then((fileExistsLocally) => {
            if (fileExistsLocally)
                return loadTSModuleDirect(modulePath);
...
function createImportMapPlugin(projectDir, adapter, config) {
        ...
        setup(build) {
            build.onResolve({ filter: /.*/ }, (args) => {
                ...
                if (args.namespace === "import-map" && args.path.startsWith(".")) {
                    ...
                    logger.debug(`[API] Import map relative resolve: ${args.path} (from ${args.importer}) -> ${absolutePath}`);
...
function loadAndTranspileModule(modulePath, projectDir, adapter, fs, config) {
    return withSpan("api.loadAndTranspileModule", async () => {
        const { filePath: resolvedPath, contents: source } = await readFileWithExtensions(adapter, modulePath, [
            "",
            ".ts",
            ".tsx",
            ".js",
...
async function readFileWithExtensions(adapter, basePath, extensions) {
...
async function loadModuleFromCode(code, projectDir, fs) {
...
async function rewriteExternalImports(code, projectDir, fs) {
...
function extractAPIRouteHandlers(module) {
    ...
    const mod = module;
    const handler = {};
    const methods = ["GET", "POST", "PUT", "PATCH", "DELETE", "HEAD", "OPTIONS", "default"];

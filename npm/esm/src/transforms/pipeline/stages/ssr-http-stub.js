...
function extractImportNames(list) {
    return list
        .split(",")
...
function generateStub(imp, statement) {
    if (!imp.n || imp.d > -1 || !isHttpImport(imp.n) || !isBrowserOnlyModule(imp.n))
        return null;
...
export const ssrHttpStubPlugin = {
    name: "ssr-http-stub",
    ...
    async transform(ctx) {
        ...
        const needsStubbing = imports.some((imp) => imp.n && imp.d === -1 && isHttpImport(imp.n) && isBrowserOnlyModule(imp.n));
        if (!needsStubbing)
            return ctx.code;

...
import { getReactUrls, REACT_VERSION } from "./package-registry.js";
 ...
 */
function getHttpTimeout(env = getRuntimeEnv()) {
...
export function stripDenoShim(code) {
    if (!isDeno)
        return code;
    return code.replace(/globalThis\.Deno\s*=\s*globalThis\.Deno\s*\|\|\s*\{[\s\S]*?env:\s*\{[\s\S]*?\}\s*\};?/g, "/* Deno shim stripped */");
}
/** Re-export getReactUrls for backwards compatibility */
export { getReactUrls };
/** Alias for getReactUrls - used by esbuild bundling */
export function getReactAliases() {
    return getReactUrls();
}
/**
    ...
    return {
        ...
        setup(build) {
            ...
            build.onLoad({ filter: /.*/, namespace: "http-url" }, async (args) => {
                ...
                const timeout = dntShim.setTimeout(() => controller.abort(), getHttpTimeout());
                try {
                    const res = await dntShim.fetch(requestUrl, {

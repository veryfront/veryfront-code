...
import { buildReactUrl, CSSTYPE_VERSION, DEFAULT_REACT_VERSION, getReactImportMap as getReactImportMapFromRewriter, TAILWIND_VERSION, } from "../import-rewriter/url-builder.js";
...
export function normalizeReactVersion(version) {
    ...
    if (isValidReactVersion(version))
        return version;
    rendererLogger.warn(`Invalid React version format "${version}" (expected X.Y.Z). Using default: ${DEFAULT_REACT_VERSION}`);
    return DEFAULT_REACT_VERSION;
}
/**
...
export function esmShReact(pkg, version, path = "", external = false) {
    return buildReactUrl(pkg, version, path || undefined, external);
}
/**

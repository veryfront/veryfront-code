...
<div>{random}</div>`,
    ...
    [ErrorCode.RENDER_ERROR]: createSimpleError(ErrorCode.RENDER_ERROR, "Render error", "Failed to render component.", [
        "Check the component for errors",
        "Ensure all props are valid",
        "Look for null/undefined access",
        "Check error boundaries",
    ]),
    [ErrorCode.COMPONENT_ERROR]: createSimpleError(ErrorCode.COMPONENT_ERROR, "Component error", "Error in component lifecycle or render.", [
...
export default function RootLayout({ children }) {
  return (
    ...
    [ErrorCode.PAGE_NOT_FOUND]: createSimpleError(ErrorCode.PAGE_NOT_FOUND, "Page not found", "The requested page does not exist.", [
        "Check that the page file exists",
        "Verify file name matches route",
        "Ensure file extension is correct (.tsx, .jsx, .mdx)",
    ]),
    [ErrorCode.API_ERROR]: createSimpleError(ErrorCode.API_ERROR, "API handler error", "Error in API route handler.", [
        "Check API handler code for errors",
        "Ensure proper error handling",
        "Verify request/response format",
    ]),
    [ErrorCode.MIDDLEWARE_ERROR]: createSimpleError(ErrorCode.MIDDLEWARE_ERROR, "Middleware error", "Error in middleware execution.", [

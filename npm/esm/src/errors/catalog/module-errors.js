...
resolve: {
  importMap: {
    ...
    [ErrorCode.IMPORT_RESOLUTION_ERROR]: createSimpleError(ErrorCode.IMPORT_RESOLUTION_ERROR, "Import resolution failed", "Failed to resolve import specifier.", [
        "Check import paths are correct",
        "Ensure modules are in import map",
        "Verify network connectivity for remote imports",
    ]),
    [ErrorCode.CIRCULAR_DEPENDENCY]: createSimpleError(ErrorCode.CIRCULAR_DEPENDENCY, "Circular dependency detected", "Files are importing each other in a circle.", [
        "Identify the circular import chain",
        "Extract shared code to separate file",
        "Use dependency injection or lazy imports",
    ]),
    [ErrorCode.INVALID_IMPORT]: createSimpleError(ErrorCode.INVALID_IMPORT, "Invalid import statement", "Import statement has invalid syntax.", [
...
resolve: {
  importMap: {
    ...
    [ErrorCode.VERSION_MISMATCH]: createSimpleError(ErrorCode.VERSION_MISMATCH, "Dependency version mismatch", "Incompatible versions of dependencies detected.", [

...
import { getSSRProjectDomain, getSSRServerPort, isSSRClientOnlyFetching, originalFetch, } from "./context.js";
...
function isProjectDomain(hostname) {
 ...
 */
function rewriteFetchUrlForSSR(url) {
 ...
 */
function isClientOnlyApiUrl(url) {
...
function createSSRFetch() {
    return (input, init) => {
        ...
        return withSpan(SpanNames.HTTP_CLIENT_FETCH, async () => {
            ...
            const response = await (rewrittenUrl === url
...
export function enableSSRFetchInterception() {
    if (!getSSRServerPort())
        return;
    dntShim.dntGlobalThis.fetch = createSSRFetch();
}
export function disableSSRFetchInterception() {
    dntShim.dntGlobalThis.fetch = originalFetch;
}

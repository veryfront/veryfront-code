...
import { areSharedServicesInitialized, getSharedServices, initializeSharedServices, setSharedCompileMDX, } from "./shared/shared-services.js";
import { ContextAwareCacheCoordinator, } from "./shared/context-aware-cache.js";
import { createComponentRegistry, createLayoutCollector, createLayoutCompiler, createPageRenderer, createPageResolver, createSSRRenderer, createVirtualModuleSystem, } from "./factories/service-factories.js";
 ...
 */
const projectSlotLocks = new Map();
 ...
 */
async function acquireProjectSlot(projectId) {
 ...
 */
async function releaseProjectSlot(projectId) {
    if (RENDER_PER_PROJECT_LIMIT <= 0)
        return;
    const release = await acquireProjectLock(projectId);
    try {
...
export class Renderer {
    ...
    renderPage(slug, ctx, options) {
        return withSpan("renderer.renderPage", async () => {
            ...
            if (!acquired) {
                ...
                throw new VeryfrontError(`Render capacity exceeded (${renderSemaphore.waiting} waiting). Service is overloaded.`, ErrorCode.SERVICE_OVERLOADED, { slug, projectId: ctx.projectId, waiting: renderSemaphore.waiting });
            }
            try {
     ...
     */
    getSingleflightKey(slug, ctx, colorScheme) {
        return `${ctx.projectId}:${ctx.environment}:${ctx.contentSourceId ?? "draft"}:${slug}:${colorScheme ?? "default"}`;
    }
    async doRenderPage(slug, ctx, options, startTime) {
        ...
        const cachedData = await this.renderFlight.do(flightKey, async () => {
            ...
            try {
                ...
                result = await withTimeoutThrow(services.pipeline.renderPage(slug, {
                    ...options,
                    ...
                    contentSourceId,
                    skipCacheCheck: true,
                }), RENDER_PIPELINE_TIMEOUT_MS, `Render pipeline for ${ctx.projectId}:${slug}`);
            }
            catch (error) {
    ...
    resolvePageData(slug, ctx, options) {
        if (!this.initialized) {
            throw new Error("Renderer not initialized. Call initialize() first.");
        }
        const contentSourceId = ctx.contentSourceId;
    ...
    async clearCacheForProject(projectId) {
...
export function renderPage(slug, handlerCtx, options, contextOptions) {
    const r = getRenderer();

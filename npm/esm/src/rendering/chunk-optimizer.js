...
function shouldSkipDir(name, parentPath) {
    if (name === ".veryfront")
        return false; // Allow .veryfront directory itself
    if (name.startsWith("."))
        return true; // Skip other hidden directories
    if (parentPath?.includes(".veryfront") && VERYFRONT_EXCLUDED_DIRS.has(name))
        return true;
    return false;
}
const SIZE_LIMITS = {
...
export async function analyzeProjectChunks(projectDir, fs) {
...
const CHUNK_CONFIGS = [
    {
        name: "common",
        getDeps: (sharedDeps) => Array.from(sharedDeps.entries())
    ...
    {
        name: "react-vendor",
        getDeps: (sharedDeps) => Array.from(sharedDeps.keys()).filter((dep) => dep.includes("react") || dep.includes("jsx-runtime")),
        calculateBenefit: () => SIZE_LIMITS.REACT_SIZE_ESTIMATE,
    },
    {
        name: "ui-vendor",
        getDeps: (sharedDeps) => Array.from(sharedDeps.keys()).filter((dep) => dep.includes("@mui/") || dep.includes("framer-motion") || dep.includes("@headlessui/")),
        calculateBenefit: (deps) => deps.length * SIZE_LIMITS.UI_LIB_SIZE_ESTIMATE,
    },
];
function generateChunkSuggestions(pages, sharedDeps) {
    const suggestions = [];
...
export function generateChunkManifest(analysis) {
    const manifest = {
        version: "1.0",
        chunks: {},
        pages: {},
    ...
    for (const [pagePath, imports] of analysis.pages) {
        ...
        for (const chunk of analysis.suggestedChunks) {
            if (chunk.deps.some((dep) => pageDepSet.has(dep)))

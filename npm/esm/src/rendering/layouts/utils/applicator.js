...
export function applyLayoutsESM(pageElement, layoutBundle, nestedLayouts, projectDir, mergedComponents, tsxLayoutModuleCache, adapter, layoutDataMap, projectId, projectSlug, contentSourceId, preloadedImportMap) {
    return withSpan(SpanNames.LAYOUT_APPLY_LAYOUTS_ESM, async () => {
        ...
        for (let i = nestedLayouts.length - 1; i >= 0; i--) {
            ...
            try {
                if (item.kind === "mdx" && item.bundle?.compiledCode) {
                    element = await withSpan(SpanNames.LAYOUT_APPLY_MDX, () => applyMDXLayout(element, item.bundle, projectDir, mergedComponents, adapter, projectId, projectSlug, contentSourceId, preloadedImportMap), {
                        "layout.index": i,
                        "layout.kind": "mdx",
        ...
        element = await withSpan(SpanNames.LAYOUT_APPLY_MDX, () => applyMDXLayout(element, layoutBundle, projectDir, mergedComponents, adapter, projectId, projectSlug, contentSourceId, preloadedImportMap), { "layout.kind": "mdx", "layout.type": "named" });
...
export async function applyLayoutsFunctionBody(pageElement, layoutBundle, nestedLayouts, mergedComponents, tsxLayoutModuleCache, projectDir, adapter, layoutDataMap, projectId, projectSlug, contentSourceId) {
    ...
    for (let i = nestedLayouts.length - 1; i >= 0; i--) {
        ...
        try {
            ...
            element = React.createElement(LayoutComponent, props, child);

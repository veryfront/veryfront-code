...
function isValidLayoutPath(layout) {
 ...
 */
function createLayoutItem(layoutPath, bundle) {
 ...
 */
export async function discoverComponentsLayoutPath(projectDir, checker) {
...
export class LayoutCollector {
    ...
    async collectLayouts(pageInfo) {
        return await withSpan(SpanNames.LAYOUT_COLLECT, async () => {
            ...
            const { layoutBundle, layoutPath, layoutName } = await withSpan(SpanNames.LAYOUT_COLLECT_NAMED, () => this.collectNamedLayoutWithPath(pageInfo), {
                "layout.page_path": pagePath,
                "layout.config_layout": this.config?.layout || "none",
            });
            return await this.processLayoutResult(pageInfo, hasExplicitFrontmatterLayout, layoutBundle, layoutPath, layoutName);
        }, {
            "layout.page_path": pageInfo.entity.path,
            "layout.project_dir": this.projectDir,
        });
    }
    async processLayoutResult(pageInfo, hasExplicitFrontmatterLayout, layoutBundle, layoutPath, layoutName) {
        if (hasExplicitFrontmatterLayout && layoutPath) {
            const nestedLayouts = [createLayoutItem(layoutPath, layoutBundle)];
        ...
        let nestedLayouts = await withSpan(SpanNames.LAYOUT_COLLECT_NESTED, () => this.collectNestedLayouts(pageInfo), { "layout.page_path": pageInfo.entity.path });
    ...
    async collectNamedLayoutWithPath(pageInfo) {
        const layoutValue = pageInfo.entity.frontmatter.layout;
        ...
        const layoutInfo = await withSpan(SpanNames.LAYOUT_GET_ENTITY, () => getLayoutEntity(this.projectDir, layoutName, this.adapter), { "layout.name": layoutName, "layout.project_dir": this.projectDir });
        ...
        if (!layoutInfo) {
            ...
            throw new Error(`Layout "${layoutName}" not found. Specified in ${source} for page "${pageInfo.entity.path}". ` +
        ...
        const layoutBundle = await this.compileMDX(layoutInfo.entity.content, { ...layoutInfo.entity.frontmatter, isLayout: true }, layoutPath);
    ...
    async collectAPILayoutConfiguration(wrappedAdapter) {
        ...
        const checker = {
            exists: (path) => wrappedAdapter.exists.call(wrappedAdapter, path),
        };
        if (configLayout && isValidLayoutPath(configLayout)) {
            ...
            if (!layoutExists) {
                throw new Error(`Layout file not found: "${configLayout}" (resolved to "${layoutPath}"). ` +
    ...
    async collectFilesystemLayouts(pageFilePath, useAppRouter) {
        ...
        const nestedLayouts = await discoverNestedLayouts(pageFilePath, rootDir, this.projectDir, this.adapter);
     ...
     */
    async checkComponentsLayoutFallback() {
        const checker = {
            exists: async (path) => {
                try {
     ...
     */
    async createLayoutItemWithBundle(layoutPath) {
        const kind = getLayoutKind(layoutPath);
        if (kind === "mdx") {

...
function isDotPath(slug, filePath) {
    const hasDotSegment = (path) => path.split("/").some(isHiddenSegment);
...
export class RenderPipeline {
     ...
     */
    collectModulesToLoad(pagePath, isComponentPage, isInPagesOrAppDir, nestedLayouts) {
        const modules = [];
    ...
    async renderPage(slug, options) {
        ...
        return await withSpan("render.page", async () => {
            ...
            if (options?.request && options?.url) {
                await withSpan("render.data_fetching", async () => {
                    try {
                        ...
                        const modulesToLoad = this.collectModulesToLoad(pageInfo.entity.path, isComponentPage, isInPagesDir || isInAppDir, layoutResult.nestedLayouts);
                        ...
                        if (dataJobs.length === 0)
                            return;
                        const dataResults = await withSpan(SpanNames.RENDER_FETCH_DATA, () => withTimeoutThrow(Promise.all(dataJobs.map((job) => this.dataFetcher
                            .fetchData(job.mod, dataContext, this.config.mode)
                            .then((result) => ({ ...job, result, error: null }))
                            .catch((error) => ({ ...job, result: null, error })))), DATA_FETCH_TIMEOUT_MS, `Data fetch for ${slug}`), { "render.data_job_count": dataJobs.length });
                        for (const { type, id, result, error } of dataResults) {
                            ...
                            if (!result)
                                continue;
                            if (result.notFound) {
                                throw new VeryfrontError("Page/Layout returned notFound", ErrorCode.FILE_NOT_FOUND, { slug, component: id });
                            }
                            if (result.redirect) {
                                throw new VeryfrontError(`Redirect to ${result.redirect.destination}`, ErrorCode.RENDER_ERROR, { slug, redirect: result.redirect });
                            }
                            if (!result.props)
                                continue;
                            if (type === "page") {
                                dataFetchingProps = result.props;
                            }
                            else {
                                layoutDataMap.set(id, result.props);
                            }
                        }
                    }
                    catch (error) {
    ...
    async resolvePageData(slug, options) {
        setupSSRGlobals();
        const projectId = options?.projectId ?? this.config.projectDir;
        this.moduleLoaderConfig.projectId = projectId;
        if (this.config.mode === "development") {
        ...
        if (options?.request && options?.url) {
            ...
            const modulesToLoad = this.collectModulesToLoad(pageInfo.entity.path, isComponentPage, isInPagesDir || isInAppDir, layoutResult.nestedLayouts);
            if (modulesToLoad.length > 0) {
                const loadedModules = await withTimeoutThrow(Promise.all(modulesToLoad.map((m) => this.loadModule(m.path)
                    .then((mod) => ({ ...m, mod }))
                    .catch(() => ({ ...m, mod: null })))), MODULE_LOAD_TIMEOUT_MS, `Module loading for ${slug}`);
                ...
                const dataResults = await Promise.all(dataJobs.map((job) => job.promise.then((result) => ({ ...job, result }))));
        ...
        if (pageType === "mdx") {
            try {
                const bundleResult = await this.config.pageRenderer.preparePageBundles(pageInfo, slug, undefined, options);

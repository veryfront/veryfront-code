...
function createErrorFallbackComponent(componentName, error) {
    const ErrorFallback = () => {
        ...
        }, `⚠️ Component "${componentName}" failed to load`), React.createElement("pre", {
            style: {
                fontSize: "12px",
                color: "#7f1d1d",
                whiteSpace: "pre-wrap",
...
export class ComponentRegistry {
     ...
     */
    constructor(virtualModules, serverPort = DEFAULT_DASHBOARD_PORT, adapter, moduleServerUrl, vendorBundleHash, projectId, contentSourceId) {
     ...
     */
    async loadFromDirectory(dir, deferLoading = false) {
    ...
    async initializeComponents() {
        ...
        for (const [componentName, info] of this.componentSources) {
            try {
                const Component = await loadComponentFromSource(info.source, info.filePath, info.projectRoot, adapter, this.getLoaderOptions(info.projectRoot));
                ...
                this.components.set(componentName, createErrorFallbackComponent(componentName, errorMessage));
        ...
        if (failCount > 0) {
            logger.warn(`Component initialization complete: ${successCount} succeeded, ${failCount} failed (using fallbacks, set LOG_LEVEL=debug for details)`);
            return;
        }
    ...
    async collectComponents(dir, projectRoot, deferLoading) {
        ...
        for await (const entry of readDir(dir)) {
            ...
            if (entry.name === "node_modules")
                continue;
            if (entry.name.startsWith(".") && entry.name !== ".veryfront")
                continue;
            if (dir.includes(".veryfront") &&
                ["cache", "compiled", "tmp", "temp", "output", "optimized-images", "css"].includes(entry.name)) {
                continue;
            }
            ...
            try {
                ...
                if (deferLoading) {
                    ...
                    const Component = await loadComponentFromSource(fileContent, entryPath, projectRoot, this.adapter, this.getLoaderOptions(projectRoot));
                    this.components.set(componentName, Component);
        ...
        if (failureCount > 0) {
            ...
            logger.warn(`Component scan: ${failureCount} failure${failureCount === 1 ? "" : "s"} (set LOG_LEVEL=debug for details)`, { dir: relativeDir });
        }
        return count;
    }
}

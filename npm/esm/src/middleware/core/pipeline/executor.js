...
export function executeMiddlewarePipeline(req, composedMiddleware, env, executionCtx, adapter) {
    return withSpan("middleware.pipeline.execute", async () => {
        const context = new MiddlewareContext(req, env ?? {}, executionCtx);
        try {
            const response = await composedMiddleware(context, () => Promise.resolve(notFoundResponse()));

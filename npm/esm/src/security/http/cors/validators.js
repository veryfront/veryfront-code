...
import { recordCorsRejection } from "../../../observability/index.js";
import { withSpan } from "../../../observability/tracing/otlp-setup.js";
const NO_CORS_RESULT = { allowedOrigin: null, allowCredentials: false };
/** Early validation checks common to sync and async paths */
function validateEarly(requestOrigin, config) {
    if (!config)
...
function validateStaticOrigin(requestOrigin, corsConfig) {
...
function processFunctionResult(result, requestOrigin, credentials) {
...
export function validateOrigin(requestOrigin, config) {
...
export function validateOriginSync(requestOrigin, config) {
    const earlyResult = validateEarly(requestOrigin, config);
    if (earlyResult)
        return earlyResult;
    const corsConfig = config;

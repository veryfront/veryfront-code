...
import { recordSecurityHeaders } from "../../../observability/index.js";
...
export function buildCSP(_isDev, nonce, cspUserHeader, config, adapter) {
...
export function getSecurityHeader(headerName, defaultValue, config, adapter) {
...
export function applySecurityHeaders(headers, isDev, nonce, cspUserHeader, config, adapter, isVeryfrontDomain) {
    ...
    if (!isDev) {
        ...
        if (hstsPreload)
            hstsValue += "; preload";
        headers.set("Strict-Transport-Security", getHeaderOverride("strict-transport-security") ?? hstsValue);
    }

 ...
 */
export async function buildEmbeddedPreset(options) {
    ...
    const rscFiles = [
        new URL("../../rendering/rsc/client-dom.ts", globalThis[Symbol.for("import-meta-ponyfill-esmodule")](import.meta).url),
        new URL("../../rendering/rsc/client-hydrator.ts", globalThis[Symbol.for("import-meta-ponyfill-esmodule")](import.meta).url),
        new URL("../../rendering/rsc/hydrate-client.ts", globalThis[Symbol.for("import-meta-ponyfill-esmodule")](import.meta).url),
    ];
    for (const url of rscFiles) {
        try {
            ...
            logger.warn("embedded: failed to process RSC file", { error: String(e) });
        }
    }
    const manifest = {
        version: 1,
    ...
    logger.info("Embedded preset built", { outDir: embeddedDir });
...
async function findOrCreateEntryPath(fs, projectDir) {
    const candidates = [
    ...
    await fs.writeTextFile(entryPath, `export default function Page(){ return '<div>Veryfront</div>'; }`);
    return entryPath;
}
async function bundleEmbeddedApp(params) {
    ...
    try {
        ...
        throw toError(createError({
            type: "build",
            message: `Failed to bundle embedded app: ${error instanceof Error ? error.message : String(error)}`,
        }));
    }
}
async function discoverAppRoutes(fs, projectDir, embeddedDir) {
    const results = [];
    ...
    async function walk(dir, rel = "") {
        for await (const ent of fs.readDir(dir)) {
            ...
            if (!ent.isFile || (ent.name !== "page.mdx" && ent.name !== "page.md"))
                continue;
...
async function discoverPagesRoutes(fs, projectDir, embeddedDir) {
    const results = [];

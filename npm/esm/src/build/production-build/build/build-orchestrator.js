...
import { cleanupCaches, cleanupRenderer, logBuildCompletion, performCleanup, } from "./build-cleanup.js";
...
export function buildProduction(options) {
    return withSpan("build.production", async () => {
        ...
        await withSpan("build.setupDirectories", () => setupBuildDirectories(context.adapter, normalizedOptions.outputDir ?? "", normalizedOptions.dryRun ?? false), {});
        const routes = await withSpan("build.collectRoutes", () => collectAllRoutes(context.adapter, normalizedOptions.projectDir, normalizedOptions.ssg ?? true, normalizedOptions.include, normalizedOptions.exclude), {});
        const splitResult = await withSpan("build.codeSplitting", () => runCodeSplitting(normalizedOptions.projectDir, normalizedOptions.outputDir ?? "", routes.pages, normalizedOptions.enableSplitting ?? true, normalizedOptions.dryRun ?? false), {});
        context.stats.chunks = splitResult.chunks;
        const buildResult = await withSpan("build.execute", () => executeBuild(routes.pages, routes.app, {
        ...
        await withSpan("build.generateOutputs", () => generateAllOutputs({
            adapter: context.adapter,
            projectDir: normalizedOptions.projectDir,
            outputDir: normalizedOptions.outputDir ?? "",
            routes: routes.pages,

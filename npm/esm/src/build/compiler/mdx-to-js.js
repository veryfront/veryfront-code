...
function getComponentName(imp) {
    return imp.path.split("/").pop()?.replace(/\.(jsx?|tsx?)$/, "") ?? imp.name;
}
async function extractFrontmatter(mdxContent) {
    try {
        const result = extract(mdxContent);
        return {
            frontmatter: (result.attrs ?? {}),
            content: result.body,
        };
    }
    catch (error) {
 ...
 */
export async function compileMDXToJS(mdxPath, mdxContent, options) {
...
export async function compileMDXFile(mdxPath, outputDir, options) {
...
export async function compileProjectMDX(projectDir, outputDir, options) {
    const compileOptions = { ...options, projectDir };
        ...
        try {
            for await (const entry of fs.readDir(dir)) {
                ...
                if (entry.name.startsWith(".") && entry.name !== ".veryfront")
                    continue;
                if (dir.includes(".veryfront") && VERYFRONT_EXCLUDED_DIRS.has(entry.name))
                    continue;

...
export function getOutputPath(inputPath, outputDir) {
...
export function extractSelectors(content) {
    const classes = [];
    const ids = [];
    const tags = [];
...
export function extractSelectorsFromHTML(html) {
    return Array.from(extractSelectors(html).selectors);
}
const UNIVERSAL_SELECTORS = new Set(["*", ":root", "html", "body", "@"]);
export function shouldKeepSelector(selector, usedSelectors) {
...
export function basicMinify(css) {
    return css
        .replace(/\/\*[\s\S]*?\*\//g, "")
        .replace(/\s+/g, " ")
        .replace(/\s*([{};:,])\s*/g, "$1")
...
export function calculateSavings(originalSize, minifiedSize) {
    if (originalSize === 0)
        return 0;
    return Math.round(((originalSize - minifiedSize) / originalSize) * 100);
}
export function parseBrowserTargets(targets) {
    if (!targets)
        return undefined;

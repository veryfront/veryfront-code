...
export function generateVariant(sharp, image, relPath, format, width, metadata, quality, outputDir) {
...
export function generateImageVariants(sharp, image, relPath, metadata, formats, sizes, quality, outputDir) {
    return withSpan("build.asset.generateImageVariants", async () => {
        ...
        for (const size of allSizes) {
            for (const format of formats) {
                const variant = await generateVariant(sharp, image, relPath, format, size, metadata, quality, outputDir);
                if (variant)

 ...
 */
export async function buildVendorBundle(config) {
    ...
    const thirdPartyImports = Object.fromEntries(Object.entries(dependencies).map(([pkg, version]) => [
        pkg,
    ...
    const exports = Object.fromEntries(Object.keys(imports).map((key) => [key, sanitizeExportName(key)]));
 ...
 * ```
 */
function createVirtualEntry(imports) {
    const lines = [];
    const exportNames = [];
 ...
 */
function sanitizeExportName(specifier) {
    return specifier

import { buildAttributes, escapeHTML } from "./html-escape.js";
function filterAttrs(obj, excludeKeys) {
    const entries = Object.entries(obj).filter(([key]) => !excludeKeys.includes(key));
    return Object.fromEntries(entries);
}
function addNonceIfPresent(attrs, nonce) {
    if (!nonce)
        return attrs;
    return { ...attrs, nonce };
}
export function generateMetaTags(metadata) {
    ...
    for (const meta of metadata.meta ?? []) {
        tags.push(`<meta ${buildAttributes(meta)}>`);
    }
...
export function generateLinkTags(metadata) {
    const tags = [];
    ...
    for (const icon of metadata.icons ?? []) {
        ...
        tags.push(`<link ${buildAttributes({ rel, ...icon })}>`);
    }
    return tags.join("\n  ");
}
export function generateScriptTags(metadata, nonce) {
    const tags = [];
...
export function generateStyleTags(metadata, nonce) {
    const tags = [];

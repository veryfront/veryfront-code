...
let cssInputsCache = null;
let cssInputsCacheInitPromise = null;
const localCssInputsCache = new Map();
...
let projectCSSBackend = null;
let projectCSSInitialized = false;
let projectCSSInitPromise = null;
const projectCSSLocalFallback = new Map();
 ...
 */
export function isProjectCSSCacheDistributed() {
...
export async function getProjectCSS(projectSlug, stylesheet, candidates, options) {
    ...
    if (projectCSSBackend) {
        projectCSSBackend.set(cacheKey, JSON.stringify(entry), PROJECT_CSS_CACHE_TTL_SECONDS).catch((error) => {
 ...
 */
export function invalidateProjectCSS(projectSlug) {
 ...
 */
export async function invalidateProjectCSSAsync(projectSlug) {
    if (!projectCSSBackend?.delByPattern)
        return;
    try {
...
function setProjectCSSLocalFallback(key, entry) {
...
function pruneProjectCSSLocalFallback() {
...
function storeInLocalCache(hash, css) {
...
function touchLocalCache(hash, css) {
    localCssCache.delete(hash);
    localCssCache.set(hash, css);
}
function getCssCache() {
    if (cssCache)
        return Promise.resolve(cssCache);
    if (cssCacheInitPromise)
...
export async function cacheCSSAsync(css, hash) {
...
export function getCSSByHash(hash) {
    const css = localCssCache.get(hash);
    if (css)
        touchLocalCache(hash, css);
    return css;
}
export async function getCSSByHashAsync(hash) {
    return await withSpan(SpanNames.HTML_GET_CSS_BY_HASH, async () => {
...
// ============================================================================
function getCssInputsCache() {
    if (cssInputsCache)
        return Promise.resolve(cssInputsCache);
    if (cssInputsCacheInitPromise)
...
function storeInLocalCssInputsCache(hash, entry) {
    if (localCssInputsCache.has(hash))
        return;
    if (localCssInputsCache.size >= LOCAL_CSS_INPUTS_CACHE_MAX) {
        const firstKey = localCssInputsCache.keys().next().value;
        if (firstKey)
            localCssInputsCache.delete(firstKey);
    }
 ...
 */
async function storeCSSInputsAsync(hash, candidates, stylesheet) {
    const candidatesArray = Array.isArray(candidates) ? candidates : [...candidates];
    const entry = { candidates: candidatesArray, stylesheet };
 ...
 */
async function getCSSInputsByHash(hash) {

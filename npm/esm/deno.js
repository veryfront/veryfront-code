export default {
    "name": "veryfront",
    "version": "0.1.0",
    "nodeModulesDir": "auto",
    "exclude": [
        "npm/",
        "dist/",
        "coverage/",
        "scripts/",
        "examples/",
        "data/",
        ".deno_cache/",
        "src/cli/templates/files/",
        "src/cli/templates/integrations/"
    ],
    "exports": {
        ".": "./src/index.ts",
        "./cli": "./src/cli/main.ts",
        "./server": "./src/server/index.ts",
        "./middleware": "./src/middleware/index.ts",
        "./head": "./src/react/components/Head.tsx",
        "./router": "./src/react/router/index.ts",
        "./context": "./src/react/context/index.ts",
        "./fonts": "./src/react/fonts/index.ts",
        "./react": "./src/react/index.ts",
        "./react/components": "./src/react/components/index.ts",
        "./react/components/ai": "./src/react/components/ai/index.ts",
        "./react/primitives": "./src/react/primitives/index.ts",
        "./react/context": "./src/react/context/index.ts",
        "./react/router": "./src/react/router/index.ts",
        "./react/fonts": "./src/react/fonts/index.ts",
        "./react/head": "./src/react/components/Head.tsx",
        "./data": "./src/data/index.ts",
        "./config": "./src/config/index.ts",
        "./platform": "./src/platform/index.ts",
        "./tool": "./src/tool/index.ts",
        "./tool/schema": "./src/tool/schema/index.ts",
        "./tool/testing": "./src/tool/testing/index.ts",
        "./provider": "./src/provider/index.ts",
        "./provider/adapters": "./src/provider/adapters/index.ts",
        "./agent": "./src/agent/index.ts",
        "./agent/react": "./src/agent/react/index.ts",
        "./agent/react/client": "./src/agent/react/client.ts",
        "./agent/memory": "./src/agent/memory/index.ts",
        "./agent/composition": "./src/agent/composition/index.ts",
        "./agent/middleware": "./src/agent/middleware/index.ts",
        "./agent/testing": "./src/agent/testing/index.ts",
        "./agent/debug": "./src/agent/debug/index.ts",
        "./workflow": "./src/workflow/index.ts",
        "./workflow/react": "./src/workflow/react/index.ts",
        "./prompt": "./src/prompt/index.ts",
        "./resource": "./src/resource/index.ts",
        "./mcp": "./src/mcp/index.ts",
        "./oauth": "./src/oauth/index.ts",
        "./oauth/providers": "./src/oauth/providers/index.ts",
        "./oauth/handlers": "./src/oauth/handlers/index.ts",
        "./oauth/token-store": "./src/oauth/token-store/index.ts",
        "./embeddings": "./src/embeddings/index.ts"
    },
    "imports": {
        "veryfront/head": "./src/react/components/Head.tsx",
        "veryfront/router": "./src/react/router/index.ts",
        "veryfront/context": "./src/react/context/index.ts",
        "veryfront/fonts": "./src/react/fonts/index.ts",
        "veryfront/react/head": "./src/react/components/Head.tsx",
        "veryfront/react/router": "./src/react/router/index.ts",
        "veryfront/react/context": "./src/react/context/index.ts",
        "veryfront/react/fonts": "./src/react/fonts/index.ts",
        "veryfront/react/components/ai": "./src/react/components/ai/index.ts",
        "veryfront/agent/react": "./src/agent/react/index.ts",
        "veryfront/workflow/react": "./src/workflow/react/index.ts",
        "#veryfront": "./src/index.ts",
        "#veryfront/agent": "./src/agent/index.ts",
        "#veryfront/agent/react": "./src/agent/react/index.ts",
        "#veryfront/build": "./src/build/index.ts",
        "#veryfront/cache": "./src/cache/index.ts",
        "#veryfront/cli/ui": "./src/cli/ui/index.ts",
        "#veryfront/config": "./src/config/index.ts",
        "#veryfront/data": "./src/data/index.ts",
        "#veryfront/errors": "./src/errors/index.ts",
        "#veryfront/html": "./src/html/index.ts",
        "#veryfront/mcp": "./src/mcp/index.ts",
        "#veryfront/middleware": "./src/middleware/index.ts",
        "#veryfront/modules": "./src/modules/index.ts",
        "#veryfront/observability": "./src/observability/index.ts",
        "#veryfront/platform": "./src/platform/index.ts",
        "#veryfront/prompt": "./src/prompt/index.ts",
        "#veryfront/provider": "./src/provider/index.ts",
        "#veryfront/react": "./src/react/index.ts",
        "#veryfront/rendering": "./src/rendering/index.ts",
        "#veryfront/resource": "./src/resource/index.ts",
        "#veryfront/routing": "./src/routing/index.ts",
        "#veryfront/security": "./src/security/index.ts",
        "#veryfront/server": "./src/server/index.ts",
        "#veryfront/testing": "./src/testing/index.ts",
        "#veryfront/tool": "./src/tool/index.ts",
        "#veryfront/tool/schema": "./src/tool/schema/index.ts",
        "#veryfront/transforms": "./src/transforms/index.ts",
        "#veryfront/types": "./src/types/index.ts",
        "#veryfront/utils": "./src/utils/index.ts",
        "#veryfront/workflow": "./src/workflow/index.ts",
        "#veryfront/workflow/react": "./src/workflow/react/index.ts",
        "#veryfront/embeddings": "./src/embeddings/index.ts",
        "#veryfront/http/responses": "./src/platform/compat/http/responses.ts",
        "#veryfront/compat/console": "./src/platform/compat/console/index.ts",
        "#veryfront/compat/http": "./src/platform/compat/http/index.ts",
        "#veryfront/compat/kv": "./src/platform/compat/kv/index.ts",
        "#veryfront/compat/path": "./src/platform/compat/path/index.ts",
        "#veryfront/compat/": "./src/platform/compat/",
        "#veryfront/http/": "./src/platform/compat/http/",
        "#veryfront/platform/": "./src/platform/",
        "#veryfront/components": "./src/react/components/index.ts",
        "#veryfront/": "./src/",
        "@veryfront": "./src/index.ts",
        "@veryfront/": "./src/",
        "@veryfront/agent": "./src/agent/index.ts",
        "@veryfront/agent/react": "./src/agent/react/index.ts",
        "@veryfront/agent/middleware": "./src/agent/middleware/index.ts",
        "@veryfront/agent/memory": "./src/agent/memory/index.ts",
        "@veryfront/agent/composition": "./src/agent/composition/index.ts",
        "@veryfront/agent/testing": "./src/agent/testing/index.ts",
        "@veryfront/agent/debug": "./src/agent/debug/index.ts",
        "@veryfront/build": "./src/build/index.ts",
        "@veryfront/cache": "./src/cache/index.ts",
        "@veryfront/cli/ui": "./src/cli/ui/index.ts",
        "@veryfront/compat/": "./src/platform/compat/",
        "@veryfront/compat/console": "./src/platform/compat/console/index.ts",
        "@veryfront/compat/path": "./src/platform/compat/path/index.ts",
        "@veryfront/components": "./src/react/components/index.ts",
        "@veryfront/config": "./src/config/index.ts",
        "@veryfront/data": "./src/data/index.ts",
        "@veryfront/errors": "./src/errors/index.ts",
        "@veryfront/html": "./src/html/index.ts",
        "@veryfront/http/responses": "./src/platform/compat/http/responses.ts",
        "@veryfront/mcp": "./src/mcp/index.ts",
        "@veryfront/middleware": "./src/middleware/index.ts",
        "@veryfront/modules": "./src/modules/index.ts",
        "@veryfront/oauth": "./src/oauth/index.ts",
        "@veryfront/observability": "./src/observability/index.ts",
        "@veryfront/platform": "./src/platform/index.ts",
        "@veryfront/prompt": "./src/prompt/index.ts",
        "@veryfront/provider": "./src/provider/index.ts",
        "@veryfront/react": "./src/react/index.ts",
        "@veryfront/rendering": "./src/rendering/index.ts",
        "@veryfront/resource": "./src/resource/index.ts",
        "@veryfront/routing": "./src/routing/index.ts",
        "@veryfront/security": "./src/security/index.ts",
        "@veryfront/server": "./src/server/index.ts",
        "@veryfront/tool": "./src/tool/index.ts",
        "@veryfront/tool/schema": "./src/tool/schema/index.ts",
        "@veryfront/transforms": "./src/transforms/index.ts",
        "@veryfront/types": "./src/types/index.ts",
        "@veryfront/utils": "./src/utils/index.ts",
        "@veryfront/workflow": "./src/workflow/index.ts",
        "@veryfront/workflow/react": "./src/workflow/react/index.ts",
        "@veryfront/testing": "./src/testing/index.ts",
        "@veryfront/testing/assert": "./src/testing/assert.ts",
        "@veryfront/testing/bdd": "./src/testing/bdd.ts",
        "@veryfront/testing/deno-compat": "./src/testing/deno-compat.ts",
        "@veryfront/testing/utils": "./src/testing/utils.ts",
        "std/": "https://deno.land/std@0.220.0/",
        "#std/path": "https://deno.land/std@0.220.0/path/mod.ts",
        "#std/path.ts": "https://deno.land/std@0.220.0/path/mod.ts",
        "#std/assert": "https://deno.land/std@0.220.0/assert/mod.ts",
        "#std/assert.ts": "https://deno.land/std@0.220.0/assert/mod.ts",
        "#std/dotenv": "https://deno.land/std@0.220.0/dotenv/mod.ts",
        "#std/dotenv.ts": "https://deno.land/std@0.220.0/dotenv/mod.ts",
        "#std/flags": "https://deno.land/std@0.220.0/flags/mod.ts",
        "#std/flags.ts": "https://deno.land/std@0.220.0/flags/mod.ts",
        "#std/fmt/colors": "https://deno.land/std@0.220.0/fmt/colors.ts",
        "#std/fmt/colors.ts": "https://deno.land/std@0.220.0/fmt/colors.ts",
        "#std/testing": "https://deno.land/std@0.220.0/testing/mod.ts",
        "#std/testing.ts": "https://deno.land/std@0.220.0/testing/mod.ts",
        "#std/testing/bdd": "https://deno.land/std@0.220.0/testing/bdd.ts",
        "#std/testing/bdd.ts": "https://deno.land/std@0.220.0/testing/bdd.ts",
        "#std/expect": "https://deno.land/std@0.220.0/expect/mod.ts",
        "#std/expect.ts": "https://deno.land/std@0.220.0/expect/mod.ts",
        "#std/fs": "https://deno.land/std@0.220.0/fs/mod.ts",
        "#std/fs.ts": "https://deno.land/std@0.220.0/fs/mod.ts",
        "#std/async": "https://deno.land/std@0.220.0/async/mod.ts",
        "#std/async.ts": "https://deno.land/std@0.220.0/async/mod.ts",
        "#std/front-matter/yaml": "./src/platform/compat/std/front-matter-yaml.ts",
        "#std/front-matter/yaml.ts": "./src/platform/compat/std/front-matter-yaml.ts",
        "@std/path": "https://deno.land/std@0.220.0/path/mod.ts",
        "@std/assert": "https://deno.land/std@0.220.0/assert/mod.ts",
        "@std/dotenv": "https://deno.land/std@0.220.0/dotenv/mod.ts",
        "@std/flags": "https://deno.land/std@0.220.0/flags/mod.ts",
        "@std/fmt/colors": "https://deno.land/std@0.220.0/fmt/colors.ts",
        "@std/testing": "https://deno.land/std@0.220.0/testing/mod.ts",
        "@std/testing/bdd": "https://deno.land/std@0.220.0/testing/bdd.ts",
        "@std/expect": "https://deno.land/std@0.220.0/expect/mod.ts",
        "@std/fs": "https://deno.land/std@0.220.0/fs/mod.ts",
        "@std/async": "https://deno.land/std@0.220.0/async/mod.ts",
        "@std/front-matter/yaml": "./src/platform/compat/std/front-matter-yaml.ts",
        "csstype": "https://esm.sh/csstype@3.2.3",
        "@types/react": "https://esm.sh/@types/react@18.3.27?deps=csstype@3.2.3",
        "@types/react-dom": "https://esm.sh/@types/react-dom@18.3.7?deps=csstype@3.2.3",
        "react": "https://esm.sh/react@19.1.1?target=es2022&deps=csstype@3.2.3",
        "react-dom": "https://esm.sh/react-dom@19.1.1?external=react&target=es2022&deps=csstype@3.2.3",
        "react-dom/server": "https://esm.sh/react-dom@19.1.1/server?external=react&target=es2022&deps=csstype@3.2.3",
        "react-dom/client": "https://esm.sh/react-dom@19.1.1/client?external=react&target=es2022&deps=csstype@3.2.3",
        "react/jsx-runtime": "https://esm.sh/react@19.1.1/jsx-runtime?external=react&target=es2022&deps=csstype@3.2.3",
        "react/jsx-dev-runtime": "https://esm.sh/react@19.1.1/jsx-dev-runtime?external=react&target=es2022&deps=csstype@3.2.3",
        "@mdx-js/mdx": "npm:@mdx-js/mdx@3.0.0",
        "@mdx-js/react": "npm:@mdx-js/react@3.0.0",
        "unist-util-visit": "npm:unist-util-visit@5.0.0",
        "mdast-util-to-string": "npm:mdast-util-to-string@4.0.0",
        "github-slugger": "npm:github-slugger@2.0.0",
        "remark-parse": "npm:remark-parse@11.0.0",
        "remark-gfm": "npm:remark-gfm@4.0.1",
        "remark-frontmatter": "npm:remark-frontmatter@5.0.0",
        "remark-rehype": "npm:remark-rehype@11.1.1",
        "rehype-highlight": "npm:rehype-highlight@7.0.2",
        "rehype-starry-night": "npm:rehype-starry-night@2.2.0",
        "rehype-slug": "npm:rehype-slug@6.0.0",
        "rehype-stringify": "npm:rehype-stringify@10.0.1",
        "esbuild": "npm:esbuild@0.20.2",
        "esbuild/mod.js": "npm:esbuild@0.20.2",
        "es-module-lexer": "npm:es-module-lexer@1.5.0",
        "gray-matter": "npm:gray-matter@4.0.3",
        "zod": "npm:zod@3.25.76",
        "mime-types": "npm:mime-types@2.1.35",
        "mdast": "npm:@types/mdast@4.0.3",
        "hast": "npm:@types/hast@3.0.3",
        "unist": "npm:@types/unist@3.0.2",
        "unified": "npm:unified@11.0.5",
        "vfile": "npm:vfile@6.0.1",
        "ai": "npm:ai@6.0.33",
        "ai/react": "npm:@ai-sdk/react@3.0.35",
        "@ai-sdk/react": "npm:@ai-sdk/react@3.0.35",
        "@ai-sdk/openai": "https://esm.sh/@ai-sdk/openai@2.0.1",
        "@ai-sdk/anthropic": "https://esm.sh/@ai-sdk/anthropic@2.0.1",
        "tailwindcss": "https://esm.sh/tailwindcss@4.1.8",
        "redis": "npm:redis",
        "pg": "npm:pg",
        "@opentelemetry/api": "npm:@opentelemetry/api@1",
        "@opentelemetry/core": "npm:@opentelemetry/core@1",
        "@opentelemetry/context-async-hooks": "npm:@opentelemetry/context-async-hooks@1",
        "@opentelemetry/sdk-trace-base": "npm:@opentelemetry/sdk-trace-base@1",
        "@opentelemetry/exporter-trace-otlp-http": "npm:@opentelemetry/exporter-trace-otlp-http@0.57",
        "@opentelemetry/resources": "npm:@opentelemetry/resources@1",
        "@opentelemetry/semantic-conventions": "npm:@opentelemetry/semantic-conventions@1",
        "@babel/parser": "npm:@babel/parser@7.26.3",
        "@babel/traverse": "npm:@babel/traverse@7.26.3",
        "@babel/generator": "npm:@babel/generator@7.26.3",
        "@babel/types": "npm:@babel/types@7.26.3"
    },
    "compilerOptions": {
        "jsx": "react-jsx",
        "jsxImportSource": "react",
        "strict": true,
        "noImplicitAny": true,
        "noUncheckedIndexedAccess": true,
        "types": [
            "npm:@types/react@18.3.27"
        ],
        "lib": [
            "deno.window",
            "dom",
            "dom.iterable",
            "dom.asynciterable",
            "deno.ns"
        ]
    },
    "tasks": {
        "setup": "deno run --allow-all scripts/setup.ts",
        "start": "rm -rf .cache/ && deno run --allow-all scripts/server.ts",
        "start:headless": "deno run --allow-all scripts/server.ts --headless",
        "proxy": "deno run --allow-net --allow-env --allow-read proxy/main.ts",
        "renderer": "deno run --allow-all --unstable-net --unstable-worker-options src/cli/main.ts dev",
        "build": "deno run -A scripts/generate-templates-manifest.ts && deno compile --allow-all --output ../../bin/veryfront src/cli/main.ts",
        "build:npm": "deno run -A scripts/build-npm-dnt.ts",
        "release": "deno run -A scripts/release.ts",
        "test": "VF_DISABLE_LRU_INTERVAL=1 NODE_ENV=production LOG_FORMAT=text deno test --no-check --parallel --allow-all --unstable-worker-options --unstable-net",
        "test:unit": "VF_DISABLE_LRU_INTERVAL=1 NODE_ENV=production LOG_FORMAT=text deno test --no-check --parallel --allow-all --v8-flags=--max-old-space-size=8192 '--ignore=tests,src/ai/workflow/__tests__,src/cli/commands/*.integration.test.ts' --unstable-worker-options --unstable-net",
        "test:integration": "VF_DISABLE_LRU_INTERVAL=1 NODE_ENV=production LOG_FORMAT=text deno test --no-check --parallel --allow-all tests --unstable-worker-options --unstable-net",
        "test:integration:cli": "VF_DISABLE_LRU_INTERVAL=1 NODE_ENV=production LOG_FORMAT=text deno test --no-check --parallel --allow-all src/cli/commands/*.integration.test.ts --unstable-worker-options --unstable-net",
        "test:record": "VCR=record deno test --no-check --allow-all src/cli/commands/*.integration.test.ts",
        "test:coverage": "rm -rf coverage && VF_DISABLE_LRU_INTERVAL=1 NODE_ENV=production LOG_FORMAT=text deno test --no-check --parallel --fail-fast --allow-all --coverage=coverage --unstable-worker-options --unstable-net || exit 1",
        "test:coverage:unit": "rm -rf coverage && VF_DISABLE_LRU_INTERVAL=1 NODE_ENV=production LOG_FORMAT=text deno test --no-check --parallel --fail-fast --allow-all --coverage=coverage '--ignore=tests,src/ai/workflow/__tests__,src/cli/commands/*.integration.test.ts' --unstable-worker-options --unstable-net || exit 1",
        "test:coverage:integration": "rm -rf coverage && VF_DISABLE_LRU_INTERVAL=1 NODE_ENV=production LOG_FORMAT=text deno test --no-check --parallel --fail-fast --allow-all --coverage=coverage tests --unstable-worker-options --unstable-net || exit 1",
        "coverage:report": "deno coverage coverage --include=src/ --exclude=tests --exclude=src/**/*_test.ts --exclude=src/**/*_test.tsx --exclude=src/**/*.test.ts --exclude=src/**/*.test.tsx --lcov > coverage/lcov.info && deno run --allow-read scripts/check-coverage.ts 80",
        "coverage:html": "deno coverage coverage --include=src/ --exclude=tests --exclude=src/**/*_test.ts --exclude=src/**/*_test.tsx --exclude=src/**/*.test.ts --exclude=src/**/*.test.tsx --html",
        "clean": "rm -rf .cache/",
        "lint": "DENO_NO_PACKAGE_JSON=1 deno lint src/",
        "fmt": "deno fmt src/",
        "fmt:check": "deno fmt --check src/",
        "typecheck": "deno check src/index.ts src/cli/main.ts src/server/index.ts src/routing/api/index.ts src/rendering/index.ts src/platform/index.ts src/platform/adapters/index.ts src/build/index.ts src/build/production-build/index.ts src/transforms/index.ts src/config/index.ts src/utils/index.ts src/data/index.ts src/security/index.ts src/middleware/index.ts src/server/handlers/dev/index.ts src/server/handlers/request/api/index.ts src/rendering/cache/index.ts src/rendering/cache/stores/index.ts src/rendering/rsc/actions/index.ts src/html/index.ts src/modules/index.ts proxy/main.ts scripts/server.ts",
        "verify": "deno fmt --check src/ && DENO_NO_PACKAGE_JSON=1 deno lint src/ && deno task typecheck && deno task test",
        "verify:quick": "deno fmt --check src/ && DENO_NO_PACKAGE_JSON=1 deno lint src/ && deno task typecheck",
        "docs:check-links": "deno run -A scripts/check-doc-links.ts",
        "lint:ban-console": "deno run --allow-read scripts/ban-console.ts",
        "lint:ban-deep-imports": "deno run --allow-read scripts/ban-deep-imports.ts",
        "lint:ban-internal-root-imports": "deno run --allow-read scripts/ban-internal-root-imports.ts",
        "lint:check-awaits": "deno run --allow-read scripts/check-unawaited-promises.ts",
        "lint:platform": "deno run --allow-read scripts/lint-platform-agnostic.ts",
        "test:smoke": "deno run --allow-all src/platform/compat/smoke-test.ts",
        "test:cross-runtime": "deno run --allow-all src/platform/compat/cross-runtime.test.ts",
        "test:node": "node ./scripts/run-node-tests.mjs 'src/**/*.test.ts'",
        "test:bun": "node ./scripts/run-bun-tests.mjs src/",
        "test:all-runtimes": "deno task test:unit && deno task test:node && deno task test:bun",
        "test:e2e": "npx playwright test --config=tests/e2e/playwright.config.ts",
        "check:circular": "deno run -A jsr:@cunarist/deno-circular-deps src/index.ts",
        "cli": "deno run --allow-all src/cli/main.ts",
        "mcp": "deno run --allow-all src/cli/main.ts mcp",
        "batch:estimate": "deno run -A scripts/batch-simplify/batch-simplify.ts estimate",
        "batch:prepare": "deno run -A scripts/batch-simplify/batch-simplify.ts prepare",
        "batch:submit": "deno run -A scripts/batch-simplify/batch-simplify.ts submit",
        "batch:status": "deno run -A scripts/batch-simplify/batch-simplify.ts status",
        "batch:errors": "deno run -A scripts/batch-simplify/batch-simplify.ts errors",
        "batch:download": "deno run -A scripts/batch-simplify/batch-simplify.ts download",
        "batch:apply": "deno run -A scripts/batch-simplify/batch-simplify.ts apply",
        "rlm:example": "deno run -A scripts/rlm-ts/examples/basic.ts",
        "rlm:explore": "deno run -A scripts/rlm-ts/apps/explore.ts",
        "rlm:audit": "deno run -A scripts/rlm-ts/apps/audit.ts"
    },
    "lint": {
        "include": [
            "src/**/*.ts",
            "src/**/*.tsx"
        ],
        "exclude": [
            "dist/",
            "coverage/"
        ],
        "rules": {
            "tags": [
                "recommended"
            ],
            "include": [
                "ban-untagged-todo"
            ],
            "exclude": [
                "no-explicit-any",
                "no-process-global",
                "no-console"
            ]
        }
    },
    "fmt": {
        "include": [
            "src/**/*.ts",
            "src/**/*.tsx"
        ],
        "exclude": [
            "dist/",
            "coverage/"
        ],
        "useTabs": false,
        "lineWidth": 100,
        "indentWidth": 2,
        "semiColons": true,
        "singleQuote": false,
        "proseWrap": "preserve"
    }
};

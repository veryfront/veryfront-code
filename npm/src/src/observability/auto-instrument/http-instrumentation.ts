...
const headersGetter = {
  keys(carrier: dntShim.Headers): string[] {
    return [...carrier.keys()];
  },
  get(carrier: dntShim.Headers, key: string): string | undefined {
    return carrier.get(key) ?? undefined;
  },
};

function extractParentContext(headers: dntShim.Headers) {
  try {
...
export function instrumentHttpHandler(
  ...
  return async function instrumentedHttpHandler(request: dntShim.Request): Promise<dntShim.Response> {
...
export function createInstrumentedFetch(
  baseFetch: typeof dntShim.fetch = dntShim.dntGlobalThis.fetch,
): typeof dntShim.fetch {
...
function buildHttpAttributes(request: dntShim.Request, url: URL): HttpAttributes {
  return {

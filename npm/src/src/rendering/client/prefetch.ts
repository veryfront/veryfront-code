...
import { type ResourceHint, ResourceHintsManager } from "./prefetch/resource-hints.js";
import {
...
export class PrefetchManager {
  ...
  private async prefetchPageResources(response: dntShim.Response, _pageUrl: string): Promise<void> {
...
export function initPrefetch(options?: PrefetchOptions): PrefetchManager {
  ...
  (dntShim.dntGlobalThis as GlobalWithPrefetch).veryFrontPrefetch = prefetchManager;
  return prefetchManager;
}

function resolveAutoInitOptions(): PrefetchOptions | null {
  const setting = (dntShim.dntGlobalThis as GlobalWithPrefetch).__VERYFRONT_PREFETCH__;
...
function shouldAutoInitPrefetch(options: PrefetchOptions | null): options is PrefetchOptions {
  ...
  if (typeof dntShim.dntGlobalThis === "undefined" || typeof document === "undefined") return false;

  const win = dntShim.dntGlobalThis as unknown as { __veryfrontSSRStub?: boolean };

...
function encodeBase64(value: string): string {
  ...
  const bufferCtor = (dntShim.dntGlobalThis as { Buffer?: typeof Buffer }).Buffer;
...
export class AuthHandler extends BaseHandler {
  ...
  private loadAuthConfig(ctx: HandlerContext): void {
    ...
    const isTestEnv = (dntShim.dntGlobalThis as Record<string, unknown>).__vfTestEnv === true;
    if (isTestEnv) return;
  ...
  private checkBasicAuth(req: dntShim.Request): HandlerResult | null {

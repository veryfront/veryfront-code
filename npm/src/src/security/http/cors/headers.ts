...
import type { CORSConfig, CORSHeaderOptions, CORSValidationResult } from "./types.js";
...
export function applyCORSHeaders(options: CORSHeaderOptions): Promise<dntShim.Response | void> {
...
export function applyCORSHeadersSync(options: CORSHeaderOptions): dntShim.Response | void {
  const validation = validateOriginSync(options.request.headers.get("origin"), options.config);
  return applyValidatedHeaders(validation, options);
}

export function shouldApplyCORS(request: dntShim.Request, config?: boolean | CORSConfig): boolean {
  if (!config) {
    return false;
  }

...
import type { CORSPreflightOptions } from "./types.js";
import { validateOrigin } from "./validators.js";
import {
  DEFAULT_HEADERS,
  DEFAULT_MAX_AGE,
  DEFAULT_METHODS,
...
export function handleCORSPreflight(options: CORSPreflightOptions): Promise<dntShim.Response> {
  return withSpan(
    ...
    async () => {
      ...
      if (!validation.allowedOrigin) {
        if (!config) {
          return new dntShim.Response(null, { status: HTTP_NO_CONTENT });
        }
        ...
        return new dntShim.Response(errorMessage, {
...
export function isPreflightRequest(request: dntShim.Request): boolean {
  return (

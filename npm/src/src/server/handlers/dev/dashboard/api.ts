...
import { WorkflowClient, workflowRegistry } from "../../../../workflow/index.js";
...
import { ERROR_CATALOG } from "../../../../errors/catalog/index.js";
...
function errorResponse(message: string, status = 500): dntShim.Response {
...
function handleStats(): dntShim.Response {
...
function handleListTools(): dntShim.Response {
...
function handleListResources(): dntShim.Response {
...
function handleListPrompts(): dntShim.Response {
...
function handleListAgents(): dntShim.Response {
...
function handleListWorkflows(): dntShim.Response {
...
async function handleExecuteTool(req: dntShim.Request): Promise<dntShim.Response> {
  try {
...
async function handleReadResource(req: dntShim.Request): Promise<dntShim.Response> {
  try {
...
async function handleRenderPrompt(req: dntShim.Request): Promise<dntShim.Response> {
  try {
...
async function handleStartWorkflow(req: dntShim.Request): Promise<dntShim.Response> {
  try {
    ...
    const result = await Promise.race([
      ...
      new Promise((_, reject) =>
        dntShim.setTimeout(() => reject(new Error("Workflow execution timed out (30s)")), timeoutMs)
      ),
    ]);
...
function handleListHandlers(ctx: HandlerContext): dntShim.Response {
...
function handleGetMetrics(): dntShim.Response {
  try {
...
async function handleListFiles(req: dntShim.Request, ctx: HandlerContext): Promise<dntShim.Response> {
...
async function handleReadFileContent(req: dntShim.Request, ctx: HandlerContext): Promise<dntShim.Response> {
  const { adapter, projectDir } = ctx;
  if (!adapter?.fs) return errorResponse("No file adapter available", 500);
  if (!projectDir) return errorResponse("No project directory configured", 500);
...
function handleGetInfrastructure(): dntShim.Response {
...
function handleGetMemory(): dntShim.Response {
  return jsonResponse({
    heap: getHeapStats(),
...
function handleGetBuild(): dntShim.Response {
...
function handleGetErrors(): dntShim.Response {
...
function handleLiveErrors(req: dntShim.Request): dntShim.Response {
  const url = new URL(req.url);
  const type = url.searchParams.get("type") ?? undefined;
  const collector = getErrorCollector();

  const filter = type
    ? { type: type as import("../../../../cli/mcp/error-collector.js").ErrorType }
    : undefined;
...
function handleLiveLogs(req: dntShim.Request): dntShim.Response {
  const url = new URL(req.url);
  ...
  const entries = buffer.query({
    level: level as import("../../../../cli/mcp/log-buffer.js").LogLevel | undefined,
...
async function handleHmrTrigger(req: dntShim.Request): Promise<dntShim.Response> {
  try {
...
function handleGetConfig(ctx: HandlerContext): dntShim.Response {
  const featureFlags = [
    {
      name: "RSC_ENABLED",
      value: isRSCEnabled(),

...
import {
  ...
  withContext,
  withSpan,
} from "../../observability/tracing/otlp-setup.js";
...
export function createVeryfrontHandler(
...
): ((req: dntShim.Request) => Promise<dntShim.Response>) & { ready?: Promise<void> } {
  ...
  if (isProxyMode) {
    logger.debug(
      "[universal] Running in proxy mode - lazy initialization enabled",
    );
  }

  const handler = async (req: dntShim.Request): Promise<dntShim.Response> => {
    ...
    return runWithRequestContextAsync(loggerContext, async () => {
      ...
      if (!isolationCheck.allowed) {
        ...
        const response = new dntShim.Response(
          JSON.stringify({
            error: message,
      ...
      try {
        ...
        const executeHandler = async (): Promise<dntShim.Response> => {
          ...
          if (
            ...
            const { PROJECTS_SHELL_HTML } = await import(
              "../handlers/dev/projects/html-shell.js"
            );
            const { handleProjectsAPI } = await import(
              "../handlers/dev/projects/api.js"
            );
            const { handleProjectsUI } = await import(
              "../handlers/dev/projects/ui-handler.js"
            );

...
import type { Agent, AgentConfig, AgentResponse, Message, ToolCall as _ToolCall } from "./types.js";
...
function createAgentStreamResult(stream: ReadableStream<Uint8Array>): AgentStreamResult {
  return {
    toDataStreamResponse(options): dntShim.Response {
...
export function agent(config: AgentConfig): Agent {
  ...
  const agentInstance: Agent = {
    ...
    respond(request): Promise<dntShim.Response> {
      return withSpan(

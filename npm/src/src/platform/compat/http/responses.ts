...
function withCorrelationId(headers: dntShim.Headers, options?: ResponseOptions): void {
...
export function notFound(message?: string, options?: ResponseOptions): dntShim.Response {
  return errorResponse(HttpStatus.NOT_FOUND, message, options);
}

export function badRequest(message?: string, options?: ResponseOptions): dntShim.Response {
  return errorResponse(HttpStatus.BAD_REQUEST, message, options);
}

export function unauthorized(message?: string, options?: ResponseOptions): dntShim.Response {
  return errorResponse(HttpStatus.UNAUTHORIZED, message, options);
}

export function forbidden(message?: string, options?: ResponseOptions): dntShim.Response {
  return errorResponse(HttpStatus.FORBIDDEN, message, options);
}

export function internalServerError(message?: string, options?: ResponseOptions): dntShim.Response {
  return errorResponse(HttpStatus.INTERNAL_SERVER_ERROR, message, options);
}

export function badGateway(message?: string, options?: ResponseOptions): dntShim.Response {
  return errorResponse(HttpStatus.BAD_GATEWAY, message, options);
}

export function serviceUnavailable(message?: string, options?: ResponseOptions): dntShim.Response {
  return errorResponse(HttpStatus.SERVICE_UNAVAILABLE, message, options);
}

export function methodNotAllowed(allowed: string[], options?: ResponseOptions): dntShim.Response {
...
export function ok<T>(data?: T, options?: ResponseOptions): dntShim.Response {
  if (data === undefined) return new dntShim.Response(null, { status: HttpStatus.OK, ...options });
  return jsonResponse(data, HttpStatus.OK, options);
}

export function created<T>(data?: T, location?: string, options?: ResponseOptions): dntShim.Response {
  const headers = new dntShim.Headers(options?.headers);
...
export function noContent(options?: ResponseOptions): dntShim.Response {

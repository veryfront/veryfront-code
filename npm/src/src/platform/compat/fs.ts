...
class DenoFileSystem implements FileSystem {
  ...
  async exists(path: string): Promise<boolean> {
    try {
      ...
      // @ts-ignore - Deno global
      if (error instanceof dntShim.Deno.errors.NotFound) return false;
      throw error;
    }
  }
...
export function isNotFoundError(error: unknown): boolean {
  if (isDeno && error instanceof (dntShim.dntGlobalThis as any).Deno.errors.NotFound) return true;
  return (error as NodeJS.ErrnoException)?.code === "ENOENT";
}

export function isAlreadyExistsError(error: unknown): boolean {
  if (isDeno && error instanceof (dntShim.dntGlobalThis as any).Deno.errors.AlreadyExists) return true;

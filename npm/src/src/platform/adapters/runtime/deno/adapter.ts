...
function toSnapshotEntry(info: dntShim.Deno.FileInfo): FileSnapshotEntry {
  return {
...
async function collectPathSnapshot(
  ...
  let info: dntShim.Deno.FileInfo;
  try {
...
class DenoFileSystemAdapter implements FileSystemAdapter {
  ...
  async exists(path: string): Promise<boolean> {
    if (typeof dntShim.Deno === "undefined") return false;
    try {
...
class DenoServer implements Server {
  constructor(
    private server: dntShim.Deno.HttpServer,
...
export class DenoAdapter implements RuntimeAdapter {
  ...
  serve(
    ...
    if (envOverlay && envStore) {
      wrappedHandler = (request: dntShim.Request) => {

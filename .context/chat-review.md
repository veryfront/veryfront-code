...
- ✅ **ChatSidebar** — DONE. Slimmer rows (`px-2.5 py-1.5`, title-first, dropped the chunky leading icon), a per-row **⋯ context menu** (portalled `DropdownMenu` → **Rename** (pencil) + **Delete** (trash, destructive)) replacing the double-click rename + hover-trash, and tighter group spacing (`space-y-0.5` rows / `space-y-3` groups, `py-1` headers). Rename opens inline edit. Verified in Storybook (screenshot).

...
export interface ErrorCreateOptions {
  ...
  instance?: string;
  context?: unknown;
  /** Override the definition's default HTTP status. */
  status?: number;
}
...
/** Options for the {@link VeryfrontError} constructor. */
...
type VeryfrontErrorDataSnapshot = Omit<VeryfrontErrorSnapshot, "stack">;
...
export class VeryfrontError extends Error {
  ...
  toRFC9457(): RFC9457Response {
    const snapshot = snapshotKnownVeryfrontErrorData(this);
    if (!snapshot) {
      return {
...
/**
 * Snapshot the data properties that do not require runtime-specific stack
 * descriptor inspection.
 *
 * @internal
 */
export function snapshotKnownVeryfrontErrorData(
  error: VeryfrontError,
): VeryfrontErrorDataSnapshot | null {
  try {

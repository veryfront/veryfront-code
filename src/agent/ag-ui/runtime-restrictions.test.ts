...
describe("agent/ag-ui/runtime-restrictions", () => {
  ...
  it("preserves an explicitly configured MCP source behind the allowlist", () => {
    const mcpServer = {
      id: "docs",
    ...
    const restricted = applyAgUiRuntimeRestrictions(
      createConfig({
        tools: { search_docs: true, delete_project: true },
  ...
  it("preserves an explicitly allowlisted first-party MCP candidate", () => {
    const restricted = applyAgUiRuntimeRestrictions(
      createConfig({
        tools: true,
        providerTools: undefined,
        mcpServers: [{
          kind: "veryfront-api",
          toolPolicy: { allow: ["get_file", "delete_project"] },
        }],
      }),
  ...
  it("keeps an MCP fallback when the request provider does not support the colliding native tool", () => {

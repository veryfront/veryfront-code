...
export function createLifecycleAgUiBrowserAdapter(input: {
  ...
  let activeTextContentId: string | null = null;
  let nextTextContentIndex = 0;
  const startTextContent = (id?: string): string => {
    const contentId = id || `text:${nextTextContentIndex}`;
    nextTextContentIndex += 1;
    activeTextContentId = contentId;
    return contentId;
  };
  const continueTextContent = (id?: string): string => {
    if (id) activeTextContentId = id;
    return id || activeTextContentId || startTextContent();
  };
  const endTextContent = (id?: string): string => {
    const contentId = id || activeTextContentId || startTextContent();
      ...
      case "text_start":
        ...
        return [{
          ...
          payload: {
            ...
            contentId: startTextContent(event.id),
            role: "assistant",
          },
        }];
      case "text_content":
        ...
        return [{
          ...
          payload: {
            messageId: state.messageId,
            contentId: continueTextContent(event.id),
            delta: event.delta,
          },
        }];
      case "text_end":
        return [{
          ...
          payload: {
            messageId: state.messageId,
            contentId: endTextContent(event.id),
          },
        }];

...
function scoreToolExposureTerms(
  ...
  const selectivityFloor = weightedTerms.reduce((sum, term) => sum + term.documentFrequency, 0) /
  ...
  for (const candidate of candidates) {
    ...
    for (const { term, documentFrequency, inverseDocumentFrequency } of weightedTerms) {
      ...
      if (documentFrequency <= selectivityFloor) matchedSelectiveTerm = true;
    }
    // Selectivity suppresses filler relative to the query itself, not to a fixed
    // slice of the catalog. A candidate matching every term is not filler
    // however common those terms are, so the floor only guards partial matches.

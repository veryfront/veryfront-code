...
describe("stream lifecycle live adapter", () => {
  ...
  it("maps reasoning, provider output, usage, and diagnostics", () => {
    ...
    assertEquals(
      frames([{
        type: "provider_tool_result",
        toolCallId: "native-structured-error",
        toolName: "web_search",
        output: { error: "normalized provider error" },
        isError: true,
        providerExecuted: true,
        dynamic: true,
      }]).flatMap((frame) => adapter.encode(frame)),
      [{
        type: "tool-output-error",
        toolCallId: "native-structured-error",
        errorText: '{"error":"normalized provider error"}',
        providerExecuted: true,
        dynamic: true,
      }],
    );
    assertEquals(
      frames([{
        type: "provider_tool_result",
        toolCallId: "native-error-instance",
        toolName: "web_search",
        output: new Error("provider timeout"),
        isError: true,
        providerExecuted: true,
      }]).flatMap((frame) => adapter.encode(frame)),
      [{

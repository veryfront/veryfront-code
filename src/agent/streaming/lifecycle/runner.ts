    ...
    try {
      ...
      try {
        ...
        settleProviderFailure(
          ...
          reducer,
          error,
          classifyProviderFailure(input.provider, error, reducer.snapshot),
          elapsedMs(),
        );
        return;
      }
      ...
      while (!outcome.settled) {
        if (pendingRead === null) {
          let read: Promise<IteratorResult<TProviderPart>>;
          try {
            read = providerIterator.next();
          } catch (error) {
            settleProviderFailure(
              outcome,
              reducer,
          ...
          pendingRead = trackProviderRead(read, policy.clock);
        }
        ...
        if (raced.kind === "read_error") {
          settleProviderFailure(
            ...
            reducer,
            raced.error,
            classifyProviderFailure(input.provider, raced.error, reducer.snapshot),
            elapsedMs(),
          );
          return;
        }
        ...
        let signals: readonly StreamProviderSignal[];
        try {
          signals = input.provider.decode(next.value, reducer.snapshot);
        } catch (error) {
          settleProviderFailure(
            outcome,
            reducer,
...
function classifyProviderFailure<TProviderPart>(
  provider: StreamProviderAdapter<TProviderPart>,
  error: unknown,
  snapshot: Parameters<StreamProviderAdapter<TProviderPart>["classifyError"]>[1],
): StreamProviderError {
  try {
    return provider.classifyError(error, snapshot);
  } catch {
    return {

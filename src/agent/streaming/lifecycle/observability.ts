...
const SHADOW_DIVERGENCE_CATEGORY_ALLOWLIST = new Set([
  "text",
  "reasoning",
  "tool_input",
  "tool_result",
...
function normalizeShadowDivergenceCategory(category: string): string {
  return SHADOW_DIVERGENCE_CATEGORY_ALLOWLIST.has(category) ? category : "other";
}
...
export function recordStreamLifecycleShadowReport(input: {
  ...
  const categories = new Set(
    input.report.categories.map(normalizeShadowDivergenceCategory),
  );

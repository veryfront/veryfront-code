...
describe("agent/fork-runtime-stream", () => {
  ...
  it("preserves trusted platform tools in restricted child forks without trusting prefix claims", async () => {
    let executions = 0;
    ...
    const { streamResult, forkToolNames } = startAgentRuntimeForkWithHostTools({
      ...
      runStep: async (input) => {
        ...
        if (shell && typeof shell !== "boolean") await shell.execute({}, { toolCallId: "shell" });
        return {
          stream: createRuntimeEventStream([{ type: "text-delta", delta: "Done." }]),

...
export interface RuntimePreparationSourceSnapshot {
  description: unknown;
...
/** Framework-owned source/lifetime adapter, never received from the execution protocol. */
export interface RuntimePreparationSource {
  readonly signal: AbortSignal;
  prepare(
    agentId: string,
    context: ExecutorOperationContext,
  ): Promise<RuntimePreparationSourceSnapshot>;
  instantiate: typeof createPreparedHostedRuntimeAgent;
  retainTask(task: Promise<unknown>): void;
  close(): Promise<void>;
}
export interface RuntimePreparationCoreOptions {
  binding: ExecutorBinding;
  source: ExecutorDiscoverySource;
  project: RuntimePreparationSource;
  grant?: ExecutorRuntimePreparationGrant;
  facades: ExecutorRuntimeFacades;
  projectTools?: RemoteToolSource;
}
 ...
 */
export function createRuntimePreparationCore(input: RuntimePreparationCoreOptions) {
  ...
  const assertActive = () => {
    if (lifetime.signal.aborted || input.project.signal.aborted) {
  ...
  function close(): Promise<void> {
    ...
    closing = chain(resolvePrivatePromise(), async () => {
      ...
      try {
        await observePrivatePromise(input.project.close());
      } catch {
        failed = true;
      }
    ...
    apply(removeEventListener, input.project.signal, ["abort", onDiscoveryAbort]);
    return closing;
  }
  const onDiscoveryAbort = () => {
    void chain(close(), () => {}, () => {});
  };
  apply(addEventListener, input.project.signal, ["abort", onDiscoveryAbort, { once: true }]);
  if (input.project.signal.aborted) onDiscoveryAbort();
  ...
  async function prepare(
    ...
    try {
      ...
      const preparedSource = await observePrivatePromise(
        input.project.prepare(request.agentId, context),
      );
      const described = parseDiscoveryData(
        getExecutorAgentDescribeResultSchema(),
        preparedSource.description,
        true,
      );
      if (
      ...
      input.project.retainTask(preparation!);
      let localTools: HostToolSet = preparedSource.localTools;
      ...
      const normalizeGrantedToolNames = (names: readonly string[]) => [
        ...resolveOwnerScopedToolNames({
          toolNames: names,
      ...
      const normalizeToolNames = (names: readonly string[]) =>
        normalizeGrantedToolNames(
          preparedSource.toolAliases
            ? mapPrivateArray(names, (name) => preparedSource.toolAliases!.get(name) ?? name)
            : names,
        );
      ...
      const runtimeOptions: NonNullable<Parameters<typeof createPreparedHostedRuntimeAgent>[1]> = {
        ...
        onStreamCompletion: (completion) => {
          ...
          input.project.retainTask(completion);
        },
      };
      ...
      preparedOperations = createExecutorAgentOperations({
        ...
        startStream: (streamInput) => {
          ...
          input.project.retainTask(startup);
          return startup;
        },
        cleanup: release,
      });

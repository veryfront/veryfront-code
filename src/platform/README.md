...
| `readFileSnapshotWithinLimit(path, containmentRoot, byteLimit)` | Returns one verified regular-file generation beneath the canonical root. Native adapters expose this optional capability only when the runtime supports no-follow opens; it is omitted on Windows. Supported adapters reject terminal symlinks, containment escapes, identity changes, and generation changes. |

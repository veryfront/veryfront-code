...
| `readFileSnapshotWithinLimit(path, containmentRoot, byteLimit)` | Returns one verified regular-file generation beneath the canonical root. POSIX adapters use no-follow opens. Windows adapters bind the read to an opened handle and verify its native identity and generation against the pathname before and after the bounded read. Both modes reject terminal links, containment escapes, missing or ambiguous native identities, identity changes, and generation changes. |

...
describe("platform/adapters/file-system-capabilities", () => {
  ...
  it("validates every virtual generation result as a positive safe integer", async () => {
    let generation: unknown = 1;
    ...
    for (const invalid of [0, -1, 1.5, Number.MAX_SAFE_INTEGER + 1, undefined, "1"]) {
      generation = invalid;
      await assertRejects(

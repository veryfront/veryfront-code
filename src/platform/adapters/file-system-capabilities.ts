...
function positiveSafeInteger(value: unknown, label: string): number {
  if (!numberIsSafeInteger(value) || (value as number) <= 0) {
    throw invalidCapability(label, "generation must be a positive safe integer");
  }
...
export function captureSnapshotReadCapability(
  ...
  captured.read = async (
    ...
    positiveSafeInteger(byteLimit, `${label} snapshot byte limit`);

...
describe("security/csrf/helpers", () => {
  describe("generateCsrfToken", () => {
    ...
    it("should keep Secure for __Secure- cookies", () => {
      const result = generateCsrfToken({ cookieName: "__Secure-vf_csrf", secure: false });
      assertEquals(result.setCookie.includes("Secure"), true);
    });
    ...
    it("should reject unsafe cookie names and invalid TTLs", () => {
      for (const cookieName of ["", "bad cookie", "bad;cookie", "bad\r\ncookie"]) {
        assertThrows(() => generateCsrfToken({ cookieName }), TypeError, "cookieName");
      }
      for (const ttlSec of [0, -1, 1.5, Number.NaN, Number.POSITIVE_INFINITY]) {
        assertThrows(() => generateCsrfToken({ ttlSec }), TypeError, "ttlSec");
      }
    });
  });
  ...
  describe("validateCsrf", () => {
    ...
    it("should reject matching tokens that were not generated by the CSRF helper", () => {
      const req = new Request("http://localhost/", {
        headers: {
          cookie: "__Host-vf_csrf=attacker-chosen",
          "x-csrf-token": "attacker-chosen",
        },
      });
    ...
    it("should reject invalid configured header names", () => {
      const req = new Request("http://localhost/");
      assertThrows(
        () => validateCsrf(req, { headerName: "bad header" }),
        TypeError,
        "headerName",
      );
    });

...
export class ManifestHandler {
  ...
  constructor(
    ...
    options?: {
      ...
      /** Legacy locality hint used only when no explicit transport strategy is supplied. */
    ...
    this.clientModuleStrategy = options?.clientModuleStrategy === "fs" ||
        (options?.clientModuleStrategy === undefined && options?.isLocalProject === true)
      ? "fs"
      : "rsc-module";
    this.fs = options?.fs;
    this.cacheKey = [
  ...
  private createResponse(data: ManifestData): Response {
    return new Response(JSON.stringify(data), {
      headers: {
        ...
        ...(data.dependencyPinningCacheKey?.startsWith("on:")
          ? { [RSC_DEPENDENCY_PINNING_HEADER]: data.dependencyPinningCacheKey }
          : {}),
      },
    });
  }

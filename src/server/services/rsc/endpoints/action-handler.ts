...
async function handleActionRequestInner(
  ...
  const [reactVersion, importMap] = await apply(promiseAll, NativePromise, [[
    resolveProjectReactVersion({
      projectDir,
  ...
  ]]) as [string, Awaited<ReturnType<typeof preloadImportMap>>];
...
function withDependencyPinningVary(response: Response): Response {
  ...
  apply(headersSet, response.headers, ["cache-control", "no-store"]);
  return response;
}

...
function assertSnapshotError(
  ...
  if (!(error instanceof ProjectEnvSnapshotError)) {
    throw new TypeError("Expected createProjectEnvSnapshot to reject the value");
  }
...
describe("server/project-env/snapshot", () => {
  ...
  it("accepts null-prototype records and pollution-prone names as inert data", () => {
    ...
    const snapshot = createProjectEnvSnapshot(input);
    assertEquals(Object.getOwnPropertyDescriptor(snapshot, "__proto__")?.value, "data");

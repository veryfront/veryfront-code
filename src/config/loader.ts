...
const log = serverLogger.component("config");
...
function getDefaultFsConfig(): VeryfrontConfig["fs"] {
  ...
  log.info("getDefaultFsConfig called", {
    proxyModeEnv,
    isProxyMode,
  ...
  if (isProxyMode && apiBaseUrl) {
    log.info("Using veryfront-api filesystem (proxy mode)");
    return {
      type: "veryfront-api",
      veryfront: {
        apiBaseUrl,
  ...
  log.info("Using local filesystem (no proxy mode)");
  return { type: "local" };
}
...
function validateAndCacheConfig(userConfig: unknown, cacheKey: string): VeryfrontConfig {
  ...
  if (merged.react?.version) {
    log.debug("React version from config", { version: merged.react.version });
  }
...
function loadConfigFromVirtualFS(
  ...
  return withSpan(
    ...
    async () => {
      log.info("Loading config from virtual filesystem (API)", { configPath });
      const content = await adapter.fs.readFile(configPath);
      const source = typeof content === "string" ? content : new TextDecoder().decode(content);
      log.info("Got config source from API", {
        configPath,
      ...
      log.info("Loaded config from virtual filesystem", {
        configPath,
...
async function loadAndMergeConfig(
  ...
  log.info("loadAndMergeConfig called", {
    configPath,
    cacheKey,
    isVirtualFS,
    isBun,
  ...
  if (isVirtualFS) {
    log.info("Using virtual filesystem (API) for config", { configPath });
  ...
  if (isBun || isDenoCompiled) {
    log.info("Using temp file import for Bun/compiled Deno", {
      configPath,
      isBun,
      isDenoCompiled,
    });
    ...
    log.info("Successfully loaded config via temp file", {
      configPath,
      hasApp: !!(userConfig as Record<string, unknown>)?.app,
      hasRouter: !!(userConfig as Record<string, unknown>)?.router,
    });
...
export function getConfig(
  ...
  log.debug("getConfig START", { projectDir, cacheKey: cacheKeyForLog });
  ...
  return withSpan(
    ...
    async () => {
      ...
      if (cached?.revision === cacheRevision) {
        log.info("Cache HIT - using cached config", {
      ...
      log.debug("Cache MISS - loading config", {
        cacheKey: effectiveCacheKey,
        isVirtualFS,
      });
      ...
      for (const configFile of configFiles) {
        ...
        log.info("Checking config file", { configPath, exists, isVirtualFS });
        if (!exists) continue;
        ...
        try {
          ...
          log.warn(`Failed to load ${configFile}, trying next:`, {

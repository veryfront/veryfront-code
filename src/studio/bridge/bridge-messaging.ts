...
/** Validate the browser source and resolve its canonical trusted Studio origin. */
export function resolveStudioEventOrigin(event: MessageEvent): string | null {
  if (!globalThis.window.parent || event.source !== globalThis.window.parent) return null;

  return resolveTrustedStudioOrigin(event.origin || "");
}

/** Commit a validated origin to this bridge session and start draining its queue. */
export function acceptStudioOrigin(trustedOrigin: string): boolean {
  if (studioOrigin) {
...
export function isFromStudio(event: MessageEvent): boolean {
  const trustedOrigin = resolveStudioEventOrigin(event);
  return trustedOrigin !== null && acceptStudioOrigin(trustedOrigin);
}

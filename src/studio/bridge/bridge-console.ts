...
function sanitizeConsoleText(value: string, maxLength = MAX_CONSOLE_STRING_LENGTH): string {
  const boundedLength = Number.isSafeInteger(maxLength) && maxLength > 0
    ? Math.min(maxLength, MAX_CONSOLE_STRING_LENGTH)
    : MAX_CONSOLE_STRING_LENGTH;
  const sourceLength = Math.min(
    value.length,
    boundedLength + SANITIZATION_LOOKAHEAD_LENGTH,
  );
  let output = sanitizeUrlCredentials(value.slice(0, sourceLength));
  ...
  if (sourceLength === value.length && output.length <= boundedLength) return output;

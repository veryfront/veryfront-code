...
async function requestScreenshot(page: Page): Promise<void> {
  await page.evaluate((origin) => {
    const preview = document.getElementById("preview");
    if (!(preview instanceof HTMLIFrameElement) || !preview.contentWindow) {
      throw new Error("Preview iframe is unavailable");
    ...
    preview.contentWindow.postMessage(
      { action: "screenshot", requestId: "browser-capture" },
      origin,
    );
  }, STUDIO_ORIGIN);
}
...
async function waitForScreenshot(page: Page): Promise<void> {
  try {
    await page.waitForFunction(
      () => {
        const messages = (globalThis as typeof globalThis & {
  ...
  () => {
    it("delivers lifecycle state and fails closed when capture is not composed", async () => {
      ...
      try {
        ...
        await waitForScreenshot(page);
        const messagesWithScreenshot = await readStudioMessages(page);
        const screenshot = messagesWithScreenshot.find((message) =>
          message.action === "screenshotResult" &&
          message.requestId === "browser-capture"
        );

...
Deno.test("setSelectedNode: null clears selection from the current Studio consumer", () => {
  resetState();
  state.selectedNodeId = "selected-node";
  state.selectionOverlay = makeOverlay();

  handleStudioMessage(makeEvent({ action: "setSelectedNode", id: null }));
...
Deno.test("parseStudioMessage: accepts an explicit selection clear", () => {
  assertEquals(parseStudioMessage({ action: "setSelectedNode", id: null }), {
    action: "setSelectedNode",
    id: null,
  });
});

Deno.test("handleStudioMessage: malformed messages cannot establish the session origin", () => {
  resetState();
  handleStudioMessage(makeEvent({ action: "colorMode", value: "sepia" }));

  assertEquals(
    isFromStudio({
      ...makeEvent({}),
      origin: "https://veryfront.org",
    } as MessageEvent),
    true,
  );
});

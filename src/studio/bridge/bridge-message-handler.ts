...
import { acceptStudioOrigin, postToStudio, resolveStudioEventOrigin } from "./bridge-messaging.ts";
import {
  hideOverlay,
  scrollToElement,
  setColorMode,
...
type ParsedStudioMessage =
  ...
  | { action: "setSelectedNode"; id: string | null; scroll?: boolean }
...
export function parseStudioMessage(value: unknown): ParsedStudioMessage | null {
  ...
  switch (action) {
    ...
    case "setSelectedNode": {
      ...
      const id = idValue === null ? null : readBoundedString(idValue, MAX_NODE_ID_LENGTH);
      const scroll = readOptionalBoolean(message, "scroll");
      if ((id === null && idValue !== null) || scroll === null) return null;
...
export function handleStudioMessage(event: MessageEvent): void {
  ...
  if (!acceptStudioOrigin(trustedOrigin)) return;
  ...
  switch (message.action) {
    ...
    case "setSelectedNode":
      ...
      if (message.scroll && message.id) scrollToElement(message.id);
      return;

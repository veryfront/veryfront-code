...
describe("MDX compiler validator", () => {
  ...
  describe("validateCompileParams", () => {
    it("should accept valid parameters", () => {
      expect(() => {
        validateCompileParams("test.mdx", "# Hello", validOptions);
      }).not.toThrow();
    });

    it("should reject empty filePath", () => {
      expect(() => {
        validateCompileParams("", "# Hello", validOptions);
      }).toThrow("filePath must be a non-empty string");
    });

    it("should reject non-string filePath", () => {
      expect(() => {
        validateCompileParams(null as unknown as string, "# Hello", validOptions);
      }).toThrow("filePath must be a non-empty string");

      expect(() => {
        validateCompileParams(123 as unknown as string, "# Hello", validOptions);
      }).toThrow("filePath must be a non-empty string");
    });

    it("should reject non-string content", () => {
      expect(() => {
        validateCompileParams("test.mdx", null as unknown as string, validOptions);
      }).toThrow("content must be a string");

      expect(() => {
        validateCompileParams("test.mdx", 123 as unknown as string, validOptions);
      }).toThrow("content must be a string");
    });

    it("should accept empty string content", () => {
      expect(() => {
        validateCompileParams("test.mdx", "", validOptions);
      }).not.toThrow();
    });

    it("should reject null options", () => {
      expect(() => {
        validateCompileParams("test.mdx", "# Hello", null as unknown as CompileOptions);
    ...
    it("should reject missing projectDir", () => {
      ...
      expect(() => {
        validateCompileParams("test.mdx", "# Hello", opts);
      }).toThrow("options.projectDir must be a non-empty string");
    });
    ...
    it("should reject missing outputDir", () => {
      ...
      expect(() => {
        validateCompileParams("test.mdx", "# Hello", opts);
      }).toThrow("options.outputDir must be a non-empty string");
    });
    ...
    it("should reject invalid mode", () => {
      ...
      expect(() => {
        validateCompileParams("test.mdx", "# Hello", opts);
      }).toThrow('options.mode must be either "development" or "production"');
    });
  ...
  describe("validateFileExists", () => {
    ...
    it("should throw if file does not exist and content is empty", async () => {
      await expect(validateFileExists("/nonexistent/file.mdx", ""))
        .rejects.toThrow("MDX file not found");
    });
  ...
  describe("pathExists", () => {
    ...
    it("should return false for non-existent path", async () => {
      const result = await pathExists("/nonexistent/path/to/file");
    ...
    it("should handle relative paths", async () => {
      const result = await pathExists("./src");
      expect(result).toBe(true);
    });

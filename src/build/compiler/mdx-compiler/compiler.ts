...
export function compileMDXFile(
  ...
  return withSpan(
    ...
    async () => {
      ...
      const sourcePath = relative(options.projectDir, filePath);
      logger.info(`Compiling MDX file: ${sourcePath}`);
      ...
      const frontmatter: MDXFrontmatter = normalizeMDXFrontmatter({
        ...yamlFrontmatter,
        ...exportedVars,
      });
      ...
      try {
        ...
        const moduleCode = generateModuleCode(frontmatter, compiledCode);
        const transpiledCode = await transpileCode(moduleCode, options);

        const outputPath = await writeCompiledFile(filePath, transpiledCode, options);

        logger.debug("Compiled MDX file", { sourcePath });

        return { outputPath, frontmatter, imports };
      } catch (error) {
        logger.error("Failed to compile MDX file", { sourcePath });
        throw error;
      }
    },
    {},

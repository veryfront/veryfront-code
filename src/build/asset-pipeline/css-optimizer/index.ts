...
export class CSSOptimizer {
  ...
  private servicePromise: Promise<CSSOptimizerService> | null = null;
  ...
  private async ensureService(): Promise<CSSOptimizerService> {
    ...
    if (!this.servicePromise) {
      this.servicePromise = runtime.get().then((adapter) => {
        const service = new CSSOptimizerService(adapter, this.baseDir, this.options);
  ...
  async extractCriticalCSS(cssPath: string, htmlContent: string): Promise<CriticalCSSResult> {
    ...
    return service.extractCriticalCSS(cssPath, htmlContent);
  }

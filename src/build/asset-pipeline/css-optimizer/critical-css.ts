...
import { acquireConfiguredCSSPurging, type CSSPurgingSession } from "./purging-engine.ts";
...
export function extractCriticalCSS(
  ...
  const optimizationSession = shouldMinify
    ? session ?? acquireConfiguredCSSOptimization()
    : undefined;
  const resolvedPurgingSession = purgingSession ?? acquireConfiguredCSSPurging();
  ...
  return withSpan(
    ...
    async (): Promise<CriticalCSSResult> => {
      ...
      const result = await resolvedPurgingSession.run({
      ...
      const optimizePart = (css: string, sourcePath: string): string => {
        if (!optimizationSession) return css;
        return optimizationSession.run({
          css,
          sourcePath,
          minify: true,
          sourceMap: false,
      ...
      const critical = optimizePart(result.css, "critical.css");
      const remaining = optimizePart(result.rejectedCSS, "remaining.css");

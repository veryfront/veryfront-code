...
export class CSSOptimizerService {
  ...
  private validateConfiguration(): void {
    ...
    if (
      ...
      !isContainedBuildPath(this.baseDir, inputDir) ||
      !isContainedBuildPath(this.baseDir, outputDir)
    ) {
      throw new TypeError(
    ...
    this.options.inputFiles = this.options.inputFiles.map((path) => {
      ...
      if (
        !isContainedBuildPath(this.baseDir, absolutePath) ||
        isContainedBuildPath(outputDir, absolutePath) ||
  ...
  private async planFiles(): Promise<PlannedCSSFile[]> {
    ...
    for (const sourcePath of sourceFiles) {
      ...
      const logicalRoot = isContainedBuildPath(

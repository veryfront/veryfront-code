...
describe("SSR distributed cache keys", () => {
  it("bounds long read and write keys with the same full SHA-256 identity", async () => {
    ...
    try {
      const distributed = await import("./distributed.ts?bounded-key-test");
      ...
      const unsafeRawKey = "unsafe key/(marketing)/[slug].tsx:content-hash";
      ...
      await distributed.getFromDistributedCache(rawKey);
      await distributed.setInDistributedCache(rawKey, "export default 1;");
      await distributed.getFromDistributedCache(unsafeRawKey);
      await distributed.setInDistributedCache(unsafeRawKey, "export default 1;");
      await distributed.getFromDistributedCache("short-key");
      await distributed.setInDistributedCache("short-key", "export default 1;");

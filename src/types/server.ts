...
export interface HandlerResult<TResponse extends RuntimeResponse = Response> {
...
export interface HandlerMetadata {
  ...
  name: string;
  /** Numeric sort order. Lower values run first. */
  priority: number;
...
export interface Handler<TResponse extends RuntimeResponse = Response> {
  /** Metadata used to register and match the handler. */
  metadata: HandlerMetadata;
  /** Handles one request and returns the handler-chain result. */
  handle(req: Request, ctx: HandlerContext): Promise<HandlerResult<TResponse>>;
}

/** Request middleware that can delegate to the next runtime handler. */
export type MiddlewareFunction<TResponse extends RuntimeResponse = Response> = (
  req: Request,
  ctx: HandlerContext,

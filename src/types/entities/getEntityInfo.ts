...
function isFileNotFoundError(error: unknown): boolean {
  return isPlatformNotFoundError(error);
}
    ...
    async () => {
      ...
      try {
        if (adapter) {
          ...
          content = await readAdapterEntitySource(
            filePath,
            adapter,
            shouldReadDirectly,
          );
          ...
          if (!stat.isFile) return null;
          assertEntitySourceSize(stat.size);
          const boundedReader = fs.readFileBytesWithinLimit;
          if (!boundedReader) {
            throw INVALID_ROUTE_FILE.create({
              detail: "Runtime filesystem cannot enforce the entity source limit",
            });
          }
          content = textDecoder.decode(
            await boundedReader.call(fs, filePath, MAX_ENTITY_SOURCE_BYTES),
          );
        }
      } catch (error) {
...
async function readAdapterEntitySource(
  filePath: string,
  adapter: RuntimeAdapter,
  requireBoundedRead: boolean,
): Promise<string> {
  const boundedReader = findDataMethod(
    adapter.fs,
    "readFileBytesWithinLimit",
  );
  if (boundedReader) {
    const bytes = await Reflect.apply(boundedReader, adapter.fs, [
      filePath,
      MAX_ENTITY_SOURCE_BYTES,
    ]);
    if (!(bytes instanceof Uint8Array)) {
      throw INVALID_ROUTE_FILE.create({
        detail: "Entity source adapter returned invalid bounded content",
      });
    }
    return textDecoder.decode(bytes);
  }

  if (requireBoundedRead) {
    throw INVALID_ROUTE_FILE.create({
      detail: "Hosted entity source adapter must provide a bounded reader",
    });
  }
  return await adapter.fs.readFile(filePath);
}

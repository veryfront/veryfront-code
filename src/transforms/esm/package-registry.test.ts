...
describe("package-registry adapter-backed snapshot sources", () => {
  ...
  it("validates a fresh config-derived supplied snapshot synchronously", () => {
    const resolved = resolveSuppliedDependencyPinningSnapshotSync(
      {
        projectDir: "/fresh-configured-snapshot",
        config: { react: { version: "19.1.1" } },
      },
      "on:1l85hs4d8cbtc",
      { react: "19.1.1" },
    ...
    assertEquals(resolved.cacheKey, "on:1l85hs4d8cbtc");
    assertEquals(resolved.dependencies?.react, "19.1.1");
    assertEquals(resolved.configuredVersions?.react, {
      declaration: "19.1.1",
      effective: "19.1.1",
    });
    assertEquals(Object.isFrozen(resolved), true);

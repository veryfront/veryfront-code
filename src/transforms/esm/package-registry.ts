...
export interface ReactVersionResolutionConfig {
...
export interface DependencyPinningSource {
  ...
  readonly config?: ReactVersionResolutionConfig | null;
...
/**
 * Validate and detach one caller-supplied enabled snapshot synchronously.
 *
 * This is the canonical proof shared by synchronous cache-boundary consumers
 * and the async resolver below. It deliberately owns both dependency-only and
 * configured-version key forms so those consumers cannot drift.
 */
export function resolveSuppliedDependencyPinningSnapshotSync(
  source: DependencyPinningSourceInput,
  cacheKey: string,
  dependencies: Readonly<Record<string, string>>,
): DependencyPinningSnapshot {
  ...
  return Object.freeze({
    cacheKey,
    dependencies: Object.freeze(copiedDependencies),
...
async function readProjectDependencyVersionsUncoalesced(
  ...
  try {
    ...
    // state B's identity and then treating the mixed entry as current forever.

...
describe("browser-server-exports-strip", () => {
  ...
  describe("import bindings", () => {
    ...
    it("keeps partially hook-owned destructuring and pinned JSX factory names", async () => {
      const code = [
        `/** @jsxRuntime classic */`,
        `import { loadSecret, makeFactory } from "../lib/shared.ts";`,
        `const { token, client } = loadSecret();`,
        `const React = makeFactory();`,
        `export async function getServerData() { return { props: { token, React } }; }`,
      ...
      assertStringIncludes(result, "makeFactory");
      assertStringIncludes(result, "const React");
    });

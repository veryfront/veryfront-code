...
describe("browser-server-exports-strip", () => {
  ...
  describe("what a dead declaration can pin", () => {
    ...
    it("analyzes a fallthrough consequent before an unproven case test", async () => {
      const code = [
        `import { getEnv } from "veryfront";`,
        `const KEY = getEnv("SECRET_KEY");`,
        `const cond = globalThis.flag;`,
        `(() => { switch (1) { case cond: case maybe(): missing; globalThis.registered = KEY; } })();`,
        `export async function getServerData() { return { props: { k: KEY } }; }`,
        `export default function Page() { return null; }`,
      ].join("\n");

      const error = await assertRejects(() =>
        stripServerOnlyExports(code, "pages/root-iife-switch-impure-case-prefix.tsx")
      );

      assertStringIncludes((error as Error).message, "KEY");
      assertStringIncludes(
        (error as Error).message,
        "root-iife-switch-impure-case-prefix.tsx",
      );
    });

    it("analyzes later consequents reached past an unproven case test", async () => {
      const code = [
        `import { getEnv } from "veryfront";`,
        `const KEY = getEnv("SECRET_KEY");`,
        `const cond = globalThis.flag;`,
        `(() => { switch (1) { case cond: case maybe(): case 0: break; case -1: globalThis.registered = KEY; } })();`,
        `export async function getServerData() { return { props: { k: KEY } }; }`,
        `export default function Page() { return null; }`,
      ].join("\n");

      const error = await assertRejects(() =>
        stripServerOnlyExports(code, "pages/root-iife-switch-impure-case-fallthrough.tsx")
      );

      assertStringIncludes((error as Error).message, "KEY");
      assertStringIncludes(
        (error as Error).message,
        "root-iife-switch-impure-case-fallthrough.tsx",
      );
    });
    ...
    it("evaluates catch pattern keys in order", async () => {
      const code = [
        `import { getEnv } from "veryfront";`,
        `const KEY = getEnv("SECRET_KEY");`,
        `(() => { try { throw {}; } catch ({ [missing]: x, [KEY]: y }) {} })();`,
        `export async function getServerData() { return { props: { k: KEY } }; }`,
        `export default function Page() { return null; }`,
      ].join("\n");

      const error = await assertRejects(() =>
        stripServerOnlyExports(code, "pages/root-iife-catch-pattern-order.tsx")
      );

      assertStringIncludes((error as Error).message, "KEY");
      assertStringIncludes((error as Error).message, "root-iife-catch-pattern-order.tsx");
    });

    it("enters a catch body after a safe empty object pattern", async () => {
      const code = [
        `import { getEnv } from "veryfront";`,
        `const KEY = getEnv("SECRET_KEY");`,
        `(() => { try { throw {}; } catch ({}) { globalThis.registered = KEY; } })();`,
        `export async function getServerData() { return { props: { k: KEY } }; }`,
        `export default function Page() { return null; }`,
      ].join("\n");
      ...
      const result = await stripServerOnlyExports(
        code,
        "pages/root-iife-catch-empty-pattern.tsx",
      );
    ...
    it("analyzes a for update after a matching labeled continue", async () => {
      const code = [
        `import { getEnv } from "veryfront";`,
        `const KEY = getEnv("SECRET_KEY");`,
        `(() => { outer: for (; true; (missing, KEY)) { while (true) { continue outer; } } })();`,
        `export async function getServerData() { return { props: { k: KEY } }; }`,
        `export default function Page() { return null; }`,
      ].join("\n");

      const error = await assertRejects(() =>
        stripServerOnlyExports(code, "pages/root-iife-labeled-for-update.tsx")
      );

      assertStringIncludes((error as Error).message, "KEY");
      assertStringIncludes((error as Error).message, "root-iife-labeled-for-update.tsx");
    });

    it("passes stacked labels through to their loop", async () => {
      const code = [
        `import { getEnv } from "veryfront";`,
        `const KEY = getEnv("SECRET_KEY");`,
        `(() => { outer: inner: do { continue outer; } while (false); globalThis.registered = KEY; })();`,
        `export async function getServerData() { return { props: { k: KEY } }; }`,
        `export default function Page() { return null; }`,
      ].join("\n");
    ...
    it("preserves every reachable continue target", async () => {
      const code = [
        `import { getEnv } from "veryfront";`,
        `const KEY = getEnv("SECRET_KEY");`,
        `const cond = globalThis.flag;`,
        `(() => { outer: for (;; globalThis.registered = KEY) { for (;;) { if (cond) continue; else continue outer; } } })();`,
        `export async function getServerData() { return { props: { k: KEY } }; }`,
        `export default function Page() { return null; }`,
      ].join("\n");
      ...
      const result = await stripServerOnlyExports(
        code,
        "pages/root-iife-continue-target-union.tsx",
      );
    ...
    it("analyzes a do-while test before propagating outer continues", async () => {
      const code = [
        `import { getEnv } from "veryfront";`,
        `const KEY = getEnv("SECRET_KEY");`,
        `const cond = globalThis.flag;`,
        `(() => { outer: while (true) { do { if (cond) continue; else continue outer; } while ((missing, KEY)); } })();`,
        `export async function getServerData() { return { props: { k: KEY } }; }`,
        `export default function Page() { return null; }`,
      ].join("\n");

      const error = await assertRejects(() =>
        stripServerOnlyExports(code, "pages/root-iife-do-while-continue-test.tsx")
      );

      assertStringIncludes((error as Error).message, "KEY");
      assertStringIncludes((error as Error).message, "root-iife-do-while-continue-test.tsx");
    });

    it("preserves a continue target beside a return completion", async () => {
      const code = [
        `import { getEnv } from "veryfront";`,
        `const KEY = getEnv("SECRET_KEY");`,
        `const cond = globalThis.flag;`,
        `(() => { outer: for (;; globalThis.registered = KEY) { switch (cond) { case 1: return; default: continue outer; } } })();`,
        `export async function getServerData() { return { props: { k: KEY } }; }`,
        `export default function Page() { return null; }`,
      ].join("\n");
      ...
      const result = await stripServerOnlyExports(
        code,
        "pages/root-iife-continue-return-union.tsx",
      );
    ...
    it("keeps a normal alternative inside a continuing while loop", async () => {
      const code = [
        `import { getEnv } from "veryfront";`,
        `const KEY = getEnv("SECRET_KEY");`,
        `const cond = globalThis.flag;`,
        `(() => { while (true) { if (cond) continue; } globalThis.registered = KEY; })();`,
        `export async function getServerData() { return { props: { k: KEY } }; }`,
        `export default function Page() { return null; }`,
      ].join("\n");

      const error = await assertRejects(() =>
        stripServerOnlyExports(code, "pages/root-iife-while-normal-continue.tsx")
      );

      assertStringIncludes((error as Error).message, "KEY");
      assertStringIncludes((error as Error).message, "root-iife-while-normal-continue.tsx");
    });

    it("keeps a false do-while exit beside an outer continue", async () => {
      const code = [
        `import { getEnv } from "veryfront";`,
        `const KEY = getEnv("SECRET_KEY");`,
        `const cond = globalThis.flag;`,
        `(() => { outer: for (;;) { do { if (cond) continue; else continue outer; } while (false); globalThis.registered = KEY; } })();`,
        `export async function getServerData() { return { props: { k: KEY } }; }`,
        `export default function Page() { return null; }`,
      ].join("\n");
      ...
      const result = await stripServerOnlyExports(
        code,
        "pages/root-iife-do-while-normal-outer.tsx",
      );
    ...
    it("evaluates a do-while test beside an outer continue", async () => {
      const code = [
        `import { getEnv } from "veryfront";`,
        `const KEY = getEnv("SECRET_KEY");`,
        `const cond = globalThis.flag;`,
        `(() => { outer: for (;;) { do { if (cond) continue outer; } while ((globalThis.registered = KEY, false)); return; } })();`,
        `export async function getServerData() { return { props: { k: KEY } }; }`,
        `export default function Page() { return null; }`,
      ].join("\n");
      ...
      const result = await stripServerOnlyExports(
        code,
        "pages/root-iife-do-while-normal-test.tsx",
      );
    ...
    it("evaluates an if tail beside a return alternative", async () => {
      const code = [
        `import { getEnv } from "veryfront";`,
        `const KEY = getEnv("SECRET_KEY");`,
        `const cond = globalThis.flag;`,
        `(() => { if (cond) return; globalThis.registered = KEY; })();`,
        `export async function getServerData() { return { props: { k: KEY } }; }`,
        `export default function Page() { return null; }`,
      ].join("\n");
      ...
      const result = await stripServerOnlyExports(
        code,
        "pages/root-iife-if-normal-return.tsx",
      );
    ...
    it("evaluates a for update beside an outer continue", async () => {
      const code = [
        `import { getEnv } from "veryfront";`,
        `const KEY = getEnv("SECRET_KEY");`,
        `const cond = globalThis.flag;`,
        `(() => { outer: for (;;) { for (;; globalThis.registered = KEY) { if (cond) continue outer; } } })();`,
        `export async function getServerData() { return { props: { k: KEY } }; }`,
        `export default function Page() { return null; }`,
      ].join("\n");
      ...
      const result = await stripServerOnlyExports(
        code,
        "pages/root-iife-for-update-normal-outer-continue.tsx",
      );
    ...
    it("preserves normal and break alternatives beside continue", async () => {
      const code = [
        `import { getEnv } from "veryfront";`,
        `const KEY = getEnv("SECRET_KEY");`,
        `const first = globalThis.first;`,
        `const second = globalThis.second;`,
        `(() => { for (;;) { if (first) continue; if (second) break; } globalThis.registered = KEY; })();`,
        `export async function getServerData() { return { props: { k: KEY } }; }`,
        `export default function Page() { return null; }`,
      ].join("\n");
      ...
      const result = await stripServerOnlyExports(
        code,
        "pages/root-iife-continue-normal-break.tsx",
      );
    ...
    it("enters a catch beside a continue alternative", async () => {
      const code = [
        `import { getEnv } from "veryfront";`,
        `const KEY = getEnv("SECRET_KEY");`,
        `const cond = globalThis.flag;`,
        `(() => { while (true) { try { if (cond) continue; else throw null; } catch { globalThis.registered = KEY; break; } } })();`,
        `export async function getServerData() { return { props: { k: KEY } }; }`,
        `export default function Page() { return null; }`,
      ].join("\n");
      ...
      const result = await stripServerOnlyExports(
        code,
        "pages/root-iife-catch-continue-throw.tsx",
      );
    ...
    it("initializes a catch pattern beside a continue alternative", async () => {
      const code = [
        `import { getEnv } from "veryfront";`,
        `const KEY = getEnv("SECRET_KEY");`,
        `const cond = globalThis.flag;`,
        `(() => { while (true) { try { if (cond) continue; else throw {}; } catch ({}) { globalThis.registered = KEY; break; } } })();`,
        `export async function getServerData() { return { props: { k: KEY } }; }`,
        `export default function Page() { return null; }`,
      ].join("\n");
      ...
      const result = await stripServerOnlyExports(
        code,
        "pages/root-iife-catch-pattern-continue-throw.tsx",
      );
    ...
    it("enters a catch pattern for a non-nullish throw alternative", async () => {
      const code = [
        `import { getEnv } from "veryfront";`,
        `const KEY = getEnv("SECRET_KEY");`,
        `const cond = globalThis.flag;`,
        `(() => { while (true) { try { if (cond) throw {}; else throw null; } catch ({}) { globalThis.registered = KEY; break; } } })();`,
        `export async function getServerData() { return { props: { k: KEY } }; }`,
        `export default function Page() { return null; }`,
      ].join("\n");
      ...
      const result = await stripServerOnlyExports(
        code,
        "pages/root-iife-catch-pattern-mixed-nullish.tsx",
      );
    ...
    it("enters a non-empty catch pattern for a non-nullish throw alternative", async () => {
      const code = [
        `import { getEnv } from "veryfront";`,
        `const KEY = getEnv("SECRET_KEY");`,
        `const cond = globalThis.flag;`,
        `(() => { while (true) { try { if (cond) throw {}; else throw null; } catch ({ x }) { globalThis.registered = KEY; break; } } })();`,
        `export async function getServerData() { return { props: { k: KEY } }; }`,
        `export default function Page() { return null; }`,
      ].join("\n");
      ...
      const result = await stripServerOnlyExports(
        code,
        "pages/root-iife-catch-nonempty-pattern-mixed-nullish.tsx",
      );
    ...
    it("consumes a break alternative in its owning loop", async () => {
      const code = [
        ...
        `const KEY = getEnv("SECRET_KEY");`,
        `const cond = globalThis.flag;`,
        `(() => { for (; true;) { if (cond) continue; else break; } globalThis.registered = KEY; })();`,
        `export async function getServerData() { return { props: { k: KEY } }; }`,
        `export default function Page() { return null; }`,
      ].join("\n");
      ...
      const result = await stripServerOnlyExports(
        code,
        "pages/root-iife-loop-continue-break.tsx",
      );

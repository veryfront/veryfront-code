import "#veryfront/schemas/_test-setup.ts";
import {
  assertEquals,
  assertRejects,
  assertStringIncludes,
  assertThrows,
} from "#veryfront/testing/assert.ts";
import { describe, it } from "#veryfront/testing/bdd.ts";
import type { Checkpoint } from "../types.ts";
import type { WorkflowRun } from "../types.ts";
import { MAX_WORKFLOW_CHECKPOINT_HISTORY_ENTRIES } from "../limits.ts";
import { normalizeSourceIntegrationPolicy } from "#veryfront/integrations/source-policy.ts";
import {
  appendRetainedCheckpoint,
  deleteOldestCheckpointOccurrences,
} from "./checkpoint-retention.ts";
import { MemoryBackend } from "./memory.ts";
import { CheckpointManager } from "../executor/checkpoint-manager.ts";

function checkpoint(id: string, timestamp = new Date(0)): Checkpoint {
  return {
    id,
    nodeId: id,
    timestamp,
    context: { input: {} },
    nodeStates: {},
  };
}

function run(id: string, workerId?: string): WorkflowRun {
  return {
    id,
    workflowId: "checkpoint-retention",
    status: workerId ? "running" : "pending",
    ...(workerId ? { workerId } : {}),
    input: {},
    nodeStates: {},
    currentNodes: [],
    context: { input: {} },
    checkpoints: [],
    pendingApprovals: [],
    createdAt: new Date(),
    sourceIntegrationPolicy: normalizeSourceIntegrationPolicy(undefined),
  };
}

describe("workflow checkpoint retention", () => {
  it("preserves append behavior below the shared bound", () => {
    const history = [checkpoint("first")];
    const second = checkpoint("second");

    appendRetainedCheckpoint(history, second);

    assertEquals(history.map(({ id }) => id), ["first", "second"]);
    second.id = "mutated-source";
    assertEquals(history.at(-1)?.id, "second");
  });

  it("keeps the newest bounded append order without consulting timestamps", () => {
    const history = Array.from(
      { length: MAX_WORKFLOW_CHECKPOINT_HISTORY_ENTRIES + 7 },
      (_, index) => checkpoint(`old-${index}`, new Date(Number.NaN)),
    );
    const newest = checkpoint("newest", new Date(0));

    appendRetainedCheckpoint(history, newest);

    assertEquals(history.length, MAX_WORKFLOW_CHECKPOINT_HISTORY_ENTRIES);
    assertEquals(history[0]?.id, "old-8");
    assertEquals(history.at(-1)?.id, "newest");
    newest.id = "mutated-source";
    assertEquals(history.at(-1)?.id, "newest");
  });

  it("leaves existing history unchanged when checkpoint capture fails", () => {
    const history = [checkpoint("stable")];
    const invalid = checkpoint("invalid");
    invalid.context.uncloneable = () => undefined;

    assertThrows(() => appendRetainedCheckpoint(history, invalid));
    assertEquals(history.map(({ id }) => id), ["stable"]);
  });

  it("deletes duplicate IDs by oldest occurrence rather than Set membership", () => {
    const oldest = checkpoint("same");
    oldest.nodeId = "old";
    const middle = checkpoint("keep");
    middle.nodeId = "middle";
    const newest = checkpoint("same");
    newest.nodeId = "new";

    assertEquals(
      deleteOldestCheckpointOccurrences([oldest, middle, newest], ["same"]).map(
        ({ id, nodeId }) => ({ id, nodeId }),
      ),
      [
        { id: "keep", nodeId: "middle" },
        { id: "same", nodeId: "new" },
      ],
    );
  });

  it("bounds unconditional MemoryBackend appends at the shared decoder limit", async () => {
    const backend = new MemoryBackend();
    await backend.createRun(run("unconditional"));
    for (let index = 0; index <= MAX_WORKFLOW_CHECKPOINT_HISTORY_ENTRIES; index++) {
      await backend.saveCheckpoint("unconditional", checkpoint(`cp-${index}`));
    }

    const retained = await backend.getCheckpoints("unconditional");
    assertEquals(retained.length, MAX_WORKFLOW_CHECKPOINT_HISTORY_ENTRIES);
    assertEquals(retained[0]?.id, "cp-1");
    assertEquals(retained.at(-1)?.id, `cp-${MAX_WORKFLOW_CHECKPOINT_HISTORY_ENTRIES}`);
    assertEquals((await backend.getLatestCheckpoint("unconditional"))?.id, retained.at(-1)?.id);
  });

  it("atomically bounds owned MemoryBackend appends and leaves failed fences unchanged", async () => {
    const backend = new MemoryBackend();
    const workerId = "run-execution:retention-owner";
    await backend.createRun(run("owned", workerId));
    for (let index = 0; index <= MAX_WORKFLOW_CHECKPOINT_HISTORY_ENTRIES; index++) {
      assertEquals(
        await backend.saveCheckpointIfStatusAndWorker(
          "owned",
          "owned",
          ["running"],
          workerId,
          checkpoint(`owned-${index}`),
        ),
        true,
      );
    }
    const beforeFailedFence = await backend.getCheckpoints("owned");

    assertEquals(
      await backend.saveCheckpointIfStatusAndWorker(
        "owned",
        "owned",
        ["running"],
        "run-execution:stale-owner",
        checkpoint("must-not-append"),
      ),
      false,
    );
    assertEquals(await backend.getCheckpoints("owned"), beforeFailedFence);
    assertEquals(beforeFailedFence.length, MAX_WORKFLOW_CHECKPOINT_HISTORY_ENTRIES);
    assertEquals(beforeFailedFence[0]?.id, "owned-1");
    assertEquals(
      beforeFailedFence.at(-1)?.id,
      `owned-${MAX_WORKFLOW_CHECKPOINT_HISTORY_ENTRIES}`,
    );
  });

  it("cleanup retains the newest append order when timestamps are equal", async () => {
    const backend = new MemoryBackend();
    await backend.createRun(run("cleanup-order"));
    for (let index = 0; index < 6; index++) {
      await backend.saveCheckpoint("cleanup-order", checkpoint(`equal-${index}`, new Date(0)));
    }

    await new CheckpointManager({ backend }).cleanup("cleanup-order", 2);

    assertEquals(
      (await backend.getCheckpoints("cleanup-order")).map(({ id }) => id),
      ["equal-4", "equal-5"],
    );
  });

  it("cleanup deletes only the oldest occurrence when an id crosses the cutoff", async () => {
    const backend = new MemoryBackend();
    await backend.createRun(run("cleanup-duplicate-id"));
    const oldest = checkpoint("same");
    oldest.nodeId = "old";
    const middle = checkpoint("keep");
    middle.nodeId = "middle";
    const newest = checkpoint("same");
    newest.nodeId = "new";
    await backend.saveCheckpoint("cleanup-duplicate-id", oldest);
    await backend.saveCheckpoint("cleanup-duplicate-id", middle);
    await backend.saveCheckpoint("cleanup-duplicate-id", newest);

    await new CheckpointManager({ backend }).cleanup("cleanup-duplicate-id", 2);

    assertEquals(
      (await backend.getCheckpoints("cleanup-duplicate-id")).map(({ id, nodeId }) => ({
        id,
        nodeId,
      })),
      [
        { id: "keep", nodeId: "middle" },
        { id: "same", nodeId: "new" },
      ],
    );
  });

  it("rejects an explicit resume ID that matches multiple checkpoints", async () => {
    const backend = new MemoryBackend();
    await backend.createRun(run("ambiguous-resume-id"));
    const oldest = checkpoint("same");
    oldest.nodeId = "work";
    const newest = checkpoint("same");
    newest.nodeId = "work";
    await backend.saveCheckpoint("ambiguous-resume-id", oldest);
    await backend.saveCheckpoint("ambiguous-resume-id", newest);

    const error = await assertRejects(() =>
      new CheckpointManager({ backend }).prepareResume(
        "ambiguous-resume-id",
        [{ id: "work", config: { type: "step", tool: "tool" } }],
        "same",
        "1",
      )
    );
    assertStringIncludes(
      error instanceof Error ? error.message : String(error),
      "ambiguous",
    );
  });

  it("cleanup rejects invalid keep counts before reading the backend", async () => {
    const backend = new MemoryBackend();
    let reads = 0;
    backend.getCheckpoints = (_runId: string) => {
      reads++;
      return Promise.resolve([]);
    };
    const manager = new CheckpointManager({ backend });

    for (const invalid of [-1, 0.5, Number.NaN, Number.POSITIVE_INFINITY, 2 ** 53]) {
      const error = await assertRejects(() => manager.cleanup("invalid", invalid));
      assertStringIncludes(
        error instanceof Error ? error.message : String(error),
        "non-negative safe integer",
      );
    }
    assertEquals(reads, 0);
  });

  it("cleanup with keepCount zero removes every checkpoint", async () => {
    const backend = new MemoryBackend();
    await backend.createRun(run("cleanup-zero"));
    for (let index = 0; index < 3; index++) {
      await backend.saveCheckpoint("cleanup-zero", checkpoint(`zero-${index}`));
    }

    await new CheckpointManager({ backend }).cleanup("cleanup-zero", 0);

    assertEquals(await backend.getCheckpoints("cleanup-zero"), []);
    assertEquals(await backend.getLatestCheckpoint("cleanup-zero"), null);
  });
});

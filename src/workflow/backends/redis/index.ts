...
import { AGENT_NOT_FOUND, INITIALIZATION_ERROR, INVALID_ARGUMENT } from "#veryfront/errors";
...
export class RedisBackend implements WorkflowBackend {
  ...
  async updateApproval(
    ...
    if (targetIndex === -1) {
      throw AGENT_NOT_FOUND.create({ detail: `Approval not found: ${approvalId}` });
    }

    const rawTarget = rawList[targetIndex];
    if (!rawTarget) {
      throw AGENT_NOT_FOUND.create({ detail: `Approval data not found: ${approvalId}` });
    }

...
describe("DAGExecutor", () => {
  ...
  describe("loop resume (H9)", () => {
    it("resumes owned approvals from sub-workflows generated by callback loops", async () => {
      for (const composite of ["parallel", "branch"]) {
        for (const mapped of [false, true]) {
          const repeated = loop("repeat", {
            maxIterations: 1,
            while: () => true,
          ...
          const child = mapped ? map("orders", { items: [1], processor: repeated }) : repeated;
          const nodes: WorkflowNode[] = [{
            id: "group",
            config: composite === "parallel"
              ? { type: "parallel", nodes: [child] }
              : { type: "branch", condition: () => true, then: [child], else: [] },
          }];
          const first = await executor.execute(nodes, createTestRun());
          assertEquals(first.waiting, true);
          assertExists(first.waitingNode);
          const owner = first.nodeStates[first.waitingNode]!._subWorkflowOwnerPath;
          assertExists(owner);
              ...
              createTestRun({
                ...
                nodeStates: {
                  ...
                  [first.waitingNode]: {
                    ...
                    status: "completed",
                    _subWorkflowOwnerPath: ownerMatches ? owner : "another-owner",
                  },
                },
              }),
            );
            assertEquals(second.completed, ownerMatches, `${composite}, mapped: ${mapped}`);
          }
        }
      }
    });
  ...
  describe("subWorkflow node", () => {
    ...
    it("resumes legacy approvals generated by maps inside a sub-workflow", async () => {
      for (const dynamicItems of [false, true]) {
        for (const dynamicProcessor of [false, true]) {
          const decisionId = dynamicProcessor ? "orders_0/review" : "orders_0";
          const decision: NodeState = {
            nodeId: decisionId,
            status: "completed",
            output: { approved: true },
            attempt: 1,
          };
          const nodes = [subWorkflow("release", {
            workflow: {
              id: "release-wf",
              steps: [map("orders", {
                items: dynamicItems ? () => [1] : [1],
                processor: dynamicProcessor
                  ...
                  : waitForApproval("review", { message: "Approve" }),
              })],
            },
          })];
          const result = await executor.execute(
            ...
            createTestRun({
              ...
              nodeStates: {
                release: { nodeId: "release", status: "running", attempt: 1 },
                orders: { nodeId: "orders", status: "running", attempt: 1 },
                ...(dynamicProcessor
                  ? { orders_0: { nodeId: "orders_0", status: "running" as const, attempt: 1 } }
                  : {}),
                [decisionId]: decision,
              },
            }),
          ...
          assertEquals(result.waiting, false);
          assertEquals(result.nodeStates[decisionId]?.output, { approved: true });
        }
      }
    });
    ...
    it("does not seed an unreached active child from an earlier completed branch", async () => {
      ...
      assertEquals(result.nodeStates["group/a"]?.startedAt?.getTime(), originalStartedAt.getTime());
      assertEquals(result.nodeStates["group/b"]?.status, "running");
      assertNotEquals(
        result.nodeStates["group/b"]?.startedAt?.getTime(),

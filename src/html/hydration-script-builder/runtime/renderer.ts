...
import { appendDependencyPinningVersion } from "./module-urls.ts";
...
/** App Router ownership is route metadata; release coverage only picks transport. */
export function isRemoteAppRouterClientPage(
...
export function createHydrationRenderer(deps: HydrationRendererDeps): HydrationRenderer {
  ...
  const { loadComponent, loadComponentFromUrl, resolveHydrationModuleUrl } = componentLoader;
    ...
    try {
      ...
      const loadHydrationComponent = async (
        ...
        if (preferRscModule && isAppRouterPath(normalizedPath, normalizedAppRouterRoot)) {
          ...
          return await loadComponentFromUrl(path as string, moduleUrl);
        }

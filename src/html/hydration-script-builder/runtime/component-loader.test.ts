...
describe("hydration-script-builder/runtime/component-loader", () => {
  describe("pathToModuleUrl", () => {
    it("maps a recognised source directory to a module-server URL", () => {
      ...
      assertEquals(loader.pathToModuleUrl("pages/blog.mdx"), "/_vf_modules/pages/blog.mdx");
      assertEquals(
        loader.pathToModuleUrl("project/app/layout.tsx"),
        "/_vf_modules/app/layout.tsx",
      );
    });

    it("falls back to the raw path for unrecognised directories", () => {
      const { loader } = createLoaderHarness();
      assertEquals(loader.pathToModuleUrl("custom/thing.tsx"), "/_vf_modules/custom/thing.tsx");
    ...
    it("does not version-stamp studio embed fallback module URLs", () => {
      ...
      assertEquals(
        ...
        "/_vf_modules/pages/blog.mdx?studio_embed=true",
      );
    });
    ...
    it("does not version-stamp HMR fallback module URLs", () => {
      ...
      assertEquals(loader.pathToModuleUrl("pages/blog.mdx"), "/_vf_modules/pages/blog.mdx?t=123");
    });
    ...
    it("expands release asset source variants only for extensionless paths", () => {
      const assetUrl = "/_vf/assets/" + "c".repeat(64) + ".js";
      const { loader } = createLoaderHarness();
      loader.setReleaseAssetModules({ "pages/blog.tsx": assetUrl });

      assertEquals(loader.pathToModuleUrl("pages/blog"), assetUrl);
    });

    it("never aliases an explicitly requested extension to a sibling source file", () => {
      const wrongAssetUrl = "/_vf/assets/" + "b".repeat(64) + ".js";
      const { loader } = createLoaderHarness();
      loader.setReleaseAssetModules({ "app/page.ts": wrongAssetUrl });

      assertEquals(loader.pathToModuleUrl("app/page.tsx"), "/_vf_modules/app/page.tsx");
    });
    ...
    it("ignores inherited release-map properties", () => {
      ...
      const inheritedMap = Object.create({
        "app/page.tsx": inheritedAssetUrl,
      }) as Record<string, string>;
      loader.setReleaseAssetModules(inheritedMap);
    ...
    it("keeps authored JavaScript distinct from the legacy extensionless endpoint", () => {
      ...
      assertEquals(loader.pathToModuleUrl("app/page.js"), "/_vf_modules/app/page.js.js");
      assertEquals(loader.pathToModuleUrl("app/page"), "/_vf_modules/app/page.js");
    });
    ...
    it("normalizes query and hash suffixes without weakening exact extension identity", () => {
      ...
      const { loader } = createLoaderHarness();
      loader.setReleaseAssetModules({
        "app/page.tsx": exactAssetUrl,
        "app/page.ts": "/wrong.js",
      });

      assertEquals(
        loader.pathToModuleUrl("/_vf_modules/app/page.tsx?cache=1#section"),
        exactAssetUrl,
      );
    });
    ...
    it("selects transport independently for full, partial, and unrelated release maps", () => {
      ...
      const pageData: PageDataPayload = {
        releaseAssetModules: {
          "app/page.tsx": pageAsset,
          "app/unrelated.tsx": "/unrelated.js",
        },
      };

      assertEquals(
        resolveHydrationModuleUrl("app/page.tsx", true, false, pageData),
        pageAsset,
      );
      assertEquals(
        resolveHydrationModuleUrl("app/layout.tsx", true, false, pageData),
        "/_veryfront/rsc/module?rel=app%2Flayout.tsx",
      );
      assertEquals(
        resolveHydrationModuleUrl("pages/about.tsx", false, false, pageData),
    ...
    it("ignores release assets while Studio embed or HMR is serving fresh modules", () => {
      ...
      assertEquals(loader.pathToModuleUrl("pages/blog.mdx"), "/_vf_modules/pages/blog.mdx?t=77");
    });
  });
  ...
  describe("loadComponent", () => {
    ...
    it("loads pinned components through snapshot recovery", async () => {
      ...
      assertEquals(importedUrls, ["/_vf_modules/_pins/on%3Asha-a/app/layout.tsx"]);
    });
    ...
    it("caches per dependency snapshot so two snapshots cannot share a component", async () => {
      ...
      assertEquals(importedUrls, [
        "/_vf_modules/_pins/on%3Asha-a/pages/a.tsx",
        "/_vf_modules/_pins/on%3Asha-b/pages/a.tsx",
      ]);
    });
    ...
    it("does not repopulate the cache from an import invalidated while in flight", async () => {
      ...
      const pending = loader.loadComponent("stale.tsx");
      loader.clearComponentCache("stale.tsx");
      resolveImport?.({ default: "stale" });
      assertEquals(await pending, "stale");

      const second = loader.loadComponent("stale.tsx");
      resolveImport?.({ default: "fresh" });
    ...
    it("uses true LRU eviction for the bounded component cache", async () => {
      ...
      await loader.loadComponent("first.tsx");
      for (let index = 0; index < 499; index++) {
        await loader.loadComponent(`component-${index}.tsx`);
      }
      await loader.loadComponent("first.tsx");
      await loader.loadComponent("newest.tsx");
      await loader.loadComponent("component-0.tsx");
      await loader.loadComponent("first.tsx");

      assertEquals(importedUrls.length, 502);
    });
  });
});

...
describe("hydration-script-builder/runtime/module-urls", () => {
  ...
  describe("assertPageDataMatchesDocumentSnapshot", () => {
    ...
    it("rejects pinned page data for an unpinned document", () => {
      ...
      assertEquals(thrown?.message, "Page data dependency snapshot does not match the document");
    });

    it("fails closed when the document snapshot key was malformed", () => {
      let thrown: Error | undefined;
      try {
        assertPageDataMatchesDocumentSnapshot("/x", {}, undefined);
      } catch (error) {
        thrown = error as Error;
      }

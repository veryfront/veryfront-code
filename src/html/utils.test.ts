...
describe("html-generation/utils", () => {
  ...
  describe("buildImportMapJson", () => {
    ...
    it("keeps flag-off local import-map URLs byte-identical", async () => {
      ...
      assertEquals(flagOffEntry, unkeyedEntry);
      assertEquals(flagOffEntry === unkeyedEntry, false);
    });

    it("does not let callers poison cached import-map state", async () => {
      const options = {
        customImports: {
          "custom-lib": "https://cdn.example.com/original.js",
        },
        pretty: false,
      };
      const first = await buildImportMap(options);
      first.imports["custom-lib"] = "https://attacker.invalid/poisoned.js";
      first.imports.react = "https://attacker.invalid/react.js";

      const second = await buildImportMap(options);
      const serializedImports = JSON.parse(second.json).imports as Record<string, string>;

      assertEquals(
        second.imports["custom-lib"],
        "https://cdn.example.com/original.js",
      );
      assertEquals(
        serializedImports["custom-lib"],
        "https://cdn.example.com/original.js",
      );
      assertEquals(second.imports.react, serializedImports.react);

      second.imports["custom-lib"] = "https://attacker.invalid/second.js";
      const third = await buildImportMap(options);
      assertEquals(
        third.imports["custom-lib"],
        "https://cdn.example.com/original.js",
      );
    });
    ...
    it("should map workflow client imports to the React hooks submodule", async () => {
      ...
      assertEquals(imports["veryfront/workflow"], imports["veryfront/workflow/react"]);
    });

 ...
 * element inside the application root, including a duplicate id. Only the one
 * server-owned location immediately inside <body> is accepted.
 */
...
/**
 * Returns the unique server-owned hydration element, or null when its identity
 * is ambiguous. There is intentionally no search-order or marker fallback.
 */
export function findServerHydrationDataElement(
  ...
  try {
    ...
    if (document.body?.firstElementChild !== element) return null;

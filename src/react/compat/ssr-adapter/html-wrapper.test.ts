...
function createOptions(overrides: Partial<HTMLWrapOptions> = {}): HTMLWrapOptions {
  return {
    title: "Test",
    meta: {},
    links: [],
...
describe("html-wrapper", () => {
  describe("wrapInHTML", () => {
    it("should generate complete HTML5 document", () => {
      const result = wrapInHTML("<div>Content</div>", createOptions({ title: "Test Page" }));
    ...
    it("should include page title", () => {
      const result = wrapInHTML("<div>Test</div>", createOptions({ title: "My App" }));
    ...
    it("should include head scripts", () => {
      const result = wrapInHTML(
        "<div>Test</div>",
        createOptions({ scripts: [{ src: "/script.js" }] }),
      );
    ...
    it("should include head scripts with type", () => {
      const result = wrapInHTML(
        "<div>Test</div>",
        createOptions({ scripts: [{ src: "/module.js", type: "module" }] }),
      );
    ...
    it("should wrap content in root div", () => {
      const result = wrapInHTML("<h1>Hello</h1>", createOptions());
    ...
    it("should include bootstrap scripts in body", () => {
      const result = wrapInHTML(
        "<div>Test</div>",
        createOptions({ bootstrapScripts: ["/app.js", "/vendor.js"] }),
      );
    ...
    it("should handle empty content", () => {
      const result = wrapInHTML("", createOptions());
    ...
    it("should handle empty scripts array", () => {
      ...
      const head = result.substring(0, result.indexOf("</head>"));
    ...
    it("should handle empty bootstrap scripts array", () => {
      ...
      const body = result.substring(result.indexOf("<body>"));
    ...
    it("should preserve content HTML structure", () => {
      ...
      `;
      const result = wrapInHTML(content, createOptions());
    ...
    it("should handle complete real-world example", () => {
      const options = createOptions({
        ...
        bootstrapScripts: ["/runtime.js", "/vendor.js", "/app.js"],
        nonce: "nonce-123",
      });
      ...
      expect(result).toContain(
        '<div id="root"><div class="app"><h1>Welcome</h1></div></div>',
      );

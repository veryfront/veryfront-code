  ...
  describe(`Disclosure adapter conformance — ${label}`, () => {
    it("starts closed, toggles content + aria-expanded on trigger click", () => {
      ...
      try {
        ...
        assert(!host.querySelector("[data-vf-body]"), "content absent while closed");

        click(trigger);
        assert(trigger.getAttribute("aria-expanded") === "true", "expanded after click");
        assert(host.querySelector("[data-vf-body]"), "content present while open");

        click(trigger);
        assert(trigger.getAttribute("aria-expanded") === "false", "collapsed after second click");
        assert(!host.querySelector("[data-vf-body]"), "content removed after collapse");
      } finally {
        unmount();
      }
    });
...
const AltCtx = React.createContext<{ open: boolean; toggle: () => void } | null>(null);
const altDisclosure: DisclosureParts = {
  Root: ({ open, defaultOpen, onOpenChange, disabled: _d, children, ref, ...props }) => {
    ...
    return (
      <section ref={ref} data-state={isOpen ? "open" : "closed"} {...props}>
        <AltCtx.Provider value={{ open: isOpen, toggle }}>{children}</AltCtx.Provider>
      </section>
    );
  },

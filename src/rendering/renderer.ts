...
export class Renderer {
  ...
  private scheduleProductionRenderRefresh(
    ...
    setTimeout(() => {
      void (async () => {
        const renderToken = await this.beginProjectRender(ctx.projectId);
        try {
          await this.doRenderPage(slug, ctx, refreshOptions, performance.now(), {
            ...cachePolicy,
            check: false,
            singleflight: false,
          }, renderToken);
  ...
  private async doRenderPage(
    ...
    const runRender = async () => {
      try {
        const result = await this.runRenderWithCapacity(
          slug,
          ctx,
          options,
          startTime,
        ...
        return snapshotRenderResult(result);
      } catch (error) {
    ...
    const cachedData = cachePolicy.cacheKey !== null
  ...
  private createServicesForContext(
    ...
    const { virtualModules, componentRegistry } = this.getContextServices(ctx);
...
export async function initializeRenderer(options?: RendererOptions): Promise<Renderer> {
  ...
  let initialization!: Promise<Renderer>;

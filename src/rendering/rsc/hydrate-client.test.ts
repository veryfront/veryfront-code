...
import {
  admitSnapshotBoundHydrationManifest,
...
describe("rendering/rsc/hydrate-client", () => {
  ...
  it("rejects manifest snapshot disagreement before hydration can import modules", () => {
    const manifest = {
      version: 1,
      hash: "manifest-a",
    ...
    const admitted = admitSnapshotBoundHydrationManifest(manifest, {
    ...
    assertEquals(admitted, null);
    assertEquals(recoveries, 1);
    assertEquals(imports, 0);
  });

  it("requires exact manifest header and body authorities only when pinning is enabled", () => {
    const manifest = {
      version: 1,
      hash: "manifest-a",
    ...
    assertEquals(
      admitSnapshotBoundHydrationManifest(manifest, {
        requestedDependencyPinningCacheKey: "on:1",
        currentDependencyPinningCacheKey: "on:1",
        responseHeaderDependencyPinningCacheKey: "on:1",

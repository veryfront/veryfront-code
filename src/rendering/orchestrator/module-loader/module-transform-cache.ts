...
function serializeTransformConfigIdentity(
  ...
  dependencyPinningCacheVariant?: string,
): string {
  const identity = [
    projectDir,
    mode,
  ...
  if (dependencyPinningCacheVariant) {
    identity.push(dependencyPinningCacheVariant);
  }
  return `[${identity.map((value) => JSONStringify(value)).join(",")}]`;
}
...
export async function transformModuleCodeWithCache(
  ...
  const moduleServerOrigin = dependencyPinningCacheVariant ? input.moduleServerOrigin : undefined;

...
export default tool({
  ...
  inputSchema: z.object({
    ...
    dealstage: z.string().optional().describe(
    ...
    closedate: z.string().optional().describe(
      "Expected close date in format YYYY-MM-DD or timestamp",
    ),
  }),
  async execute({ dealname, amount, dealstage, pipeline, closedate }) {
    ...
    return {
      ...
      message: `Successfully created deal: ${formatDealName(deal)}`,
    };
  },
});

...
export default tool({
  ...
  inputSchema: z.object({
    scope: z.enum(["created_by_me", "assigned_to_me", "all"]).default("all").describe(
      "Scope of merge requests to list",
    ),
    state: z.enum(["opened", "closed", "merged", "all"]).default("opened").describe(
      "State of merge requests to list",
    ),
    labels: z.array(z.string()).optional().describe(
      'Filter by labels (e.g., ["feature", "review-needed"])',
    ),
    ...
    return {
      ...
      mergeRequests: mergeRequests.map((mr) => ({
        ...
        description: mr.description?.substring(0, 200) +
          (mr.description && mr.description.length > 200 ? "..." : ""),
      })),
      summary: mergeRequests.map((mr) => formatMergeRequestForDisplay(mr)).join("\n\n"),
    };
  },
});

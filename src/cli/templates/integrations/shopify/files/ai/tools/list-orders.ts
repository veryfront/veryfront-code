...
export default tool({
  ...
  inputSchema: z.object({
    limit: z.number().min(1).max(250).default(20).describe("Maximum number of orders to return"),
    status: z.enum(["open", "closed", "cancelled", "any"]).optional().describe("Filter by order status"),
    financialStatus: z.enum(["pending", "authorized", "paid", "refunded", "voided"]).optional().describe("Filter by financial status"),

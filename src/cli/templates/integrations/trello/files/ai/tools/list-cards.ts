...
export default tool({
  ...
  inputSchema: z.object({
    ...
    includeArchived: z.boolean().default(false).describe("Include archived/closed cards"),
    limit: z.number().min(1).max(100).default(50).describe("Maximum number of cards to return"),
  }),
  async execute({ boardId, listId, includeArchived, limit }) {
    if (!boardId && !listId) {
      return {
        cards: [],
        message: "Please specify either a boardId or listId",
      };
    }

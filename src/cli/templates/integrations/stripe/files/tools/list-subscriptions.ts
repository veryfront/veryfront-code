...
export default tool({
  ...
  inputSchema: z.object({
    limit: z.number().min(1).max(100).default(10).describe(
      "Maximum number of subscriptions to retrieve",
    ),
    ...
    status: z.enum([
      ...
      "unpaid",
      "paused",
    ]).optional().describe("Filter by subscription status"),
    createdAfter: z.number().optional().describe(
      "Filter subscriptions created after this Unix timestamp",
    ),
    createdBefore: z.number().optional().describe(
      "Filter subscriptions created before this Unix timestamp",
    ),
  }),

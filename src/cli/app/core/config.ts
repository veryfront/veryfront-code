...
export async function loadConfig(): Promise<ConfigState> {
  ...
  try {
    // Load preferences
    const prefContent = await Deno.readTextFile(getConfigPath());
  ...
  try {
    // Load history
    const histContent = await Deno.readTextFile(getHistoryPath());
  ...
  try {
    ...
    const recentContent = await Deno.readTextFile(getRecentPath());

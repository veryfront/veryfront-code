...
export default defineConfig({ styles: { stylesheet: "src/styles/app.css" } });`,
      },
    ...
    await runReleaseAssetBuild({
      ...
      loadConfig: (source) => {
        ...
        return releaseConfigLoader({
          styles: { stylesheet: "src/styles/app.css" },
        })(source);
      },
    }, await tmp());

    assertEquals(seenStylesheet, ":root { --brand: blue; } /* release-config */");
    assertEquals(seenConfig?.styles?.stylesheet, "src/styles/app.css");
    ...
    const files = [
      {
        path: "veryfront.config.ts",
        content: `import { defineConfig } from "veryfront";
export default defineConfig({ styles: { stylesheet: "src/styles/release.css" } });`,
      },
    ...
    await runReleaseAssetBuild({
      ...baseInput(client, transform),
      loadConfig: releaseConfigLoader({
        styles: { stylesheet: "src/styles/release.css" },
      }),
    }, await tmp());
  ...
  it("fails closed when the configured release stylesheet is missing", async () => {
    ...
    const result = await runReleaseAssetBuild({
      ...baseInput(client, (source) => Promise.resolve(source)),
      loadConfig: releaseConfigLoader({
        styles: { stylesheet: "src/styles/missing.css" },
      }),
    }, await tmp());
...
export default { styles: { stylesheet: "src/styles/imported.css" } };`,
      },
      { path: "globals.css", content: "/* fallback stylesheet that should not be used */" },
      {
    ...
    const result = await runReleaseAssetBuild({
      ...baseInput(client, transform),
      loadConfig: releaseConfigLoader({
        styles: { stylesheet: "src/styles/imported.css" },
      }),
    }, await tmp());

    assertEquals(result.success, true);
    assertEquals(seenStylesheet, ":root { --brand: blue; } /* imported release config */");
    assertEquals(seenConfig?.styles?.stylesheet, "src/styles/imported.css");
  });
    ...
    const files = [
      {
        path: "veryfront.config.ts",
        content: `import { defineConfig } from "veryfront";
export default defineConfig({ styles: { stylesheet: "globals.css" }, typoKey: true });`,
      },
...
const shared = { styles: { stylesheet: getEnv("VERYFRONT_RELEASE_CONFIG_STYLESHEET") } };
export default defineConfigWithEnv((env) =>
  mergeConfigs(shared, { react: { version: env === "production" ? "19.2.8" : "19.2.9" } })
    ...
    await runReleaseAssetBuild({
      ...baseInput(client, transform),
      loadConfig: releaseConfigLoader({
        styles: { stylesheet: "src/styles/helper.css" },

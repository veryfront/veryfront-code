...
| Layer                                                | State                                                |
| ---------------------------------------------------- | ---------------------------------------------------- |
| React version                                        | yes 19.2.4 (matches latest shadcn)                   |
| Tailwind version                                     | yes v4.2.2, CSS-first (matches latest shadcn)        |
| `@radix-ui/*`, `cva`, `clsx`, `lucide-react` imports | yes auto-resolve via esm.sh with React dedupe        |
| `@/...` path aliases                                 | yes resolve to project root by default               |
| Tailwind `@theme` / CSS-var theming / `@plugin`      | yes supported                                        |
| `@plugin "tailwindcss-animate"`                      | yes **verified** - emits keyframes + `animate-in`    |
| `@import "tw-animate-css"` (current shadcn)          | no **verified no-op** - silently dropped             |
| `npx shadcn init/add` CLI                            | no no Deno/Veryfront target                          |
| Version pinning                                      | warning shadcn emits unversioned imports -> warnings |
| `cn` = `twMerge(clsx())` semantics                   | warning works, but a v4 config-awareness caveat      |
| Component registry / docs                            | no none for Veryfront                                |
...
**SSR target:** every npm package returns `null` -> left native. That works only when the app has installed those packages or mapped them in `deno.json` / `package.json`; a newly scaffolded app cannot SSR a copied `clsx`, Radix, or `tailwind-merge` import until that dependency step exists.
...
### 2.2 Committed Veryfront UI primitives use the same substrate

This checkout does not include the production app tree, so this RFC does not claim production proof from an uncommitted app. The committed evidence is narrower: `src/react/components/ui/*` uses React 19-compatible primitives, class-variant-like composition, portal/floating behavior, and the same Tailwind token model that shadcn components rely on. The external render testbed linked in Appendix B provides the end-to-end npm-package proof.
...
- `@import "tailwindcss"`, `@theme { --color-* }`, `@custom-variant dark (&:is([data-theme="dark"] *))`, and `@plugin "@tailwindcss/typography"` all work through the CSS-first pipeline.
- `@plugin` bundles load at runtime from esm.sh via `globalThis` shims so plugin code binds to the same Tailwind copy - **verified**: `@plugin "tailwindcss-animate"` emits `@keyframes enter/exit` + `animate-in` (section 3 P2b). Note the asymmetry: `@plugin` fetches from esm.sh, but stylesheet `@import` of anything other than `"tailwindcss"` is dropped (P2b).
...
**Fix:** ship a version-pinning step. For browser builds, rewrite generated source imports to explicit versions or add a browser-aware mapping stage before `bareStrategy` rewrites bare specifiers. A `deno.json` import-map block is still useful for SSR dependency resolution, but it does not currently silence browser warnings or pin emitted esm.sh URLs because browser bare imports are rewritten before import-map handling.

### P3 - `cn` / tailwind-merge correctness caveat

shadcn's canonical `cn` is `twMerge(clsx(inputs))`. `tailwind-merge` **does resolve** (section 2.1), so it works out of the box. But two caveats worth documenting:
...
**Load-bearing runtime fact (verified):** the framework's own SSR'd `veryfront/chat` primitives use **real `react-dom` `createPortal`** (`src/react/components/ui/{floating,tooltip}.tsx`) and hydrate through the framework provider stack. The external render testbed also rendered and hydrated the real npm packages listed above, so the portal -> SSR -> hydrate path is now proven for both the framework primitives and the package set covered by that suite.

### Tier A - resolves + portal/headless pattern proven
...
### Tier B - resolves + verified by the external render testbed
...
1. **Portals** - mechanism verified in-framework and by the external npm-package render testbed. Keep a portal smoke test in the starter because regressions here are high-impact.
...
3. **Version floating** (P2) - unpinned esm.sh imports can pull **two** copies of the same lib -> duplicate React context (e.g. two `react-hook-form`/`cmdk` instances), which silently breaks. Pinning (P2 fix) also _dedupes_. Higher stakes for context-heavy Tier-A libs than for the button.
...
2. **Pin-on-add codemod** - after `shadcn add`, rewrite source imports to a Veryfront-tested pinned set for browser builds, and add matching `deno.json` / package metadata for SSR. Kills P2, gives reproducible builds.
3. **Blessed `cn`** exported from `veryfront/ui` (real `twMerge`, pinned v4-compatible), referenced by the starter's `lib/utils.ts`.
4. **Veryfront registry** (`registry.json`) so `shadcn add @veryfront/button` pulls variants already tuned to the `@theme` token vocabulary. Seed it from committed framework primitives or a dedicated example package, not an uncommitted production tree.
...
- **Does Veryfront interop with shadcn?** Yes. Same React 19 + Tailwind v4; all shadcn deps auto-resolve with React dedupe in browser builds, and SSR works once the app installs or maps the copied dependencies.
...
- **Do the complex/niche components work (Dialog, Select, Command, Form, Chart, Carousel, Drawer, Data Table, OTP, Toast...)?** Every dep **resolves** (verified sweep), and the external 19-component testbed rendered and hydrated the real packages. The remaining caveats are package CSS imports, version floating, and SSR module-eval / DOM-read behavior. Full matrix in section 3B.
- **So what's missing for _seamless_?** Tooling, not runtime: a `shadcn` CLI target/preset, version pinning, a documented `cn`, and a registry/docs - plus the P2b `@import` fix. See section 3.
...
- Auto-resolution: `src/transforms/import-rewriter/strategies/bare-strategy.ts` (+ verified run, section 2.1)
- `@/` alias: `src/transforms/import-rewriter/strategies/alias-strategy.ts`
- Committed UI substrate: `src/react/components/ui/*`, `src/utils/clsx.ts`, and `src/react/runtime/*`
...
- **Import resolution (section 2.1):** drove `bareStrategy.rewrite()` over the verbatim import block of `npx shadcn@latest add button` (`button.tsx` + `lib/utils.ts`), browser + ssr targets.

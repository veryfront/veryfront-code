...
| Component registry / docs                            | no Veryfront-specific docs or registry               |
...
### Tier A - resolves + portal/headless pattern proven in the external testbed
...
### Tier B - resolves + DOM-measuring cases proven in the external testbed
...
- Browser auto-resolution: `src/transforms/import-rewriter/strategies/bare-strategy.ts` (+ verified run, section 2.1)
...
- **Import resolution (section 2.1):** drove `bareStrategy.rewrite()` over the verbatim import block of `npx shadcn@latest add button` (`button.tsx` + `lib/utils.ts`), browser + SSR targets. Browser rewrites to esm.sh and warns on unversioned deps; SSR leaves package specifiers native.
- **SSR dependency requirement (section 2.1/P2):** checked `ImportMapStrategy.matches()` and `cli/commands/init/deno-config-generator.ts`. Import maps are SSR-only in this path, and the scaffold does not create shadcn dependency mappings.

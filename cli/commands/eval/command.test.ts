...
describe("eval CLI command helpers", () => {
  ...
  it("exposes request-scoped mock tools eagerly", async () => {
    ...
    const model: ModelRuntime = {
      provider: "hosted",
      modelId: "hosted/eval-tool-search-mocks",
      async doGenerate() {
        modelStep++;
        if (modelStep === 1) {
          return {
        ...
        return {
          content: [{ type: "text", text: "Release marker rel_eval" }],
          finishReason: "stop",
          usage: { inputTokens: 1, outputTokens: 1, totalTokens: 2 },
        };
      },
    ...
    const definition = evalAgent({
      id: "eval:tool-search-mocks",
      target: "agent:assistant",
      dataset: datasets.inline([{ id: "release", input: "Read the release marker" }]),
      mockTools: {
        read_release_marker: tool({
          id: "read_release_marker",
          description: "Read the current release marker.",
    ...
    assertEquals(markerExecutions, 1);
    assertEquals(

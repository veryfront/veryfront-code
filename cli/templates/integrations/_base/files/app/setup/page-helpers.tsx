...
const TOKEN_STORAGE_MODES = new Set<TokenStorageStatus["mode"]>([
  "memory",
  "database",
  "kv",
  "redis",
...
export function isTokenStorageStatus(
  value: unknown,
): value is TokenStorageStatus {
  if (!value || typeof value !== "object") return false;
  const candidate = value as Record<string, unknown>;
  return typeof candidate.mode === "string" &&
    TOKEN_STORAGE_MODES.has(candidate.mode as TokenStorageStatus["mode"]) &&
...
export const OAUTH_SETUP_GUIDES: Record<string, SetupGuide> = {
  ...
  sheets: {
    ...
    steps: [
      ...
      "Authorize scopes: https://www.googleapis.com/auth/spreadsheets, https://www.googleapis.com/auth/drive.readonly, https://www.googleapis.com/auth/drive.file",
  ...
  onedrive: {
    ...
    steps: [
      ...
      "Add permissions: Files.Read, Files.ReadWrite, Files.Read.All, Files.ReadWrite.All, offline_access",
  ...
  sharepoint: {
    ...
    steps: [
      "Uses same Microsoft OAuth credentials",
      "Add permissions: Sites.Read.All, Sites.ReadWrite.All, Files.Read.All, Files.ReadWrite.All, offline_access",
  ...
  bitbucket: {
    ...
    steps: [
      ...
      "Add permissions: repository, pullrequest:write, issue, account",
  ...
  github: {
    ...
    steps: [
      ...
      "Authorize scopes: repo, read:user, read:org",
      "Copy Client ID and Secret to .env",
    ],
...
export function getTokenStorageStyles(
  ...
  const isDurable = tokenStorage.durable;

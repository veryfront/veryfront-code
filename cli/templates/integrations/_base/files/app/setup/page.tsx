...
import {
  ...
  type Integration,
  isTokenStorageStatus,
  OAUTH_SETUP_GUIDES,
  ServiceIcon,
  type TokenStorageStatus,
} from "./page-helpers.tsx";
...
export default function SetupPage(): React.JSX.Element {
  ...
  const [tokenStorage, setTokenStorage] = useState<TokenStorageStatus | null>(
    null,
  );
  ...
  async function fetchTokenStorage(): Promise<void> {
    try {
      ...
      setTokenStorage(isTokenStorageStatus(data) ? data : null);
    } catch {
      setTokenStorage(null);
    }
  }
  ...
  const tokenStorageStyles = useMemo(
    () => getTokenStorageStyles(tokenStorage),
    [tokenStorage],
  );
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                <h3 className={tokenStorageStyles.title}>
                  Token Storage: {!tokenStorageStyles.isDurable
                    ? "Development Mode"
                    : `${tokenStorage.mode.charAt(0).toUpperCase()}${
                      tokenStorage.mode.slice(
                        1,
                      )
                    } Storage`}
                </h3>
                ...
                <p className={tokenStorageStyles.text}>
                  {!tokenStorageStyles.isDurable
                    ? <>Tokens are not durable and may be lost on restart.</>
                    : <>Tokens are persisted to {tokenStorage.mode} storage.</>}
                </p>

                <div
                  className={`mt-2 flex items-center gap-1.5 text-sm ${
                    tokenStorage.encrypted === true
                ...
                >
                  ...
                  <span>
                    {tokenStorage.encrypted === true
                      ? "Storage adapter reports encryption at rest"
                      : tokenStorage.encrypted === false
                      ? "Storage adapter reports no encryption at rest"
                      : "Storage adapter cannot attest to encryption at rest"}
                  </span>
                </div>
            ...
            : (
              <div>
                {CATEGORIES.filter((cat) =>
                  (groupedIntegrations[cat.id]?.length ?? 0) > 0
                ).map(
                  (category) => (
                    <div key={category.id}>
                      ...
                      <div className="divide-y divide-neutral-200 dark:divide-neutral-700">
                        {groupedIntegrations[category.id]?.map(
                          (integration) => {
                                ...
                                <div className="p-6 flex items-center justify-between">
                                  ...
                                  <div className="flex items-center gap-3">
                                    ...
                                    {integration.connected
                                      ...
                                      : (
                                        <span className="inline-flex items-center px-4 py-2 bg-amber-50 dark:bg-amber-900/20 text-amber-700 dark:text-amber-300 rounded-xl text-sm font-medium">
        ...
        {allConnected && (
          <div className="mt-8 bg-gradient-to-r from-green-50 to-emerald-50 dark:from-green-900/20 dark:to-emerald-900/20 rounded-2xl p-6 border border-green-200 dark:border-green-800 text-center">
            ...
            <p className="text-green-700 dark:text-green-300 mb-4">
              Your AI agent is ready to use. Start chatting to automate your
              workflows.
            </p>
            <a
              href="/"

    ...
    <div className="flex flex-col h-screen bg-white dark:bg-neutral-900">
      <header className="sticky top-0 z-10 flex-shrink-0 border-b border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900">
        <div className="px-4 py-3 flex items-center justify-between">
          <h1 className="font-medium text-neutral-900 dark:text-white">
            AI Agent
          </h1>
    ...
    <div className="flex items-center gap-2">
      {connected.map((service) => (
        <span
          ...
          title={`${service.name} ${
            service.connectionState === "configured"
              ? "configured"
              : "connected"
          }`}
        >
      ...
      {disconnected.map((service) => {
        const action = service.connectUrl ? "Connect" : "Configure";
        return (
          <a
            key={service.id}
            href={service.connectUrl ?? "/setup"}

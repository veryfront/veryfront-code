...
export function createGitLabClient(userId: string) {
  function gitlabFetch<T>(
    endpoint: string,
    options: RequestInit = {},
  ): Promise<T> {
  ...
  function getCurrentUser(): Promise<GitLabUser> {
  ...
  function getProject(projectId: number | string): Promise<GitLabProject> {
  ...
  function listMergeRequests(options?: {
    scope?: "created_by_me" | "assigned_to_me" | "all";
  ...
  function addMergeRequestComment(
    projectId: number | string,
    mrIid: number,
  ...
  function formatIssueForDisplay(issue: GitLabIssue): string {
...
URL: ${issue.web_url}`;
  ...
  function formatMergeRequestForDisplay(mr: GitLabMergeRequest): string {

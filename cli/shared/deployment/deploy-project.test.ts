...
describe("DeployProject", () => {
  ...
  it("rejects a gate challenge when routing convergence is unconfirmed", async () => {
    await withDeployEnv(async () => {
      const { projectDir } = await createPushedProject();
      const controlPlane = new InMemoryDeployControlPlane();
      controlPlane.environmentProtected = true;
      ...
      try {
        ...
        assertStringIncludes((error as Error).message, "routing convergence was not confirmed");
      } finally {
        await Deno.remove(projectDir, { recursive: true });
      }
    });

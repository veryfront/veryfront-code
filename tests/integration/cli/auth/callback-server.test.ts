...
/**
 * Callback server integration tests use Deno-specific APIs (Deno.serve,
  ...
  () => {
    ...
    describe("callback handling", {
      ...
      it("should receive token when callback state matches", async () => {
        ...
        setTimeout(() => {
          void fetchAndCancel(
            `${callbackUrl}?token=test-oauth-token&state=expected-state`,
          );
        }, scaleMs(100));
      ...
      it("should reject callback token when state is mismatched", async () => {
        ...
        setTimeout(() => {
          void fetchAndCancel(
            `${callbackUrl}?token=test-oauth-token&state=wrong-state`,
          );
        }, scaleMs(100));
      ...
      it("should return 404 for non-callback paths", async () => {
        ...
        const response = await fetch(
          `http://localhost:${server.port}/other-path`,
        );

...
import { testEnterPinnedParent } from "../../../../../cli/scaffold/secure-writer.ts";
...
describe("scaffold missing-parent containment", () => {
  it("rejects a checked parent moved outside the project before chdir", async () => {
    const projectDir = await makeTempDir({ prefix: "vf-scaffold-enter-project-" });
    const outside = await makeTempDir({ prefix: "vf-scaffold-enter-outside-" });

...
it("snapshot provider capture rejects unsupported hosts before invoking proxy traps", async () => {
  const script = `
    ...
    const { canIdentifyProxyWithoutHooks } = await import("./src/platform/compat/error-introspection.ts");
    const { createDependencySnapshotStoreHandle } = await import("./src/platform/adapters/dependency-snapshot-store.ts");
    let traps = 0;
    ...
    const { DependencySnapshotRegistry } = await import("./src/transforms/esm/dependency-snapshot-registry.ts");
    const { createDependencyPinningSnapshot, hashDependencyPins } = await import("./src/transforms/esm/dependency-snapshot.ts");

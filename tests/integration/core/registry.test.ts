  ...
  describe("Edge Cases", () => {
    it("should handle race conditions during initialization", async () => {
      await withTestContext("registry-race", async (context) => {
        ...
        await Promise.all([reg.discover(), reg.discover(), reg.discover()]);
    ...
    it("rejects component name conflicts within a directory tree", async () => {
      await withTestContext("registry-conflict", async (context) => {
        ...
        await assertRejects(
          () => reg.discover(),
          TypeError,
          'Duplicate component name "Component"',
        );
      });
    });
    ...
    it("rejects components with the same name in different roots", async () => {
      await withTestContext("registry-same-name", async (context) => {
        ...
        await writeTextFile(join(componentsDir, "Button.tsx"), "export default function B1(){}");
    ...
    it("should handle get() before initialization", async () => {
      await withTestContext("registry-get-before-init", async (context) => {
        const reg = await createRegistry(context.projectDir);
    ...
    it("preserves explicit components during filesystem discovery", async () => {

...
for (const probe of ["baseline", "strings", "arrays"]) {
  describe(`private framework tool search ${probe}`, () => {
    for (const mode of ["generate", "stream"] as const) {
      it(`loads and executes a matching tool in ${mode} while preserving structured cache metadata`, async () => {
        ...
        try {
          ...
          output = mode === "stream"
            ? await (await runtime.stream({ input: "Find and use the lookup tool" }))
              .toDataStreamResponse().text()
            : (await runtime.generate({ input: "Find and use the lookup tool" })).text;
        } finally {

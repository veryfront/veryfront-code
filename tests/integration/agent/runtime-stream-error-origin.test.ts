...
async function streamProviderPartBody(
  mode: "legacy" | "active",
  ...
  return await streamBody(runtimeAgent);
}
  ...
  for (const mode of ["legacy", "active"] as const) {
    ...
    it(`redacts ${mode} provider-part normalization failures`, async () => {
      await withLifecycleMode(mode, async () => {
        ...
        const body = await streamProviderPartBody(mode, providerPart, "type-private");
    ...
    it(`redacts ${mode} provider-part decoder failures`, async () => {
      await withLifecycleMode(mode, async () => {
        ...
        const body = await streamProviderPartBody(mode, providerPart, "text-private");
    ...
    for (const field of ["toolName", "input", "output"] as const) {
      it(`redacts ${mode} provider ${field} materialization failures`, async () => {
        await withLifecycleMode(mode, async () => {
          const privateMarker = `private-provider-${field}-${mode}`;
          let accessorReads = 0;
          const hostileValue = Object.defineProperty({}, "private", {
            enumerable: true,
            get() {
              accessorReads += 1;
              if (accessorReads > 1) throw new Error(privateMarker);
              return "first read must not escape provider materialization";
            },
          });
          const providerPart = field === "toolName"
            ? Object.defineProperty(
              { type: "tool-call", toolCallId: "tool-1", input: {} },
              "toolName",
              {
                enumerable: true,
                get() {
          ...
          const body = await streamProviderPartBody(mode, providerPart, `${field}-private`);

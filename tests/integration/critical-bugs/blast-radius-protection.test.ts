...
describe("Blast Radius Protection", {
    ...
    it("syntax error in Project A does not affect Project B rendering", async () => {
      await withTestContext("blast-syntax-healthy", async (healthyContext) => {
        await withTestContext("blast-syntax-broken", async (brokenContext) => {
          ...
          try {
            // Create healthy renderer first
            const healthyRenderer = await createRenderer({
            ...
            let _brokenRenderError: Error | null = null;
            ...
            try {
              ...
              // Try to render the broken page
              await brokenRenderer.renderPage("/");
            } catch (_e) {
              _brokenRenderError = _e as Error;
            ...
            if (brokenRenderer) {
              try {
                const _brokenResult = await brokenRenderer.renderPage("/");
                // If it didn't throw, verify it at least has an error state
                // or doesn't contain "broken" content
              } catch (_e) {
            ...
            if (healthyRenderer && typeof healthyRenderer.clearAllState === "function") {
              await healthyRenderer.clearAllState();
            }
            if (brokenRenderer && typeof brokenRenderer.clearAllState === "function") {
              await brokenRenderer.clearAllState();
            }
          } finally {
      ...
      await withTestContext("blast-runtime-healthy", async (healthyContext) => {
        await withTestContext("blast-runtime-broken", async (brokenContext) => {
          ...
          try {
            ...
            const brokenRenderer = await createRenderer({
              projectDir: brokenContext.projectDir,
              mode: "development",
            });
    ...
    it("slow renders in Project A do not block Project B", async () => {
      await withTestContext("blast-slow-project", async (slowContext) => {
        await withTestContext("blast-fast-project", async (fastContext) => {
          // Slow project with simulated delay component
          ...
          try {
            const slowRenderer = await createRenderer({
              projectDir: slowContext.projectDir,
              mode: "development",
            });

            const fastRenderer = await createRenderer({
            ...
            const fastResult = await fastRenderer.renderPage("/");
            ...
            if (slowRenderer && typeof slowRenderer.clearAllState === "function") {
              await slowRenderer.clearAllState();
            }
            if (fastRenderer && typeof fastRenderer.clearAllState === "function") {
              await fastRenderer.clearAllState();
            }
          } finally {
            await cleanupBundler();
          }
    ...
    it("error state from Project A does not appear in Project B", async () => {
      await withTestContext("blast-error-state-a", async (contextA) => {
        await withTestContext("blast-error-state-b", async (contextB) => {
          // Project A: Will generate an error
          ...
          try {
            ...
            // Trigger error in Project A multiple times
            for (let i = 0; i < 3; i++) {
              try {
                await rendererA.renderPage("/");
              } catch (_e) {
                // Expected
              }
            }
    ...
    it("corrupted cache entry does not affect other projects", async () => {
      await withTestContext("blast-cache-corrupt", async (corruptContext) => {
        await withTestContext("blast-cache-clean", async (cleanContext) => {
          // Project that will have a failing transform
          ...
          try {
            ...
            try {
              ...
              await corruptRenderer.renderPage("/");
            } catch (_e) {
              // Expected
            }
    ...
    it("missing file in Project A does not crash Project B", async () => {
      await withTestContext("blast-missing-a", async (contextA) => {
        await withTestContext("blast-missing-b", async (contextB) => {
          // Project A: References a non-existent component
          await mkdir(join(contextA.projectDir, "app"), { recursive: true });
          await writeTextFile(
            join(contextA.projectDir, "app", "layout.tsx"),
            `export default function Layout({ children }) {
          ...
          // Project B: Normal working project
          await mkdir(join(contextB.projectDir, "app"), { recursive: true });
          await writeTextFile(
            join(contextB.projectDir, "app", "layout.tsx"),
            `export default function Layout({ children }) {
          ...
          try {
            ...
            // Verify B works first
            const result1 = await rendererB.renderPage("/");
            assertStringIncludes(result1.html, "Project B Works", "B should work initially");

            // Try to render A (will fail due to missing import)
            try {
              const rendererA = await createRenderer({
                projectDir: contextA.projectDir,
                mode: "development",

...
import { createRunsClient } from "../../src/runs/index.ts";
...
// === Guide: runs.mdx ===
describe("Guide: runs.mdx", () => {
  it("should create a task run and read canonical events", async () => {
    const responses = [
      jsonResponse({
        accepted: true,
        run: {
          ...
          kind: "task",
          status: "pending",
          owner: { kind: "project", id: "22222222-2222-4222-8222-222222222222" },
          parent_run_id: null,
    ...
    await withMockedFetch(responses, async (calls) => {
      ...
      assertEquals(accepted.run.kind, "task");
      assertEquals(events.data.length, 1);
      assert(calls[0]?.url.includes("/runs"));
      assert(calls[1]?.url.includes(`/runs/${accepted.run.run_id}/events`));
    });
  });
});
